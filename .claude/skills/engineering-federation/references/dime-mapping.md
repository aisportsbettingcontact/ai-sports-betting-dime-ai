...
- Identity for limiter keys: `ipKeyGenerator(req.ip)` under `trust proxy = 1` only. Never key on client-writable headers; never trust the leftmost `X-Forwarded-For` entry for anything but forensics.
