...
9. **Prove live** — for production-boundary changes, rendered/runtime proof via the `verify` skill or deploy-smoke, not unit tests alone.
10. **Record** — fill the evidence record (`references/routing.md` template) and close with a terminal outcome from the fixed enum. A partially implemented change is never `shipped`.

## Routing

| Work | Lead | Notes |
| --- | --- | --- |
| Architecture design, audit, modernization, incident analysis | `architect-backend-systems` | Its own architecture-standard governs design method; evidence + controls come from this federation |
| Repo-wide structure audits, dead/duplicated files | `architect-github-repos` | |
...
Exact invocation surfaces, precedence detail, and the evidence-record template: `references/routing.md`. Control-by-control repo mapping and gate commands: `references/dime-mapping.md`.
...
- **If the change touches traffic control (rate limits, timeouts, retries)** → declare the per-route-class failure policy (fail-open vs fail-closed, from the mapping's table) and derive client identity only from trust-proxy-resolved `req.ip` — never from client-controlled forwarding headers; `/health` is never limited (Railway healthcheck kills the deploy otherwise).
- **If retrying a mutation** → only under an idempotency contract; non-idempotent retries are a defect.
- **If claiming done** → the evidence record exists with verbatim gate output and a terminal outcome from: `shipped | rejected | halted_attempts | halted_budget | halted_permission | halted_environment | failed_verification`.

## Common mistakes

| Mistake | Fix |
| --- | --- |
| Prose "done" with no evidence record | Fill the §21.3 record (template in routing.md); outcome from the enum |
