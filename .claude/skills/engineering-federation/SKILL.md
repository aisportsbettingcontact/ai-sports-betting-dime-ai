...
- **If the change touches traffic control (rate limits, timeouts, retries)** → declare the per-route-class failure policy (fail-open vs fail-closed, from the mapping's table) and **never hand-roll client identity at a call site** — call the shared resolver in `server/_core/clientIdentity.ts` (`clientIdentityKey` for limiter keys, `resolveClientIdentity` for the raw IP, `identitySource` to record which tier answered). Which header is authoritative depends on whether the request is cryptographically proven to have come through the Cloudflare edge, and getting it wrong silently merges or splits limiter budgets — dime-mapping.md §11.4 carries the resolution order and the reasoning. `req.ip` is the last-resort fallback inside that resolver, not the rule; `clientIpKey`/`resolveClientIp` are deprecated aliases, not new API. `clientIdentityCallSites.test.ts` fails the build if a hand-rolled derivation reappears. `/health` is never limited (Railway healthcheck kills the deploy otherwise).
