...
5. **Observe** — rendered proof via the `verify` skill (production build + boot + smoke) and screenshots at 1440×900 and 390×844 including a reduced-motion pass. `/sp-verify` is command-output evidence (tsc/tests/build) — it never substitutes for rendered proof.
