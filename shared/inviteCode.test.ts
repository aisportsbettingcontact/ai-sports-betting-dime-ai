...
describe("invite code codec", () => {
  it("round-trips uid + token", () => {
    ...
    expect(parseInviteCode(code)).toEqual({ uid: 1680001, token: "invite_token_fixture-0" });
  });

  it("round-trips every uid shape including small ids", () => {
    for (const uid of [1, 12, 480001, 1680001, 9007199254740]) {
      const parsed = parseInviteCode(buildInviteCode(uid, "aaaaaaaaaaaaaaaaaaaaaa"));
  ...
  it("rejects malformed codes", () => {
    for (const bad of ["", "nodot", ".leading", "trailing.", "0.token1234567890123456789", "-1.aaaaaaaaaaaaaaaaaaaaaa", "abc.short", "a b.aaaaaaaaaaaaaaaaaaaaaa", "abc.aaaaaaaaaaaaaaaaaaaaaa.extra"]) {
