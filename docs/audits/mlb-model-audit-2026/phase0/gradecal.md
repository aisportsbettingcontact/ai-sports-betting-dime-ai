...
3. **Nightly outcome ingestion + f5_share drift + auto-recalibration** — `server/mlbOutcomeIngestor.ts` + `server/mlbDriftDetector.ts`, orchestrated by `server/mlbOutcomeAndDriftScheduler.ts` (all VERIFIED, whole files read). Writes `games.actualFgTotal/actualF5Total/actualNrfiBinary/brier*` and `mlb_drift_state`, `mlb_model_learning_log`, `mlb_calibration_constants` (seed only). Its production liveness is **UNKNOWN** — see Scheduling. [FIXED in Phase 4 — M-208 added `POST /api/cron/mlb-outcomes` (also runs the internal drift check), `/api/cron/mlb-closing-capture`, and `/api/cron/mlb-backtest` to `server/cron/cronRoutes.ts`, so ingestion no longer depends on the in-process scheduler; monthly recalibration still has no HTTP trigger, and no GitHub workflow files for the new endpoints exist yet]
...
- **The `games` table's own grading columns (`fgMlResult`, `fgRlResult`, `fgTotalResult`, `fgMlCorrect`, `fgRlCorrect`, `fgTotalCorrect`, `f5MlResult/f5RlResult/f5TotalResult/f5*Correct`, `nrfiBacktestResult`, `nrfiCorrect`, `fgBacktestRunAt/f5BacktestRunAt/nrfiBacktestRunAt`) have NO writer in the current codebase.** VERIFIED at the pre-fix commit: repo-wide grep finds them only in `drizzle/schema.ts`, read-only usages in `server/routers.ts:121-138` (field-strip list), `server/routers/mlbSchedule.ts:719-1080` (SELECTs for admin pages), and client-side readers of those query results (`client/src/pages/F5EdgeLeaderboard.tsx`, `ModelResults.tsx`, `TheModelResults.tsx` — correction: the original enumeration omitted these), plus this audit's own tool `docs/audits/mlb-model-audit-2026/tools/grade-season.mjs`. Whatever populated them is deleted history (candidates in `git log --diff-filter=D`: `run_full_backtest_march25_april5.ts`, `generate_backtest_report.ts`, etc. — UNKNOWN which). [FIXED in Phase 4 — M-101 forward path: `ingestMlbOutcomes` now computes model-pick grades (`computeModelPickGrades`, grading the model's pick rather than the abandoned fixed-away-side convention) and writes all of these columns plus the `*BacktestRunAt` stamps on every ingest]
- **`mlb_calibration_constants` is a write-only table.** VERIFIED at the pre-fix commit: the only code touching it is the seed in `server/mlbDriftDetector.ts:210-244` (INSERT-if-missing), one-off scripts `scripts/recompute_calibration.mjs` / `scripts/recalibrate_and_model_may23.mjs` (writers), and read-only diagnostic dumps `server/auditDataSources.mjs` / `server/auditHrPropsGap.mjs` (correction: omitted from the original enumeration; they SELECT for audit printouts, not runtime consumption). No SELECT anywhere in `server/` or `client/` consumes its values at runtime. Every "calibration constant" the live model actually uses is a hard-coded literal in `server/MLBAIModel.py`, `server/mlbKPropsModelService.ts`, or `server/mlbHrPropsModelService.ts` (see constants map below). [FIXED in Phase 4 (partial) — M-207: `mlbModelRunner.ts` now reads `fg_ml_home_edge` + `league_env_mult` (5-min cache, injected into the Python engine via `DIME_FG_ML_HOME_EDGE`/`DIME_LEAGUE_ENV_MULT` env), `kPropsDbHelpers.getKCalibrationFactors` reads `k_calibration_factor_over`/`_under` (new param names — not yet present in the DB census, so defaults apply until Phase 5 writes them), and `mlbHrPropsModelService.ts` reads `hr_calibration_factor`; hard-coded values remain as fallbacks]
...
| Model probabilities being graded | `games.model*` columns written by the model runner (e.g. `modelPNrfi` written 0–1 at `server/mlbModelRunner.ts:2507` pre-fix), `mlb_hr_props.modelPHr`, `mlb_strikeout_props.kProj` | — | — |
...
| `fg_ml_home_edge` 0.01781473 | No DB read (pre-fix) | `MLBAIModel.py:1617` `FG_ML_HOME_EDGE = 0.03` (comment cites the DB param, `:1616`) [FIXED in Phase 4 — now `_env_float("DIME_FG_ML_HOME_EDGE", 0.03)`, injected from the DB row by `mlbModelRunner.loadCalibrationConstant`] | **MISMATCH** pre-fix (0.0178 vs 0.03); post-fix the DB value wins |
| `k_calibration_factor` 0.776 (prev 0.739) | No DB read (pre-fix) | `mlbKPropsModelService.ts:88-89` `K_CALIBRATION_FACTOR_OVER = 0.870`, `_UNDER = 0.810` (applied to lambda at `:467-468`) [FIXED in Phase 4 — factors now read from `k_calibration_factor_over`/`_under` rows via `kPropsDbHelpers.getKCalibrationFactors`; those rows don't exist yet, so 0.870/0.810 defaults still apply] | **MISMATCH** (DB single 0.776 vs code split 0.870/0.810) |
| `hr_base_rate` 0.1009 | No DB read | HR model uses `HR_CALIBRATION_FACTOR = 0.5317` (`mlbHrPropsModelService.ts:97`) — the DB row records the empirical rate, not a code constant [FIXED in Phase 4 — factor renamed `HR_CALIBRATION_FACTOR_FALLBACK`; live value read per cycle from `hr_calibration_factor` row (M-207), and lambda basis corrected to AB (M-212)] | informational only |
...
| 12× `brier_*`, 12× `ece_*`, 12× `log_loss_*` (updateSource `backtest_2026_recalibration_v2`) [corrected: TSV has 12 brier rows, not 13] | No consumer — metric snapshots from `scripts/recompute_calibration.mjs` | — | orphans |

INFERRED conclusion: the entire `mlb_calibration_constants` table is an audit trail, not a control plane. **Every published probability derives from hard-coded literals**, several of which have drifted from the DB's "current" values. [FIXED in Phase 4 (partial) — M-207 wires `fg_ml_home_edge`, `league_env_mult`, `k_calibration_factor_over/_under`, and `hr_calibration_factor` into live reads with the old literals as fallbacks; the remaining rows (bias_correction_*, metric snapshots, f5_share, nrfi_rate, f5_under_bias) are still unconsumed]
...
Written **only** by `scripts/run_full_season_backtest.mjs` (one-off, `auditVersion='v2026-full-audit-1.0'`, season 2026-03-25→2026-05-22, `:39-41` [corrected from `:43`]):
...
- Historical bulk rows: `scripts/run_full_season_backtest.mjs` upsert `ON DUPLICATE KEY UPDATE` over 34 columns [corrected from 33] (`:622-646`), `modelSide = market` string (not side).
...
UI: `/admin/backtest` → `client/src/pages/MlbBacktest.tsx` (full report, time series, edge buckets, K/HR reports — `MlbBacktest.tsx:111-127`); `/admin/model-results` → `TheModelResults.tsx` (rolling accuracy panel `:740`, drift banner via `mlbSchedule.checkDrift` `:1036`, `:1317-1323`); routes at `client/src/App.tsx:315-351`. **CORRECTED**: the original claim "no public surface exposes grading directly (public feed strips grading columns)" is wrong. `stripSportNullFields` removes the MLB grading/Brier fields only from **non-MLB** games (`routers.ts:148` `if (sport !== 'MLB') for (const f of mlbOnlyFields) delete result[f]`); the `alwaysStrip` list (`:86-94`) drops only the `*BacktestRunAt`/`outcomeIngestedAt` stamps. So MLB rows returned by the **public** `games.list` procedure (`publicProcedure`, `routers.ts:311-336`) carry `fgMlResult…nrfiCorrect` and `brier*` on the wire. Also note the report/rolling-accuracy procedures are `protectedProcedure` (any logged-in user), not admin-gated — only the run/trigger mutations are `ownerProcedure`; the "/admin" gating is a client-side route convention.
...
- **Nightly outcome/drift pipeline**: `startMlbOutcomeAndDriftScheduler()` 60-second tick; nightly at 00:30 PST ingests **yesterday** then runs `checkF5ShareDrift(true)` (`mlbOutcomeAndDriftScheduler.ts:97-165`, `:230-244`); monthly recal 1st @ 03:00 PST (`:213-228`). **Critical**: there is **no `/api/cron/*` endpoint or GitHub workflow** covering outcome ingestion, drift, or recalibration (VERIFIED pre-fix: `cronRoutes.ts` registers only vsin-odds, scores, mlb-cycle, mlb-asg, status, `:80-119`). If `DISABLE_BACKGROUND_JOBS=1` in production, nightly Brier/outcome/drift/monthly-recal **never run** (F3). [FIXED in Phase 4 (partial) — M-208: `cronRoutes.ts` now also registers `POST /api/cron/mlb-outcomes` (ingest, `?date=` or last 2 PT days; drift check runs inside `ingestMlbOutcomes`), `POST /api/cron/mlb-closing-capture`, and `POST /api/cron/mlb-backtest` (enrolls final games missing `mlb_game_backtest` rows, last 3 ET days). Still missing: GitHub workflow files for these endpoints, and any HTTP trigger for the monthly full recalibration] Note `ingestMlbOutcomes` also runs its own internal drift check with `triggerRecal` defaulting true (`mlbOutcomeIngestor.ts:700`, default at `mlbDriftDetector.ts:379`), so the scheduler's Step 2 is a duplicate check (24 h cooldown prevents double recal).
...
7. **Who wrote `games.fgMlResult...nrfiCorrect` and when did that writer die?** Are `/admin/model-results` panels reading these columns showing stale (pre-deletion) data? [FIXED in Phase 4 going forward — `ingestMlbOutcomes` is now the writer (model-pick grades, M-101); the historical who/when question remains open, and per M-101 the abandoned writer graded a fixed away-side bet]
...
| F2 | P1 | `mlb_calibration_constants` is a write-only audit table; live model constants are hard-coded and have drifted from DB "current" values (fg_ml_home_edge 0.03 vs 0.0178; nrfi_rate 0.5093 vs 0.5106; k_calibration_factor 0.870/0.810 vs 0.776) [FIXED in Phase 4 (partial) — M-207 adds runtime reads for fg_ml_home_edge, league_env_mult, k_calibration_factor_over/_under, hr_calibration_factor with old literals as fallbacks; nrfi_rate and the other EMPIRICAL_PRIORS remain hard-coded] | Grep: no runtime SELECT (only seed `mlbDriftDetector.ts:210-244` — itself uncalled — and one-off scripts); `MLBAIModel.py:1617`, `:387`; `mlbKPropsModelService.ts:88-89`; census TSV rows 40, 42, 55. VERIFIED code / VERIFIED TSV |
| F3 | P1 | Nightly outcome ingestion, Brier writes, drift detection and monthly recalibration have no production trigger if `DISABLE_BACKGROUND_JOBS=1` (which repo comments say is set on Railway) — no cron endpoint/workflow covers them [FIXED in Phase 4 (partial) — M-208 added `/api/cron/mlb-outcomes` + `/mlb-closing-capture` + `/mlb-backtest` endpoints; no GH workflows for them yet and monthly recal still uncovered] | Guard `server/_core/index.ts:840-873`; cron scope `cronRoutes.ts:80-119` (pre-fix); flag asserted set at `cronRoutes.ts:46-48` (pre-fix); runbook `references/railway-deploy.md:98-107`. VERIFIED code, env value UNKNOWN |
...
| F5 | P1 | `games.brierNrfi` is computed with a /100 rescale of `modelPNrfi`, which is stored 0–1 — NRFI Brier scores (owner Brier-trend chart) are systematically wrong. Verifier addition: the same pre-fix bug also hit `brierF5Total` (`modelF5OverRate` is likewise stored 0–1, was divided by 100 at pre-fix `:218`) [FIXED in Phase 4 — M-203: `brierScore` now takes a pre-normalized [0,1] p; call sites normalize explicitly via `probFromPct` (0–100 columns) / `probFromUnit` (`modelPNrfi`, `modelF5OverRate`)] | writer 0–1: `mlbModelRunner.ts:2507` (pre-fix); ingestor divides by 100: `mlbOutcomeIngestor.ts:156-167` + `:224-227` (pre-fix); consumed by `mlbSchedule.getBrierTrend` (`routers/mlbSchedule.ts:443-562`). VERIFIED code path; DB values UNKNOWN |
...
| F8 | P2 | CLV is advertised (column, math, spec) but never computed: every writer writes NULL closing odds/CLV; the only implementation (`calcCLV`) lives in dead code | `mlbBacktestAuditCore.ts:330-338` (unused in prod); `run_full_season_backtest.mjs:454-457`; live writer omits columns (`mlbMultiMarketBacktest.ts:885-904`). VERIFIED — still true post-Phase-4 (the live writer is unchanged); note remediation tooling now exists (`docs/audits/mlb-model-audit-2026/tools/remediation/b9-clv.mjs`) and closing-line locking got a cron trigger (`/api/cron/mlb-closing-capture` → `captureClosingLines`, which stamps `mlb_schedule_history`, not `mlb_game_backtest.clv`) |
...
| F12 | P2 | `games` grading columns (`fgMlResult`…`nrfiCorrect`) have no writer in the codebase yet are read by admin Model Results pages — panels are frozen at whatever the deleted writer last wrote [FIXED in Phase 4 — M-101 forward path: `ingestMlbOutcomes` now writes the full set as model-pick grades on every ingest, nulls included so stale away-side-era values can't survive under a fresh stamp] | grep results; readers `routers/mlbSchedule.ts:719-1080`, strip list `routers.ts:121-138`. VERIFIED absence of writer pre-fix |
...
*Prepared for the 2026 MLB model forensic audit, phase 0. All citations refer to repo state at commit c9b5b903 (clean working tree). Verifier note: for the 8 files touched by the Phase 4 fix commit 6bce4e36 (`MLBAIModel.py`, `cron/cronRoutes.ts`, `kPropsDbHelpers.ts`, `mlbHrPropsModelService.ts`, `mlbKPropsModelService.ts`, `mlbModelRunner.ts`, `mlbOutcomeIngestor.ts`, `mlbScoreRefresh.ts`), cited line numbers were verified against the pre-fix tree (1ccf0fa5, byte-identical to c9b5b903 for all `server/` files); all other files are unchanged and were verified against the working tree.*

---

## Verification (re-run)

Adversarial re-verification performed 2026-07-25 at commit b0578e60 (pre-fix baseline for changed files: `git show 1ccf0fa5:…`; Phase 4 fixes: 6bce4e36). Every load-bearing claim was checked against code: all constants/thresholds in `mlbMultiMarketBacktest.ts` (0.65 / 0.05 / 0.06 / 0.18 / 0.55 / 0.60 / 2.0σ / n=20 / k=0.4 / version tag, lines 48-83, 310), the MARKETS map (`:86-103`), every grading-rule row in the market table (ML `:237-273`, RL `:277-375`, totals `:379-453`, F5 `:459-641`, NRFI/YRFI `:648-707`, HR `:715-784`, K delegation `:1023-1034`), drift + write paths (`:816-861`, `:867-945`), the F1 uppercase-keys bug (`:1144`), K/HR prop services (all cited lines), the outcome ingestor (pre-fix Brier /100 at `:156-167`/`:224-227`, write `:609-622`, matching `:508-534`/`:838-857`), drift detector (all constants `:59-71`, paths `:98-127`, state `:139-201`, seed `:210-244` with no caller, window `:318`, cooldown `:354-366`, recal `:570-825`), scheduler times (`mlbOutcomeAndDriftScheduler.ts:97-165`, `:210-244`, `:270-274`), the dead audit-core family (grades `:43`, P/L `:277-285`, CLV `:330-338`, EDT `:393-397`, leakage `:409-439`, preflight `:603-655`, VOID props `:980-982`/`:1023-1025`; importers = test + intra-family only — `mlb_publish_audit.mjs`/`mlb_state_audit.mjs` mention names in strings only), walk-forward/calibration-audit/segmentation constants, `mlbFullBacktestEngine.ts` thresholds (`:76-93`, `:72`, `:201`, `:315-317`), schema facts (`modelProb decimal(5,2)` `:1530`, `leakageSafe .default(1)` `:1578`, `uq_backtest_game_market` `:1594`, census `schema-columns.tsv` rows 225/229/255), pre-fix `MLBAIModel.py` (`:387`, `:389`, `:1182-1184`, `:1616-1617`), K/HR model constants, `mlbModelRunner.ts:2507`, season/recompute/may23 scripts (leakage filter also at `recompute_calibration.mjs:98`), Dockerfile `:14-19` (no pip), `runMlbBacktest2.py:124` (`mysql.connector`), `cron-mlb-cycle.yml` (`:4-11`, cron at `:36`), `_core/index.ts` guard (`:132-135`, `:840-926`, `:846`, `:869-873`), all cited tRPC procedures and client pages, census `calibration-constants.tsv` values (rows 38-42, 55), the deleted-writer candidates in `git log --diff-filter=D`, and `mlbFeedbackLoop.test.ts` importing only vitest.

**Tally: 120 claims checked — 114 confirmed, 6 corrected, 0 unbacked.**

Corrections applied inline:

1. **Exposure (material)**: "public feed strips grading columns" was wrong — `stripSportNullFields` strips MLB grading/Brier fields only from non-MLB games; public `games.list` (`publicProcedure`) returns them on MLB rows. Report procedures are `protectedProcedure`, not admin-gated.
2. Census TSV has **12** `brier_*` rows, not 13 (12+12+12 metric rows + 12 bias_correction + 6 named params = 54).
3. `run_full_season_backtest.mjs` upsert covers **34** columns, not 33; season constants are at `:39-41`, not `:43`.
4. The grading-column reference enumeration omitted three client-side readers (`F5EdgeLeaderboard.tsx`, `ModelResults.tsx`, `TheModelResults.tsx`).
5. The `mlb_calibration_constants` toucher enumeration omitted read-only diagnostics `server/auditDataSources.mjs` / `server/auditHrPropsGap.mjs`; updateSource list omitted `LIVE_2026_N8491`.
6. F5 scope: the pre-fix /100 Brier bug also affected `brierF5Total` (`modelF5OverRate` is stored 0–1), not just `brierNrfi`.

Phase 4 fix annotations added (claims that were TRUE pre-fix, now addressed): headline fact 1 + F12 + open question 7 (games grading columns now written by `ingestMlbOutcomes`, M-101); headline fact 2 + F2 + constants map (M-207 runtime reads for fg_ml_home_edge / league_env_mult / k_calibration_factor_over+under / hr_calibration_factor, hard-coded fallbacks retained); F3 + Scheduling + Overview §3 (M-208 cron endpoints `/api/cron/mlb-outcomes`, `/mlb-closing-capture`, `/mlb-backtest`; no GH workflows yet, monthly recal still untriggered); F5 (M-203 Brier scale normalization). Unchanged and still valid post-fix: F1, F4, F6, F7, F8, F9, F10, F11, F13, F14, F15, F16 (the fix commit did not touch `mlbMultiMarketBacktest.ts`, the Dockerfile, `runMlbBacktest2.py`, the dead audit family, or the K substitution fallback). Minor cite drift not corrected (within one line, statement anchored correctly): schema comment at `:1529` vs cited `:1530`; `railway-deploy.md:107-111` vs cited `:108-112`.
