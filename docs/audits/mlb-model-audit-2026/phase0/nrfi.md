...
> ⚠️ **ERRATUM (2026-08-07) — the 6 `[FIXED in Phase 4]` annotation(s) in this file are superseded.**
...
- `server/mlbOutcomeIngestor.ts` independently derives `nrfiBinary` (1 = scoreless I1) from a linescore-hydrated schedule call and writes `games.actualNrfiBinary` + `games.brierNrfi` (VERIFIED, `server/mlbOutcomeIngestor.ts:313-319`; DB write at `606-623` [corrected from 585-601 — that range is the staging/log block]).
...
- `games.actualNrfiBinary`, `games.brierNrfi`, `games.outcomeIngestedAt` — nightly outcome ingestor (VERIFIED, `server/mlbOutcomeIngestor.ts:606-623` write block [corrected from "585-601 region"]; brier at `223-227`).
- `games.nrfiBacktestResult`, `games.nrfiCorrect`, `games.nrfiBacktestRunAt` — **no writer found anywhere in the repo** (VERIFIED absence via repo-wide grep at pre-fix commit `1ccf0fa5`; only readers `server/routers/mlbSchedule.ts:1029, 1080` and the audit census tool). [FIXED in Phase 4 — commit `6bce4e36` adds `gradeNrfiPick` (pick = NRFI when P(NRFI) > 0.5, no push) to `server/mlbOutcomeIngestor.ts`, and the nightly ingest now writes `nrfiBacktestResult`/`nrfiCorrect`/`nrfiBacktestRunAt` going forward; historical rows remain null until backfill.]
...
- **Backtests**: `server/mlbMultiMarketBacktest.ts` evaluates NRFI/YRFI vs `nrfiActualResult` using `modelPNrfi` (treated as 0–1) or model odds, no-vig book prob from `nrfiOverOdds`/`yrfiUnderOdds`, confidence 0.55 (VERIFIED, `mlbMultiMarketBacktest.ts:644-701`); `server/mlbFullBacktestEngine.ts` defines the `nrfi`/`yrfi` breakeven 0.524 and min-edge 0.05 (VERIFIED, `mlbFullBacktestEngine.ts:69-70, 89`) but does **not** grade games itself [CORRECTED — the file is 835 lines, cited lines `924-963, 1078-1079` do not exist, and there is no `nrfiResult` reference in it: it aggregates already-graded `mlb_game_backtest` rows (`213-240`), and `runHistoricalBacktestRange` (`787-835`) delegates re-grading to `runMultiMarketBacktestForDate`; all NRFI grading from actuals happens in `mlbMultiMarketBacktest.evaluateNrfi` against `nrfiActualResult` (`nrfiResult` is only a local variable inside `mlbScoreRefresh.ts`)].
...
| `startVsinAutoRefresh()` → `runMlbCycleOnce()` | every 5 min (`MLB_INTERVAL_MS`), immediate on boot | Step 1 score refresh (`refreshMlbScoresNow` → writes `nrfiActualResult` on final); Step 5 lineups watcher — on lineup change it calls plain `runMlbModelForDate(dateStr)` with **no** `targetGameIds`/`forceRerun` (`server/mlbLineupsWatcher.ts:582-583`), so the modelRunAt same-date skip guard makes it a no-op for games already modeled that day [CORRECTED — the "targeted forceRerun" pattern belongs to the RL-invalidation re-run and the routers.ts Layer3 path, not the lineups watcher]; Step 6 fallback full model run for today+tomorrow; Step 7 F5/NRFI odds scrape — gated to after 7:00 AM EST (12:00 UTC), today only | `server/vsinAutoRefresh.ts:1361, 1704-1710, 1819, 1859-1875, 1996-2020, 2068, 2096-2101`; started `_core/index.ts:846` |
| `startMlbOutcomeAndDriftScheduler()` | 60s tick; nightly pipeline at 12:30 AM PST (ingests yesterday); monthly recalibration 1st @ 3:00 AM PST | Nightly: `ingestMlbOutcomes` (writes `actualNrfiBinary`, `brierNrfi`; since Phase 4 also `nrfiBacktestResult`/`nrfiCorrect`/`nrfiBacktestRunAt`) then f5-share drift check. Monthly: `triggerRecalibration("SCHEDULED")` → runs `server/scripts/runMlbBacktest2.py` → writes calibration JSON → **regex-patches `EMPIRICAL_PRIORS` in MLBAIModel.py in place** (including `nrfi_rate`, `i1_share`) with a `.bak` backup | `server/mlbOutcomeAndDriftScheduler.ts:207-244, 265-277`; `server/mlbDriftDetector.ts:570-664, 729-824 (nrfi_rate patch at 785-786)`; started `_core/index.ts:873` |
...
2. **Are `games.nrfiBacktestResult` / `nrfiCorrect` / `nrfiBacktestRunAt` populated at all?** No writer in pre-fix code; ModelResults UI reads `nrfiCorrect`. [FIXED in Phase 4 — the nightly ingestor now writes them going forward; the census question narrows to whether historical rows get backfilled.] Census: non-null counts + last `nrfiBacktestRunAt`.
...
4. **brierNrfi distribution.** If finding 1 is right, `brierNrfi` should be bimodal at ≈0.99 (NRFI outcomes) and ≈0.00002 (YRFI outcomes), average ≈0.5. [Finding 1's forward path is FIXED in Phase 4; this census question now dates the cutover and sizes the poisoned historical rows.] Census: histogram of `brierNrfi`.
...
| 9 | **P3** | `nrfiBacktestResult`/`nrfiCorrect`/`nrfiBacktestRunAt` have no writers but are exposed in API and rendered in results UI (permanently null cells) | absence VERIFIED via grep at pre-fix `1ccf0fa5`; readers `routers/mlbSchedule.ts:1029, 1080`, `ModelResults.tsx:1109`. [FIXED in Phase 4 — commit `6bce4e36` adds `gradeNrfiPick` + writes of all three columns to the nightly `mlbOutcomeIngestor`; forward path only, historical rows stay null until backfill] |
...
Adversarial re-verification performed 2026-07-25 against the pre-fix tree (`git show 1ccf0fa5:...` = HEAD before the Phase 4 root-cause-fixes commit `6bce4e36`, which the dossier describes) and the current working tree for files Phase 4 did not touch. Line cites for the four Phase-4-touched files (`MLBAIModel.py`, `mlbModelRunner.ts`, `mlbOutcomeIngestor.ts`, `mlbScoreRefresh.ts`) were checked against the pre-fix versions; all other cites against the working tree. No DB queries were run.

**Tally: 82 load-bearing claims checked — 78 confirmed, 4 corrected (2 material, 2 cite-only), 0 unbacked.**

Confirmed (spot list of the highest-leverage ones):

- All NRFI constants and their exact values/lines in pre-fix `MLBAIModel.py`: `SIMULATIONS=400_000` (68), `INNING1_RUN_SHARE=0.1166` (103), `LEAGUE_NRFI_PRIOR=0.8899` (105), `NRFI_SHRINKAGE_K=10` (107), `NRFI_MIN_STARTS_FULL=20` (109), `TEAM_NRFI_RATES` PIT 0.5765…COL/NYY 0.4559 (272-303), league mean 0.5150 (304), weights 0.35/0.15/0.50 (350-352), Python thresholds 0.52/0.54 (361-366), `EMPIRICAL_PRIORS["nrfi_rate"]=0.5093` (387).
- The 50/35/15 blend, 0.65/0.35 and 0.50/0.50 fallbacks, `_apply_nrfi_shrinkage` (starts=None bypass at 996), `_nrfi_to_mu` clamp [0.01,0.99], pitcher crossing at `project_game` (2842-2845), team-rate auto-resolve (2478-2481), p_nrfi from `(home_1st==0)&(away_1st==0)` (1150-1152).
- The dead `variance` argument: `_r_base,_p_base` assigned at 798 and never used; per-draw `max(adj_mu*1.5, adj_mu+0.5)` at 809; gamma shape 4.0; clip [0.3,3.0]; exactly 24 `self.dist.sample()` call sites per game (2 FG + 2 I1 + 2 F5 + 18 per-inning).
- Both competing combined-signal implementations: Python geometric ≥0.52 computed and discarded (result interface `mlbModelRunner.ts:264-283` has no field; write at 2555 uses the TS value); TS arithmetic ≥0.56 at 1872-1877 persisted at 2554-2558 — still arithmetic/0.56 in the post-fix tree (now lines 1912-1913), i.e. finding 2 is NOT fixed by Phase 4.
- Write path: `modelPNrfi = String(r.p_nrfi.toFixed(4))` at 2507 vs `decimal(5,2)` (census tsv:141, schema 475-476 with wrong "(0-100)" comment); no `modelPYrfi`; inning JSON arrays; `inning_p_both_score`/std never persisted; modelable gate 1586-1618.
- Scraper/AN chain: `/usr/bin/python3.11` (scraper:25), FD NJ book "69" + `periods=event,firstfiveinnings,firstinning` (AN py:43-45), odds swap `nrfiOverOdds ← nrfi.underOdds` with comments (scraper:225-229), doubleheader array-shift with no gameNumber verification (193-195); `book_lines` = ML/RL/total only (runner:1753-1762, 1854).
- Grading: scoreRefresh NRFI derivation 434-443 + final-game write 660-680; ingestor `nrfiBinary` 313-319; pre-fix `brierScore` /100 bug 155-166 + 223-227.
- No writer of `mlb_pitcher_stats.nrfiRate/nrfiStarts/nrfiCount` anywhere (repo-wide grep; Junis one-off scripts update only general stats and read nrfiRate); `seedPitcherStats.ts` update-set omits / insert lacks the NRFI columns (205-243).
- Schedulers: 5-min model sync + 15-min watchdog + self-healing (runner 2673-2674, 2799-2825), started at `_core/index.ts:846/873/877` exactly; vsin `MLB_INTERVAL_MS` 5-min (1361), Step 7 scrape gated ≥12:00 UTC and today-only (1996-2020); outcome/drift 60s tick, nightly 00:30 PST, monthly 1st 03:00 PST (scheduler 207-244, 265-277); drift baseline nrfi_rate 0.4882 n=5103 at detector:216; `patchConstant` quoted-dict-key regex (762-780), `nrfi_rate` patch (786-788), `i1_share` patch (791-793), gate-only use of `EMPIRICAL_PRIORS["nrfi_rate"]` (pre-fix 1182, post-fix 1215 — still gate-only); `runMlbBacktest2.py` exists at `server/scripts/`.
- UI/exposure: routers MLB-only field set (124-131) stripped for non-MLB; cheat-sheet ×100 handling, physics-vs-blended dual display (pNeitherI1 at 776/803/627 vs modelPNrfi at 795-798), badge 818-819/858-866, edge flag |edge|≥0.03; dead `MlbF5NrfiCard` (only reference = import `ModelProjections.tsx:23`; f5nrfi tab renders `CheatSheetView` at 1694) with the latent 100× bug (263-272, 132-150); multi-market `evaluateNrfi` (648-705, threshold 0.55 at 78); scoreGrader/betTracker NRFI timeframes; census tsv rows 141/375-376/395-403/600.
- Patch history: P1-C commit `f25c16c0` (2026-05-10, "NRFI geometric mean formula") and the still-unchecked TS migration todo (`todo.md:3127`); P3-C constants and rationale comments (`MLBAIModel.py:353-366`); `edb58f20` touches only one-off scripts; "Rename F5/NRFI tab to CHEAT SHEETS" at `todo.md:2580`.

Corrected (applied inline above):

1. **[MATERIAL] Lineups-watcher trigger mis-described.** The dossier claimed the watcher fires a targeted `runMlbModelForDate(..., {targetGameIds, forceRerun:true})`. Actual code (`server/mlbLineupsWatcher.ts:582-583`, unchanged by Phase 4) calls plain `runMlbModelForDate(dateStr)` with no options — the modelRunAt same-date skip guard therefore makes a mid-day lineup change a no-op for already-modeled games. The targeted-forceRerun pattern exists only in the RL-invalidation re-run (`mlbModelRunner.ts:2599-2623`) and the routers.ts Layer3 path (822-823).
2. **[MATERIAL] Fabricated line cites + wrong grading attribution for `mlbFullBacktestEngine.ts`.** Cited lines `924-963, 1078-1079` do not exist (file is 835 lines, single commit `30275454`); the engine contains no `nrfiResult` reference and does not grade — it aggregates `mlb_game_backtest` rows (213-240) and delegates re-grading to `runMultiMarketBacktestForDate` (787-835). Breakeven 0.524 / min-edge 0.05 cites (69-70, 89) were correct.
3. [cite-only] Ingestor NRFI/brier DB write is at `mlbOutcomeIngestor.ts:606-623` (pre-fix), not "585-601" (that range is the staging/log block).
4. [cite-only] `census/schema-columns.tsv` rows proving `fullName`/`teamAbbrev` are `375-376`, not `372-373`.

Phase-4 fix annotations added ([FIXED in Phase 4], claims were true pre-fix):

- Finding 1 / Open Q4: `brierNrfi` scale — `computeBrierScores` now passes `probFromUnit(game.modelPNrfi)` and `brierScore` takes a normalized [0,1] probability (M-203). Forward path only; historical rows still poisoned.
- Finding 9 / Open Q2 / grading-writes bullet: `nrfiBacktestResult`/`nrfiCorrect`/`nrfiBacktestRunAt` now written by the nightly ingestor via new `gradeNrfiPick` (pick = NRFI iff P(NRFI) > 0.5, no push). Forward path only.
- Not fixed by Phase 4 (checked against post-fix tree): finding 2 (TS arithmetic 0.56 still persisted), finding 3 (dead sampler variance unchanged), finding 4 (`decimal(5,2)` unchanged), finding 5 (still no nrfiRate writer), findings 6-8, 10-11 (unchanged). Phase 4's M-207 `mlb_calibration_constants` read-back covers only `fg_ml_home_edge`/`league_env_mult` env injection — it does not touch any NRFI parameter.

Unbacked claims: none — every VERIFIED tag either reproduced or was corrected above; INFERRED/UNKNOWN tags were checked for internal consistency and stand as labeled.
