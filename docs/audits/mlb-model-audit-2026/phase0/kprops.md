...
| `runKPropsBacktest` (kPropsBacktestService.ts:400-411, 337-343, 373-376) | actualKs, backtestResult (OVER/UNDER/PUSH/NO_LINE/NAME_MATCH_FAILED), modelError (= actualKs − kProj), modelCorrect, backtestRunAt | pending rows for date; every cycle (vsinAutoRefresh.ts:1934) and per-final-game via `runMultiMarketBacktest` (mlbMultiMarketBacktest.ts:1024-1029) |
