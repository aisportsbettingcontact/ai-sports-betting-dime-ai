...
|---|---|---|
| `startMlbModelSyncScheduler()` | Registered at server boot in `server/_core/index.ts:877` (behind a background-jobs guard per surrounding comments). Runs `runMlbModelSyncCycle()` immediately, then every **5 min** (`MLB_MODEL_SYNC_INTERVAL_MS = 300 000`), 24/7. Each cycle runs `runMlbModelForDate(todayET)` then `(tomorrowET)` with 3-attempt exponential-backoff DB retry. Watchdog every 2 min; if no completed cycle in 15 min and not running, self-heals by starting a new cycle. | mlbModelRunner.ts:2673-2824; _core/index.ts:874-877 VERIFIED |
...
| MLBCycle fallback | `server/vsinAutoRefresh.ts` runMlbCycle Step 6: `runMlbModelForDate(today)` + `(tomorrow)` after odds scrape/lineup watcher | vsinAutoRefresh.ts:1860-1884 VERIFIED (cadence of runMlbCycle itself not traced — UNKNOWN) |
| Line-move re-run | vsinAutoRefresh.ts:1136-1138 — targeted `forceRerun` for a single game (context appears to be an L3 line-change handler; surrounding logic not read — UNKNOWN detail) |
...
- **`server/patchRlSigmoid.py`**: a string-replacement script that edited **`server/mlbMultiMarketBacktest.ts`** (not the live model) to add the k=0.4 sigmoid *fallback* for RL cover probability when `modelHomePLCoverPct` is NULL. **Its change IS in the live backtest file today**: `RL_SIGMOID_K = 0.4` and the stored-vs-sigmoid-fallback logic exist at mlbMultiMarketBacktest.ts:307-325, with `FG_RL_AWAY_EDGE_THRESHOLD = 0.18` (raised from 0.05 "to filter out systematic home-edge correction bias") at mlbMultiMarketBacktest.ts:67. **VERIFIED**. Neither script ever modified MLBAIModel.py or mlbModelRunner.ts.
...
1. **Who calls `runMlbCycle` in vsinAutoRefresh.ts and on what cadence?** The Step-6 model fallback is verified, but the cycle's own scheduler/interval was not traced. (Needed to know total trigger pressure on the model.)
2. **Does `mlbFullBacktestEngine.ts` / the multi-market backtest run on a schedule** (vsinAutoRefresh.ts:1954 calls `runMultiMarketBacktest` — context unread), and does anything automated ever consume `mlbPublicationGate` output outside tests? Current evidence says no consumer besides the test.
...
8. **vsinAutoRefresh L3 single-game rerun** (vsinAutoRefresh.ts:1136) — trigger condition unread.
9. **`server/dime` / DimeModelFeed backend route** — whether the Dime `/feed` migration reads the same `games.list` procedure or a separate endpoint (client gate verified; server route for the Dime shell not traced).
