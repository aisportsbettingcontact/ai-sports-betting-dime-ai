...
**Publication gate (follow-on agent).** `publish_*` rows in the calibration table now gate each market server-side (fail-open when absent), exposed via tRPC, enforced client-side by nulling gated markets' model fields through the exact rendering path already used for invalidated models — so gating produces the established "—" state rather than layout breakage. Owner backtest surfaces are deliberately ungated: they are the BACKTEST-ONLY audience.
