...
- ~~Which Railway builder actually runs~~ — **RESOLVED 2026-08-05: the Dockerfile.** One build-log read settled it, exactly as this entry predicted. The `RAILPACK` config value is stale dashboard state overridden by `railway.json` at deploy time. Post-merge deploy verification for the cronRunner change is therefore trustworthy. See `os/audits/2026-08-05-builder-resolution.md`.
