...
function waitForServerReady(proc: ChildProcess, timeoutMs: number): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    proc.on("exit", (code) => { clearTimeout(timer); reject(new Error(`prod server exited early (code ${code}). Output:\n${buffer}`)); });
...
const STUB_USER = {
  id: 1, email: "prez@aisportsbettingmodels.com", username: "prez", role: "user",
  hasAccess: true, expiryDate: null, termsAccepted: true,
  discordId: null, discordUsername: null, discordAvatar: null, discordConnectedAt: null,
  sessionExpiresAt: null, stripePlanId: null, stripeCustomerId: null,
  stripeSubscriptionId: null, cancelAtPeriodEnd: false,
};

function mlbRow(overrides: Record<string, unknown>) {
  return {
    id: 1, gameDate: GAME_DATE, startTimeEst: "7:05 PM", sport: "MLB",
    gameStatus: "upcoming", awayScore: null, homeScore: null, gameClock: null,
    venue: "Fenway Park", doubleHeader: "N", gameNumber: 1,
...
const LONG_PICK_GAME = mlbRow({
  id: 501, awayTeam: "CHW", homeTeam: "TB", startTimeEst: "6:05 PM", venue: "Tropicana Field",
  awayML: 123, homeML: -145, modelAwayML: -125, modelHomeML: 105,
  modelAwayWinPct: 55.5, modelHomeWinPct: 44.5,
});
...
const SHORT_PICK_GAME = mlbRow({
  id: 502, awayTeam: "SF", homeTeam: "SEA", startTimeEst: "9:10 PM", venue: "T-Mobile Park",
...
const PASS_GAME = mlbRow({
  id: 503, awayTeam: "ATH", homeTeam: "TEX", startTimeEst: "6:35 PM", venue: "Globe Life Field",
  awayML: 130, homeML: -150, modelAwayML: 128, modelHomeML: -148,
...
const FINAL_GAME = mlbRow({
  id: 504, awayTeam: "LAD", homeTeam: "NYY", startTimeEst: "1:05 PM", venue: "Yankee Stadium",
  gameStatus: "final", awayScore: 3, homeScore: 5,
  awayML: -110, homeML: -105, modelAwayML: -145, modelHomeML: 125,
  modelAwayWinPct: 59.2, modelHomeWinPct: 40.8,
});

function lineupRow(overrides: Record<string, unknown>) {
  return {
    id: 1, gameId: 1, scrapedAt: 1_721_740_000_000,
    awayPitcherName: null, awayPitcherHand: null, awayPitcherEra: null,
    awayPitcherRotowireId: null, awayPitcherMlbamId: null, awayPitcherConfirmed: false,
    homePitcherName: null, homePitcherHand: null, homePitcherEra: null,
    homePitcherRotowireId: null, homePitcherMlbamId: null, homePitcherConfirmed: false,
    awayLineup: null, homeLineup: null,
...
const LINEUPS_BY_GAME_ID = {
  501: lineupRow({
    gameId: 501,
    awayPitcherName: "Matthew Liberatore", awayPitcherHand: "L",
    awayPitcherEra: "5-8 · 4.02 ERA", awayPitcherRotowireId: 14201, awayPitcherConfirmed: true,
    homePitcherName: "Simeon Woods Richardson", homePitcherHand: "R",
    homePitcherEra: "7-7 · 4.18 ERA", homePitcherRotowireId: 10755, homePitcherConfirmed: false,
  }),
  502: lineupRow({
    gameId: 502,
    awayPitcherName: "Logan Webb", awayPitcherHand: "R",
    awayPitcherEra: "7-4 · 3.21 ERA", awayPitcherRotowireId: 14222, awayPitcherConfirmed: true,
    homePitcherName: "George Kirby", homePitcherHand: "R",
    homePitcherEra: "8-5 · 3.62 ERA", homePitcherRotowireId: 15669, homePitcherConfirmed: false,
  }),
};
...
async function stubApi(page: Page) {
  await page.route("**/api/trpc/**", (route) => {
    ...
    const body = ops.map((op) => {
      ...
      if (op === "games.list")
        return { result: { data: { json: [LONG_PICK_GAME, SHORT_PICK_GAME, PASS_GAME, FINAL_GAME] } } };
      if (op === "games.mlbLineups") return { result: { data: { json: LINEUPS_BY_GAME_ID } } };
      if (op === "wc2026.matchesByDate") return { result: { data: { json: [] } } };
      return { error: { json: { message: "stubbed offline (e2e)", code: -32603, data: { code: "INTERNAL_SERVER_ERROR", httpStatus: 500 } } } };
    });
  ...
  await page.route("https://www.mlbstatic.com/**", (route) =>
    route.fulfill({ status: 200, contentType: "image/svg+xml", body: svg(150, 150) }));
  await page.route("https://img.mlbstatic.com/**", (route) =>
    route.fulfill({ status: 200, contentType: "image/svg+xml", body: svg(180, 270) }));
  await page.route("https://www.rotowire.com/images/photos/**", (route) =>
    route.fulfill({ status: 200, contentType: "image/svg+xml", body: svg(250, 250) }));
}

async function gotoFeed(page: Page, width: number, height = 1400, colorScheme: "dark" | "light" = "dark") {
  await stubApi(page);
  await page.emulateMedia({ colorScheme });
  await page.setViewportSize({ width, height });
  await page.addInitScript((mode) => { localStorage.setItem("dime-theme", mode); }, colorScheme);
...
const WIDTHS = [320, 360, 375, 390, 430, 640, 700, 767, 768, 900, 1024, 1229, 1231, 1280, 1366, 1440, 1512];

test.describe("content-driven columns + zero overflow", () => {
  for (const width of WIDTHS) {
    test(`columns match the 622/940 law and the page never scrolls sideways at ${width}px`, async ({ page }) => {
      await gotoFeed(page, width);
      const g = await measureGrid(page);
      expect(g.pageOverflow, `page overflow at ${width}`).toBe(0);
      expect(g.columns, `columns for body ${g.bodyWidth}px`).toBe(expectedColumns(g.bodyWidth));
...
test.describe("uniform summary anatomy (width-deterministic, never mixed)", () => {
  for (const width of [390, 768, 1280, 1512]) {
    test(`every priced card shares one anatomy at ${width}px`, async ({ page }) => {
      await gotoFeed(page, width);
      const anatomies = await page.evaluate(() => {
        return [...document.querySelectorAll<HTMLElement>(".projection-card")]
          ...
          .map((card) => {
            ...
            const readout = card.querySelector<HTMLElement>(".summary__readout")!;
            const signal = card.querySelector<HTMLElement>(".summary__signal")!;
            const twoRow = signal.getBoundingClientRect().top >= readout.getBoundingClientRect().bottom - 2;
      ...
      for (const a of anatomies) {
        ...
        expect(a.display, `group display at card ${a.cardW}px`).toBe(expectTwoRow ? "grid" : "flex");
        expect(a.twoRow, `signal row position at card ${a.cardW}px`).toBe(expectTwoRow);
      }
...
test.describe("one-line text contracts", () => {
  for (const width of [320, 390, 768, 1280, 1512]) {
    test(`trigger label and pitcher names hold one line at ${width}px`, async ({ page }) => {
      await gotoFeed(page, width);
      for (const ok of await isOneLine(page, ".projection-card__markets-toggle > span")) {
  ...
  test("full league name is unclipped at the 320px floor", async ({ page }) => {
    ...
    expect(clipped.sw, "league name ellipsized at 320px").toBeLessThanOrEqual(clipped.cw + 1);
  });
});

test.describe("sticky chrome — one offset variable", () => {
  for (const width of [390, 767, 1024, 1512]) {
    test(`feedhead top equals the topbar band at ${width}px`, async ({ page }) => {
      await gotoFeed(page, width);
      const { feedheadTop, bandH, topbarVisible } = await page.evaluate(() => {
        ...
        return {
          ...
          bandH: parseFloat(getComputedStyle(root).getPropertyValue("--dmf-topbar-h")) ||
      ...
      if (topbarVisible) {
        const tbH = await page.evaluate(() => document.querySelector<HTMLElement>(".dmf-topbar")!.getBoundingClientRect().height);
        expect(Math.abs(feedheadTop - tbH), `feedhead top ${feedheadTop} vs topbar ${tbH}`).toBeLessThanOrEqual(0.5);
      } else {
...
test.describe("interactive target floors", () => {
  test("date arrows (effective), next control, and LINEUPS keep >=44px", async ({ page }) => {
    await gotoFeed(page, 1512);
    const targets = await page.evaluate(() => {
      const sq = document.querySelector<HTMLElement>(".dmf-sq")!;
      const sqAfter = getComputedStyle(sq, "::after");
      const lineups = document.querySelector<HTMLElement>(".pregame-pitchers__lineups");
      ...
      if (lineups) {
        ...
        const ext = before.content !== "none"
          ? Math.abs(parseFloat(before.top) || 0) + Math.abs(parseFloat(before.bottom) || 0)
          : 0;
      ...
      return {
        ...
        lineupsEffective,
        nextBox: next ? { w: next.getBoundingClientRect().width, h: next.getBoundingClientRect().height } : null,
      };
    });
    expect(targets.sqBox).toBeGreaterThanOrEqual(28);
    expect(targets.sqInset).toContain("-9px"); // 28 + 2×9 = 46px effective
    if (targets.lineupsEffective != null) expect(targets.lineupsEffective).toBeGreaterThanOrEqual(43.5);
    if (targets.nextBox) {
...
test.describe("theme parity", () => {
  for (const width of [390, 1440]) {
    test(`dark and light share geometry and correct chip tokens at ${width}px`, async ({ page }) => {
      ...
      expect(Math.max(...d), "dark chip must be a dark mint tint").toBeLessThan(70);
      expect(d[1], "dark chip green channel leads").toBeGreaterThan(d[0]);
      expect(Math.min(...l), "light chip must be a light mint tint").toBeGreaterThan(190);
    });
  }
});
...
test.describe("text zoom (200%)", () => {
  for (const width of [1280, 390]) {
    test(`no page overflow and no wrapped trigger at 200% text scale, ${width}px`, async ({ page }) => {
      await gotoFeed(page, width);
