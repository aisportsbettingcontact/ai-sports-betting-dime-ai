...
// ─── Analytics Events (versioned product-analytics ingestion seam) ───────────
/**
 * One row per product-analytics event accepted by the `analytics.track` tRPC
 * procedure. This is the storage side of a MINIMAL, additive, versioned
 * event-ingestion seam. Emitters are a separate future phase — shipping this
 * table only stands up the sink.
 *
 * PRIVACY / SAFETY CONTRACT (enforced at the router + store layer, documented
 * here so the schema is self-describing):
 *   - `subjectId` is the SERVER-DERIVED app_users.id (from ctx.appUser.id). It is
 *     a pseudonymous reference, NEVER a client-supplied identity. The client can
 *     never set or forge it.
 *   - NO PII is stored here: no names, emails, IPs, chat text, wager amounts,
 *     losses, balances, or payment/entitlement/consent state. `propsJson` holds
 *     ONLY server-allowlisted, non-sensitive scalar props.
 *   - Idempotency: `eventId` is UNIQUE. Re-delivery of the same client-generated
 *     eventId is a no-op (see server/analytics/eventStore.ts).
 *   - `schemaVersion` versions the shape of a given `eventName` so the payload
 *     contract can evolve without breaking historical rows.
 *
 * Retention: rows are analytics-grade and may be pruned by a scheduled job.
 * Access: owner/analytics only — no user-facing queries.
 */
export const analyticsEvents = mysqlTable(
  "analytics_events",
  {
    id: int("id").autoincrement().primaryKey(),
    /** Client-generated idempotency key (e.g. uuid/nanoid). UNIQUE — dedupes re-delivery. */
    eventId: varchar("eventId", { length: 64 }).notNull().unique(),
    /** Past-tense lower_snake_case event name; must be in the server ALLOWLIST. */
    eventName: varchar("eventName", { length: 64 }).notNull(),
    /** Schema version for this eventName's payload contract. */
    schemaVersion: int("schemaVersion").notNull(),
    /** Server-derived app_users.id — pseudonymous subject ref, NOT PII, never client-supplied. */
    subjectId: int("subjectId").notNull(),
    /** Optional opaque client session id (pseudonymous). Nullable. */
    sessionId: varchar("sessionId", { length: 64 }),
    /** Emission surface, e.g. 'web' | 'ios' | 'android' | 'server' (validated allowlist). */
    source: varchar("source", { length: 32 }).notNull(),
    /** Server-derived deployment environment ('production' | 'development' | 'test' | ...). */
    environment: varchar("environment", { length: 32 }).notNull(),
    /** UTC ms when the event occurred (client clock, bounded/validated). */
    occurredAtUtc: bigint("occurredAtUtc", { mode: "number" }).notNull(),
    /** UTC ms when the server accepted the event (server clock — authoritative). */
    receivedAtUtc: bigint("receivedAtUtc", { mode: "number" }).notNull(),
    /** Optional coarse outcome label ('success' | 'failure' | 'cancelled' | ...). Nullable. */
    outcome: varchar("outcome", { length: 32 }),
    /** Optional coarse data-state label (non-sensitive, e.g. 'empty' | 'partial' | 'full'). Nullable. */
    dataState: varchar("dataState", { length: 32 }),
    /** JSON of ALLOWLISTED, non-sensitive scalar props only. Nullable. Never raw text/PII. */
    propsJson: text("propsJson"),
    /** Row-insert wall clock (DB default). */
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  },
  (t) => ({
    idxSubjectId: index("idx_analytics_events_subject_id").on(t.subjectId),
    idxEventName: index("idx_analytics_events_event_name").on(t.eventName),
    idxOccurredAt: index("idx_analytics_events_occurred_at").on(t.occurredAtUtc),
  }),
);
