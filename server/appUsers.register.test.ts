...
async function insertOwnerUser(
  ...
  if (!rows.length)
    throw new Error(
      `[insertOwnerUser] Failed to retrieve inserted owner for suffix=${suffix}`
    );
  const owner = rows[0];
  console.log(
    `[insertOwnerUser] [OUTPUT] id=${owner.id} tokenVersion=${owner.tokenVersion}`
  );
...
afterAll(async () => {
  ...
  console.log(
    "[afterAll] [STEP] Deleted all test users with username LIKE 'testuser_reg_%'"
  );
});
...
describe.skipIf(SKIP_DB_IN_CI)(
  "appUsers.register — createUser invariants (real DB)",
  () => {
    // ── HAPPY PATH — account creation ────────────────────────────────────────────
    ...
    it("[CR-1..13] happy path: creates user, hashes password correctly, no cookie issued", async () => {
      ...
      const { ctx, setCookies } = await createOwnerContext(
        owner.id,
        owner.tokenVersion
      );
      ...
      console.log(
        `[INPUT] email=${email} username=${username} password=${plainPassword}`
      );
      ...
      const user = rows[0];
      console.log(
        `[STATE] DB row: id=${user.id} email=${user.email} username=${user.username} pendingSetup=${user.pendingSetup}`
      );
      ...
      console.log(
        `[VERIFY] [CR-4] PASS — passwordHash starts with bcrypt prefix: ${user.passwordHash.slice(0, 7)}`
      );
      ...
      console.log(
        `[VERIFY] [CR-5] PASS — bcrypt cost rounds=${costRounds} >= 10 (OWASP minimum)`
      );
      ...
      console.log(
        `[VERIFY] [CR-6] PASS — bcrypt.compare(plaintext, hash)=true`
      );

      // [CR-7] pendingSetup === false (admin-created accounts are immediately active)
      expect(user.pendingSetup).toBe(false);
      console.log(
        `[VERIFY] [CR-7] PASS — pendingSetup=false (admin-created account is immediately active)`
      );

      // [CR-8] NO cookie is issued (res.cookie is never called)
      const appSessionCookies = setCookies.filter(
        c => c.name === APP_USER_COOKIE
      );
      expect(appSessionCookies.length).toBe(0);
      console.log(
        `[VERIFY] [CR-8] PASS — res.cookie NOT called (setCookies.length=${setCookies.length}) — no auto-login`
      );

      // [CR-9] email is stored lowercase
      expect(user.email).toBe(email.toLowerCase());
      console.log(
        `[VERIFY] [CR-9] PASS — email stored lowercase: ${user.email}`
      );

      // [CR-10] username is stored lowercase
      expect(user.username).toBe(username.toLowerCase());
      console.log(
        `[VERIFY] [CR-10] PASS — username stored lowercase: ${user.username}`
      );
    ...
    it("[RV-1] role=admin is stored correctly", async () => {
      ...
      await caller.appUsers.createUser({
        email,
        username,
    ...
    it("[ER-1] throws CONFLICT when email is already in use", async () => {
      ...
      console.log(
        `[INPUT] email=${email} — first insert with username1, second with username2`
      );
      ...
      console.log(
        `[VERIFY] [ER-1] PASS — CONFLICT thrown for duplicate email=${email}`
      );
    });
    ...
    it("[ER-2] throws CONFLICT when username is already taken", async () => {
      ...
      console.log(
        `[INPUT] username=${username} — first insert with email1, second with email2`
      );
      ...
      console.log(
        `[VERIFY] [ER-2] PASS — CONFLICT thrown for duplicate username=${username}`
      );
    });
    ...
    it("[ER-3] Zod rejects password shorter than 8 chars (BAD_REQUEST)", async () => {
      ...
      console.log(
        `[VERIFY] [ER-3] PASS — BAD_REQUEST thrown for password shorter than 8 chars`
      );
    });
    ...
    it("[ER-4] Zod rejects invalid email format (BAD_REQUEST)", async () => {
      ...
      console.log(
        `[VERIFY] [ER-4] PASS — BAD_REQUEST thrown for invalid email format`
      );
    });
    ...
    it("[ER-5] throws UNAUTHORIZED when called without owner JWT", async () => {
      ...
      console.log(
        `[VERIFY] [ER-5] PASS — UNAUTHORIZED thrown for non-owner caller; no cookie issued`
      );
    });
  }
);
