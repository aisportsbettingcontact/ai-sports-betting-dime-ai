...
export async function seedBullpenStats(): Promise<{
  inserted: number;
  updated: number;
  errors: number;
}> {
  ...
  console.log(
    `[INPUT] Season: ${SEASON} | Teams: ${MLB_TEAMS.length} | Filter: gamesStarted=0 AND ip>=1.0`
  );
  ...
  for (const team of MLB_TEAMS) {
    ...
    try {
      ...
      console.log(
        `[STATE] ${team.abbrev}: total pitchers in API response = ${allPitchers.length}`
      );
      ...
      console.log(
        `[STATE] ${team.abbrev}: relievers (GS=0, IP>=1) = ${relievers.length}`
      );

      if (relievers.length === 0) {
        console.warn(
          `[STATE] ${team.abbrev}: WARN — no qualifying relievers found, skipping`
        );
        continue;
      }
      ...
      console.log(
        `[STATE] ${team.abbrev}: ip=${totalIp.toFixed(1)} er=${totalEr} k=${totalK} bb=${totalBb} hr=${totalHr} h=${totalH}`
      );
      console.log(
        `[STATE] ${team.abbrev}: ERA=${eraBullpen?.toFixed(2)} K/9=${k9Bullpen?.toFixed(2)} BB/9=${bb9Bullpen?.toFixed(2)} HR/9=${hr9Bullpen?.toFixed(2)} WHIP=${whipBullpen?.toFixed(3)} K/BB=${kBbRatio?.toFixed(2)} FIP=${fipBullpen?.toFixed(2)}`
      );

      // Validate ERA range
      if (eraBullpen !== null && (eraBullpen < 0 || eraBullpen > 15)) {
        console.warn(
          `[VERIFY] WARN — ${team.abbrev} bullpen ERA=${eraBullpen.toFixed(2)} outside expected range [0, 15]`
        );
      }
  ...
  console.log(
    `[OUTPUT] inserted=${inserted} updated=${updated} errors=${errors} teams_processed=${leagueTotals.teams}`
  );

  // League-wide bullpen ERA
  const leagueEra =
    leagueTotals.ip > 0 ? (leagueTotals.er / leagueTotals.ip) * 9 : 0;
  const leagueK9 =
    leagueTotals.ip > 0 ? (leagueTotals.k / leagueTotals.ip) * 9 : 0;
  const leagueBb9 =
    leagueTotals.ip > 0 ? (leagueTotals.bb / leagueTotals.ip) * 9 : 0;
  console.log(
    `[VERIFY] League bullpen: ERA=${leagueEra.toFixed(3)} K/9=${leagueK9.toFixed(3)} BB/9=${leagueBb9.toFixed(3)}`
  );
  ...
  const allRows = await db
    ...
    .from(mlbBullpenStats)
    .where(eq(mlbBullpenStats.season, SEASON));

  const sorted = [...allRows].sort(
    (a, b) => (a.eraBullpen ?? 99) - (b.eraBullpen ?? 99)
  );
  console.log("\n[VERIFY] Top 5 best bullpens (ERA):");
  sorted.slice(0, 5).forEach((r: any) => {
    console.log(
      `  ${r.teamAbbrev}: ERA=${r.eraBullpen?.toFixed(2)} K/9=${r.k9Bullpen?.toFixed(2)} FIP=${r.fipBullpen?.toFixed(2)} (${r.relieverCount} relievers)`
    );
  });
  ...
  const outliers = allRows.filter(
    (r: any) =>
      r.eraBullpen !== null && (r.eraBullpen < 2.0 || r.eraBullpen > 8.0)
  );
  if (outliers.length > 0) {
    console.warn(
      `[VERIFY] WARN — ${outliers.length} teams with ERA outside [2.0, 8.0]: ${outliers.map((r: any) => r.teamAbbrev).join(", ")}`
    );
  }

  if (errors === 0) {
    console.log(
      `[VERIFY] PASS — ${inserted + updated} bullpen rows seeded with 0 errors`
    );
  } else {
...
if (
  process.argv[1]?.endsWith("seedBullpenStats.ts") ||
  process.argv[1]?.endsWith("seedBullpenStats.js")
) {
