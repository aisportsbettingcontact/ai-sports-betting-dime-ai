  ...
  app.get(`${ROUTE_PREFIX}/connect`, async (req: Request, res: Response) => {
    ...
    try {
      const deleted = await db
        ...
        .where(lt(discordOAuthStates.expiresAt, now));
      console.log(
      ...
      console.warn(
        `[DiscordAuth][CHECKPOINT:3.CLEANUP_WARN] /connect — requestId=${requestId}` +
          ` Failed to clean expired states (non-fatal):`,
        cleanErr
      );
    }
  ...
  app.get(`${ROUTE_PREFIX}/callback`, async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!tokenRes.ok) {
        ...
        res.redirect(
          302,
          "/feed/model/mlb?discord_error=token_exchange_failed"
        );
        return;
      }
      ...
      void syncDiscordRole(userId, updatedUser?.hasAccess === true).catch(
        (syncErr: unknown) =>
          console.warn(
            `[DiscordAuth][ROLE_SYNC_WARN] requestId=${requestId} userId=${userId}`,
            syncErr
          )
      );
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      ...
      if (!origin || !isOriginAllowed(origin)) {
        console.warn(
          `[DiscordAuth][CHECKPOINT:A.FAIL] /disconnect — requestId=${requestId} rejected origin="${origin ?? "missing"}"`
        );
