...
function decodeJwt(token: string): {
  ...
  if (parts.length !== 3) {
    throw new Error(
      `[decodeJwt] Expected 3 parts, got ${parts.length}: "${token.substring(0, 40)}..."`
    );
  }
...
async function insertTestPendingUser(suffix: string): Promise<{
  ...
  console.log(
    `[insertTestPendingUser] [INPUT] suffix=${suffix} sessionId=${sessionId} username=${username}`
  );
  ...
  if (!rows.length)
    throw new Error(
      `[insertTestPendingUser] Row not found after insert suffix=${suffix}`
    );
  const row = rows[0];
  console.log(
    `[insertTestPendingUser] [OUTPUT] userId=${row.id} username=${row.username} tokenVersion=${row.tokenVersion}`
  );
  return row as {
    id: number;
    username: string;
...
afterAll(async () => {
  console.log(
    "[CLEANUP] Deleting all test rows (pendingStripeSessionId LIKE 'test-session-%' OR username LIKE 'testuser_cas_%')"
  );
  const db = await getDb();
  if (!db) {
...
describe.skipIf(SKIP_DB_IN_CI)(
  "stripe.completeAccountSetup — auto-login cookie invariant",
  () => {
    // ── HAPPY PATH — cookie issuance ──────────────────────────────────────────────
    ...
    it("[HP-1..9] issues app_session cookie with correct flags and returns success", async () => {
      ...
      console.log(
        `[INPUT] sessionId=${user.pendingStripeSessionId} email=${testEmail} userId=${user.id}`
      );
      ...
      if (setCookies.length > 0) {
        ...
        console.log(
          `[STATE] cookie[0].options=${JSON.stringify(setCookies[0].options)}`
        );
        console.log(
          `[STATE] cookie[0].value (first 40 chars)=${setCookies[0].value.substring(0, 40)}...`
        );
      }
      ...
      console.log(
        "[VERIFY] [HP-8] PASS — result.success=true alreadySetup=false"
      );
      ...
      console.log(
        `[VERIFY] [HP-5] PASS — maxAge=${opts.maxAge} (${NINETY_DAYS_MS}ms = 90 days)`
      );
      ...
      console.log(
        "[VERIFY] [HP-9] PASS — session consumed: second call throws NOT_FOUND (one-time token cleared)"
      );
    });
    ...
    it("[JW-1..6] JWT claims are correct: sub, role, type, tv, alg, exp", async () => {
      ...
      console.log(
        `[INPUT] userId=${user.id} role=${user.role} tokenVersion=${user.tokenVersion}`
      );
      ...
      console.log(
        `[STATE] JWT payload=${JSON.stringify({ ...payload, exp: payload.exp })}`
      );
      ...
      console.log(
        `[VERIFY] [JW-1] PASS — sub="${payload.sub}" === userId="${user.id}"`
      );
      ...
      console.log(
        `[VERIFY] [JW-4] PASS — tv=${payload.tv} === tokenVersion=${user.tokenVersion}`
      );
      ...
      console.log(
        `[STATE] exp=${new Date(expMs).toISOString()} expectedExp=${new Date(expectedExpMs).toISOString()} drift=${drift}ms`
      );
    ...
    it("[EC-1] already-setup user (pendingSetup=false, sessionId still set) returns alreadySetup=true without cookie", async () => {
      ...
      console.log(
        `[INPUT] sessionId=${sessionId} pendingSetup=false (already-setup race condition)`
      );
      ...
      console.log(
        "[VERIFY] [EC-1a] PASS — alreadySetup=true for pendingSetup=false user"
      );
      ...
      console.log(
        "[VERIFY] [EC-1b] PASS — no cookie set for already-setup user"
      );
    });
    ...
    it("[ER-1] throws NOT_FOUND for unknown sessionId", async () => {
      ...
      console.log(
        "[VERIFY] [ER-1] PASS — NOT_FOUND thrown for unknown sessionId"
      );
    });
    ...
    it("[ER-2] throws CONFLICT when email is already used by a different user", async () => {
      ...
      console.log(
        `[INPUT] user2.sessionId=${user2.pendingStripeSessionId} email=${sharedEmail} (already used by user1)`
      );
    ...
    it("[ER-6] Zod rejects password without special character", async () => {
      ...
      console.log(
        "[VERIFY] [ER-6] PASS — BAD_REQUEST for missing special character"
      );
    });
  }
);
