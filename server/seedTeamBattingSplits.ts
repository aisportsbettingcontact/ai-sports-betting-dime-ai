...
async function fetchTeamBattingSplits(
  teamId: number,
  teamAbbrev: string,
  teamName: string
): Promise<
...
> {
  ...
  if (!res.ok)
    throw new Error(`HTTP ${res.status} for team ${teamAbbrev} (${teamId})`);
...
export async function seedTeamBattingSplits(): Promise<{
  ...
  console.log(
    "[INPUT] Starting team batting splits seed for all 30 MLB teams..."
  );
  console.log(`[STATE] Season: 2026 | sitCodes: vl (vs LHP), vr (vs RHP)`);
  console.log(
    `[STATE] wOBA weights: BB=${WOBA_WEIGHTS.bb} 1B=${WOBA_WEIGHTS.s1b} 2B=${WOBA_WEIGHTS.d2b} 3B=${WOBA_WEIGHTS.t3b} HR=${WOBA_WEIGHTS.hr}`
  );
  ...
  await runConcurrent(
    ...
    async (team, i) => {
      ...
      try {
        const splits = await fetchTeamBattingSplits(
          team.id,
          team.abbrev,
          team.name
        );
  ...
  console.log(
    `[STEP] Upserting ${rowsToUpsert.length} rows into mlb_team_batting_splits...`
  );
  let upserted = 0;
  let upsertErrors = 0;
  ...
  for (const row of rowsToUpsert) {
    try {
      ...
      const existing = await db
        .select({ id: mlbTeamBattingSplits.id })
      ...
      if (existing.length > 0) {
        ...
        await db
          .insert(mlbTeamBattingSplits)
          .values({ ...row, lastFetchedAt: now });
      }
      upserted++;
    } catch (e: any) {
      upsertErrors++;
      console.error(
        `  [ERROR] DB upsert failed for ${row.teamAbbrev} vs ${row.hand}HP: ${e.message}`
      );
    }
  }
  ...
  for (const r of sample) {
    console.log(
      `  ${r.teamAbbrev} vs ${r.hand}HP: avg=${r.avg} obp=${r.obp} slg=${r.slg} ops=${r.ops} woba=${r.woba?.toFixed(3)} hr9=${r.hr9?.toFixed(3)}`
    );
  }
...
if (process.argv[1]?.endsWith("seedTeamBattingSplits.ts")) {
