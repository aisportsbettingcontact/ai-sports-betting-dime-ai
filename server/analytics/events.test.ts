...
describe("trackInputSchema", () => {
  ...
  it("accepts an engagement event name (login is now value-agnostic)", () => {
    expect(
      trackInputSchema.safeParse({ ...base, eventName: "login" }).success
    ).toBe(true);
  });
  it("rejects a too-short event id", () => {
    expect(trackInputSchema.safeParse({ ...base, eventId: "x" }).success).toBe(
      false
    );
  });
...
describe("sanitizeProps", () => {
  it("keeps scalars, caps string length, drops non-scalars", () => {
    const out = sanitizeProps({
      ...
      b: 3,
      c: true,
      d: { nested: 1 } as unknown as string,
    });
...
describe("action_performed contract (D3)", () => {
  ...
  it("accepts feature-lifecycle event names", () => {
    for (const n of FEATURE_EVENTS) {
      expect(
        trackInputSchema.safeParse({ ...actionBase, eventName: n }).success
      ).toBe(true);
      expect(ALL_EVENTS).toContain(n);
    }
  });
  ...
  it("accepts the P0 profiling action names on action_performed", () => {
    const base = {
      ...
      schemaVersion: 1,
      occurredAtUtc: 1700000000000,
      eventName: "action_performed" as const,
    };
    expect(
      trackInputSchema.safeParse({ ...base, actionName: "results_viewed" })
        .success
    ).toBe(true);
    expect(
      trackInputSchema.safeParse({ ...base, actionName: "referral_landed" })
        .success
    ).toBe(true);
  });
});
