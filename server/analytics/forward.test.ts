...
describe("forwardEvent", () => {
  ...
  it("posts to the PRIVATE ingest path with the secret header when configured", async () => {
    process.env.USER_ACTIVITY_BACKEND_URL =
      "http://ai-sports-betting-backend.railway.internal:3000";
    ...
    expect(calls[0].url).toBe(
      "http://ai-sports-betting-backend.railway.internal:3000/api/internal/analytics/ingest"
    );
    expect(
      (calls[0].init.headers as Record<string, string>)["x-analytics-secret"]
    ).toBe("sekret");
    expect(JSON.parse(String(calls[0].init.body)).eventName).toBe(
  ...
  it("never throws on network failure", async () => {
    ...
    await expect(forwardEvent(evt, boom)).resolves.toEqual({
      ok: false,
      reason: "network",
    });
  });
