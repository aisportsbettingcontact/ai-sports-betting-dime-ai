  ...
  const result = await db.execute(sql`
    INSERT IGNORE INTO analytics_events
      (event_id, event_name, schema_version, definition_version, source_user_id,
       session_id, tab_id, feature_id, surface, outcome, occurred_at_utc,
       received_at_utc, environment, app_version, is_test, props_json)
    VALUES
      (${e.eventId}, ${e.eventName}, ${e.schemaVersion}, ${e.definitionVersion ?? 1}, ${e.sourceUserId},
       ${e.sessionId ?? null}, ${e.tabId ?? null}, ${e.featureId ?? null}, ${e.surface}, ${e.outcome ?? null}, ${e.occurredAtUtc},
       ${receivedAt}, ${e.environment}, ${e.appVersion ?? null}, ${e.isTest ? 1 : 0}, ${propsJson})
  `);
