...
async function fetchSegmentRows(
  ...
  if (startDate)
    conditions.push(sql`${mlbGameBacktest.gameDate} >= ${startDate}` as any);
  if (endDate)
    conditions.push(sql`${mlbGameBacktest.gameDate} <= ${endDate}` as any);
...
function buildTeamSegments(
  ...
  if (homeRows.length > 0)
    segments.push(computeSegmentStats(homeRows, market, "team_side", "home"));
  if (awayRows.length > 0)
    segments.push(computeSegmentStats(awayRows, market, "team_side", "away"));

  // By team (home team)
  const homeTeams = Array.from(
    new Set(rows.map(r => r.homeTeam).filter(Boolean))
  ) as string[];
  ...
  // By team (away team)
  const awayTeams = Array.from(
    new Set(rows.map(r => r.awayTeam).filter(Boolean))
  ) as string[];
  ...
  if (dh1.length > 0)
    segments.push(
      computeSegmentStats(dh1, market, "doubleheader_game", "game_1")
    );
  if (dh2.length > 0)
    segments.push(
      computeSegmentStats(dh2, market, "doubleheader_game", "game_2")
    );
  ...
  if (nonDh.length > 0)
    segments.push(
      computeSegmentStats(nonDh, market, "doubleheader_game", "single_game")
    );
...
function buildPitcherSegments(
  ...
  // By home pitcher
  const homePitchers = Array.from(
    new Set(rows.map(r => r.homePitcher).filter(Boolean))
  ) as string[];
  for (const pitcher of homePitchers) {
    ...
    if (pitcherRows.length >= MIN_SEGMENT_SAMPLE) {
      segments.push(
        computeSegmentStats(pitcherRows, market, "home_pitcher", pitcher)
      );
    }
  }

  // By away pitcher
  const awayPitchers = Array.from(
    new Set(rows.map(r => r.awayPitcher).filter(Boolean))
  ) as string[];
  for (const pitcher of awayPitchers) {
    const pitcherRows = rows.filter(r => r.awayPitcher === pitcher);
    if (pitcherRows.length >= MIN_SEGMENT_SAMPLE) {
      segments.push(
        computeSegmentStats(pitcherRows, market, "away_pitcher", pitcher)
      );
    }
  }
...
function buildScheduleSegments(
  ...
  const nightRows = rows.filter(
    r => r.dayNight === "N" || r.dayNight === "night"
  );
  if (dayRows.length > 0)
    segments.push(computeSegmentStats(dayRows, market, "day_night", "day"));
  if (nightRows.length > 0)
    segments.push(computeSegmentStats(nightRows, market, "day_night", "night"));

  // By month
  const months = Array.from(
    new Set(rows.map(r => r.gameDate?.slice(0, 7)).filter(Boolean))
  ) as string[];
  ...
  for (let dow = 0; dow < 7; dow++) {
    ...
    if (dowRows.length >= MIN_SEGMENT_SAMPLE) {
      segments.push(
        computeSegmentStats(dowRows, market, "day_of_week", dows[dow])
      );
    }
  }
...
function buildMarketSegments(
  ...
  // Edge buckets
  const edgeBuckets = [
    { label: "edge_0_2pct", min: 0.0, max: 0.02 },
    { label: "edge_2_4pct", min: 0.02, max: 0.04 },
    { label: "edge_4_6pct", min: 0.04, max: 0.06 },
    { label: "edge_6_8pct", min: 0.06, max: 0.08 },
    { label: "edge_8_10pct", min: 0.08, max: 0.1 },
    { label: "edge_10_15pct", min: 0.1, max: 0.15 },
    { label: "edge_15plus", min: 0.15, max: 1.0 },
  ];
  for (const bucket of edgeBuckets) {
    const bucketRows = rows.filter(
      r => r.edge !== null && r.edge >= bucket.min && r.edge < bucket.max
    );
    if (bucketRows.length > 0) {
      segments.push(
        computeSegmentStats(bucketRows, market, "edge_bucket", bucket.label)
      );
    }
  }
  ...
  const probBuckets = [
    { label: "prob_50_55", min: 0.5, max: 0.55 },
    { label: "prob_55_60", min: 0.55, max: 0.6 },
    { label: "prob_60_65", min: 0.6, max: 0.65 },
    { label: "prob_65_70", min: 0.65, max: 0.7 },
    { label: "prob_70plus", min: 0.7, max: 1.0 },
  ];
  for (const bucket of probBuckets) {
    const bucketRows = rows.filter(
      r =>
        r.modelProb !== null &&
        r.modelProb >= bucket.min &&
        r.modelProb < bucket.max
    );
    if (bucketRows.length > 0) {
      segments.push(
        computeSegmentStats(
  ...
  for (const tier of oddsTiers) {
    ...
    if (tierRows.length >= MIN_SEGMENT_SAMPLE) {
      segments.push(
        computeSegmentStats(tierRows, market, "odds_tier", tier.label)
      );
    }
  }
...
function buildTrendSegments(
  ...
  if (sorted.length > 0) {
    ...
    if (last30.length >= MIN_SEGMENT_SAMPLE) {
      segments.push(
        computeSegmentStats(last30, market, "trend_window", "last_30_days", {
          reportingOnly: true,
          leakageSafe: false,
        })
      );
  ...
  if (sorted.length > 0) {
    ...
    if (last60.length >= MIN_SEGMENT_SAMPLE) {
      segments.push(
        computeSegmentStats(last60, market, "trend_window", "last_60_days", {
          reportingOnly: true,
          leakageSafe: false,
        })
      );
...
export async function runSegmentationReport(
  ...
  const teamSideSegments = allSegments.filter(
    s => s.segmentName === "team_side"
  );
  const teamSideTotal = teamSideSegments.reduce(
    (s, seg) => s + seg.sourceRowCount,
    0
  );
  const reconciled =
    teamSideTotal === rows.length || teamSideSegments.length === 0;
  ...
  if (!reconciled) {
    auditLog(
      ...
      reconciliationNote,
      {
        ...
        failed: 1,
        impact: "segment_counts_may_be_incorrect",
        action: "investigate_missing_rows",
      }
    );
  }
