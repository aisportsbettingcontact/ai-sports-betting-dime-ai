...
const WEIGHTS = { 2024: 0.2, 2025: 0.3, 2026: 0.5 };
...
export async function seedParkFactors(): Promise<{
  inserted: number;
  updated: number;
  errors: number;
}> {
  console.log(
    "[INPUT] Starting park factor seeder — 2024/2025/2026 rolling seasons"
  );
  console.log(
    `[INPUT] Weights: 2026=${WEIGHTS[2026]} 2025=${WEIGHTS[2025]} 2024=${WEIGHTS[2024]}`
  );
  ...
  const leagueData: Record<
    SeasonKey,
    { totalRuns: number; totalGames: number }
  > = {
  ...
  for (const tv of TEAM_VENUES) {
    ...
    for (const season of SEASONS) {
      try {
        ...
        console.log(
          `[STATE] ${tv.abbrev} (${tv.venueName}) ${season}: runs=${d.totalRuns} games=${d.games} avgRpg=${d.avgRpg.toFixed(3)}`
        );
      } catch (e: any) {
        console.warn(
          `[STATE] ${tv.abbrev} ${season}: FETCH ERROR — ${e.message}`
        );
        seasonResults[season as SeasonKey] = {
          totalRuns: 0,
          games: 0,
          avgRpg: 0,
        };
  ...
  const leagueAvgRpg: Record<SeasonKey, number> = {
    2024:
      leagueData[2024].totalGames > 0
        ? leagueData[2024].totalRuns / leagueData[2024].totalGames
        : 9.0,
    2025:
      leagueData[2025].totalGames > 0
        ? leagueData[2025].totalRuns / leagueData[2025].totalGames
        : 9.0,
    2026:
      leagueData[2026].totalGames > 0
        ? leagueData[2026].totalRuns / leagueData[2026].totalGames
        : 9.0,
  };
  for (const season of SEASONS) {
    const s = season as SeasonKey;
    console.log(
      `[STATE] League ${season}: totalRuns=${leagueData[s].totalRuns} totalGames=${leagueData[s].totalGames} avgRpg=${leagueAvgRpg[s].toFixed(4)}`
    );
  }
  ...
  for (const tv of TEAM_VENUES) {
    ...
    const pf2024 =
      sd[2024].avgRpg > 0 && leagueAvgRpg[2024] > 0
        ? sd[2024].avgRpg / leagueAvgRpg[2024]
        : null;
    const pf2025 =
      sd[2025].avgRpg > 0 && leagueAvgRpg[2025] > 0
        ? sd[2025].avgRpg / leagueAvgRpg[2025]
        : null;
    const pf2026 =
      sd[2026].avgRpg > 0 && leagueAvgRpg[2026] > 0
        ? sd[2026].avgRpg / leagueAvgRpg[2026]
        : null;
    ...
    if (available.length > 0) {
      ...
      parkFactor3yr = available.reduce(
        (s, x) => s + x.pf * (x.w / totalWeight),
        0
      );
    }
    if (!include2026 && pf2026 !== null) {
      console.log(
        `[STATE] ${tv.abbrev}: pf2026=${pf2026.toFixed(4)} excluded (games2026=${games2026} < MIN_GAMES_FOR_PF=${MIN_GAMES_FOR_PF}) — using prior years only`
      );
    }

    const leagueAvgDisplay =
      leagueAvgRpg[2026] > 0 ? leagueAvgRpg[2026] : leagueAvgRpg[2025];

    console.log(
      `[STATE] ${tv.abbrev}: pf2024=${pf2024?.toFixed(4) ?? "N/A"} pf2025=${pf2025?.toFixed(4) ?? "N/A"} pf2026=${pf2026?.toFixed(4) ?? "N/A"} → 3yr=${parkFactor3yr.toFixed(4)}`
    );

    // Validate: park factor should be between 0.70 and 1.50
    if (parkFactor3yr < 0.7 || parkFactor3yr > 1.5) {
      console.warn(
        `[VERIFY] WARN — ${tv.abbrev} parkFactor3yr=${parkFactor3yr.toFixed(4)} outside expected range [0.70, 1.50]`
      );
    }

    try {
      const existing = await db
        .select({ id: mlbParkFactors.id })
        .from(mlbParkFactors)
  ...
  console.log(
    `[OUTPUT] inserted=${inserted} updated=${updated} errors=${errors}`
  );
  ...
  const sorted = [...allRows].sort(
    (a, b) => (b.parkFactor3yr ?? 0) - (a.parkFactor3yr ?? 0)
  );
  console.log("\n[VERIFY] Top 5 hitter-friendly parks:");
  sorted.slice(0, 5).forEach((r: (typeof allRows)[0]) => {
    console.log(
      `  ${r.teamAbbrev} (${r.venueName}): 3yr=${r.parkFactor3yr?.toFixed(4)} | 2024=${r.pf2024?.toFixed(4) ?? "N/A"} 2025=${r.pf2025?.toFixed(4) ?? "N/A"} 2026=${r.pf2026?.toFixed(4) ?? "N/A"}`
    );
  });
  ...
  if (coors && coorsRank <= 3) {
    console.log(
      `[VERIFY] PASS — Coors Field ranked #${coorsRank} with pf3yr=${coors.parkFactor3yr?.toFixed(4)}`
    );
  } else {
    console.warn(
      `[VERIFY] WARN — Coors Field ranked #${coorsRank} (expected top 3), pf3yr=${coors?.parkFactor3yr?.toFixed(4) ?? "N/A"}`
    );
  }

  if (errors === 0) {
    console.log(
      `[VERIFY] PASS — ${inserted + updated} park factors seeded with 0 errors`
    );
  } else {
...
if (
  process.argv[1]?.endsWith("seedParkFactors.ts") ||
  process.argv[1]?.endsWith("seedParkFactors.js")
) {
