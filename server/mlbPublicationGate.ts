...
export type PublicationGateStatus =
  "SAFE_TO_PUBLISH" | "BLOCKED" | "PARTIALLY_SAFE" | "REQUIRES_MANUAL_REVIEW";
...
export function runMarketGate(
  ...
  const sampleCheck: GateCheck = {
    ...
    severity: "CRITICAL",
    reason:
      stats.gradedCount >= GATE.MIN_SAMPLE
  ...
  const accCheck: GateCheck = {
    ...
    severity: "CRITICAL",
    reason:
      stats.accuracy >= GATE.ACCURACY_HARD_FLOOR
  ...
  const quarantineCheck: GateCheck = {
    ...
    severity: "WARN",
    reason:
      quarantineRate < GATE.QUARANTINE_RATE_MAX
  ...
  auditLog(
    ...
    `${market} gate status: ${status} | ${reason}`,
    {
      ...
      action:
        status === "BLOCKED" ? "resolve_blockers_before_publication" : "none",
    }
  );
...
export function buildPublicationGateReport(
  ...
  const safeMarkets = marketResults
    .filter(r => r.status === "SAFE_TO_PUBLISH")
    .map(r => r.market);
  const blockedMarkets = marketResults
    .filter(r => r.status === "BLOCKED")
    .map(r => r.market);
  const partialMarkets = marketResults
    .filter(r => r.status === "PARTIALLY_SAFE")
    .map(r => r.market);
  const reviewMarkets = marketResults
    .filter(r => r.status === "REQUIRES_MANUAL_REVIEW")
    .map(r => r.market);
  ...
  auditLog(
    ...
    `Overall gate status: ${overallStatus} | Trust: ${finalTrustStatus}`,
    {
      ...
      impact:
        overallStatus === "BLOCKED"
          ? "all_markets_blocked"
          : "partial_publication_possible",
      action:
        blockedMarkets.length > 0
          ? "resolve_blockers_for_blocked_markets"
          : "none",
    }
  );
