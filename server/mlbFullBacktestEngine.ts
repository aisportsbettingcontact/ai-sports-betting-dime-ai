...
export const MARKET_META: Record<
  MarketKey,
  { label: string; group: string; breakeven: number }
> = {
...
async function getMarketStats(
  ...
  for (const row of rows) {
    ...
    const modelProb =
      row.modelProb != null ? parseFloat(String(row.modelProb)) : null;
    const bookProb =
      row.bookNoVigProb != null ? parseFloat(String(row.bookNoVigProb)) : null;
...
export async function getFullBacktestReport(
  ...
  console.log(
    `${TAG} [INPUT] days=${days} minEdge=${minEdge} minSample=${minSample}`
  );
  ...
  for (const market of ALL_MARKETS) {
    ...
    if (stats.wins + stats.losses >= minSample) {
      status = acc >= 0.7 ? "above_target" : "below_target";
    }
  ...
  const above70 = marketStats.filter(
    m => m.accuracy >= 0.7 && m.wins + m.losses >= minSample
  ).length;
  const above60 = marketStats.filter(
    m => m.accuracy >= 0.6 && m.wins + m.losses >= minSample
  ).length;
  const above50 = marketStats.filter(
    m => m.accuracy >= 0.5 && m.wins + m.losses >= minSample
  ).length;
  ...
  console.log(
    `${TAG} [OUTPUT] totalGames=${totalGames} totalBets=${totalBets} overallAcc=${(overallAcc * 100).toFixed(1)}% overallRoi=${(overallRoi * 100).toFixed(1)}%`
  );
  console.log(
    `${TAG} [VERIFY] above70=${above70} above60=${above60} above50=${above50}`
  );
...
export async function getDailyBacktestTimeSeries(
  ...
  for (const [date, d] of Array.from(byDate.entries()).sort()) {
    ...
    points.push({
      ...
      losses: d.losses,
      accuracy:
        dailyTotal > 0 ? parseFloat((d.wins / dailyTotal).toFixed(4)) : 0,
...
export async function getKPropsBacktestReport(
  days: number
): Promise<KPropsReport> {
  const db = await getDb();
  const cutoff = cutoffTs(days);
  ...
  for (const row of rows) {
    ...
    const actual =
      row.actualKs != null ? parseFloat(String(row.actualKs)) : null;
    const bookLine =
      row.bookLine != null ? parseFloat(String(row.bookLine)) : null;
    const verdict = row.verdict as string | null;
    const btResult = row.backtestResult as string | null;
    const edge =
      verdict === "OVER"
        ? row.edgeOver != null
          ? parseFloat(String(row.edgeOver))
          : null
        : row.edgeUnder != null
          ? parseFloat(String(row.edgeUnder))
          : null;
...
export async function getHrPropsBacktestReport(
  days: number
): Promise<HrPropsReport> {
  const db = await getDb();
  const cutoff = cutoffTs(days);
  ...
  for (const row of rows) {
    const modelPHr =
      row.modelPHr != null ? parseFloat(String(row.modelPHr)) : null;
    const actualHr =
      row.actualHr != null ? parseInt(String(row.actualHr), 10) : null;
    const bookOdds =
      row.consensusOverOdds != null
        ? parseInt(String(row.consensusOverOdds), 10)
        : null;
  ...
  const avgModelPHr =
    count > 0 ? parseFloat((modelPHrSum / count).toFixed(4)) : 0;
  const avgActualHrRate =
    count > 0 ? parseFloat((actualHrSum / count).toFixed(4)) : 0;
  ...
  return {
    ...
    byProbBucket: probBuckets
      ...
      .map(pb => ({
        ...
        avgProb:
          pb.count > 0 ? parseFloat((pb.probSum / pb.count).toFixed(4)) : 0,
        count: pb.count,
      })),
...
export async function runHistoricalBacktestRange(
  ...
  console.log(
    `${TAG} [INPUT] runHistoricalBacktestRange start=${startDate} end=${endDate}`
  );
  ...
  const dates = dateRows
    .map((r: { gameDate: string | null }) => r.gameDate)
    .filter(Boolean) as string[];
  console.log(`${TAG} [STATE] Found ${dates.length} dates to backtest`);

  let processed = 0,
    errors = 0;
  const { runMultiMarketBacktestForDate } =
  ...
  for (const date of dates) {
    try {
      ...
      if (processed % 5 === 0) {
        console.log(
          `${TAG} [STATE] Progress: ${processed}/${dates.length} dates processed`
        );
      }
    } catch (err) {
      errors++;
      console.error(
        `${TAG} [ERROR] date=${date}: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }
  ...
  console.log(
    `${TAG} [VERIFY] ${errors === 0 ? "PASS" : "WARN"} — ${errors} errors in ${dates.length} dates`
  );
