...
// ─── Team stats scraper ───────────────────────────────────────────────────────
export async function scrapeNhlTeamStatsFromMoneyPuck(): Promise<
...
> {
  ...
  if (!resp.ok)
    throw new Error(`[MPFallback] Teams CSV fetch failed: HTTP ${resp.status}`);
  ...
  for (const row of fv5) {
    ...
    if (iceTimeMin < 1) {
      console.warn(
        `[MPFallback]   Skipping ${abbrev}: near-zero icetime (${iceTimeMin.toFixed(1)} min)`
      );
      continue;
    }
  ...
  console.log(
    `[MPFallback]   ✅ Team stats computed for ${statsMap.size} teams`
  );
  ...
  for (const [abbrev, s] of Array.from(statsMap.entries())) {
    if (s.xGF_60 < 1.5 || s.xGF_60 > 5.0) {
      console.warn(
        `[MPFallback]   ⚠ ${abbrev}: xGF_60=${s.xGF_60} (out of expected 1.5–5.0 range)`
      );
    }
  }
...
export async function scrapeNhlGoalieStatsFromMoneyPuck(): Promise<
...
> {
  ...
  if (!resp.ok)
    throw new Error(
      `[MPFallback] Goalies CSV fetch failed: HTTP ${resp.status}`
    );
  const text = await resp.text();
  const rows = parseCSV(text);
  ...
  for (const row of allSit) {
    ...
    const sv_pct = shots > 0 ? (shots - GA) / shots : 0.91;
  ...
  console.log(
    `[MPFallback]   ✅ Goalie stats loaded for ${goalieMap.size} goalies`
  );
  return goalieMap;
}
