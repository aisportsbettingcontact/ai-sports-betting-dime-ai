...
export const moneylineVerdictSchema = baseVerdictSchema.extend({
  market_type: z.literal("moneyline"),
});
export const spreadVerdictSchema = baseVerdictSchema.extend({
  market_type: z.literal("spread"),
  current_line: z.number().optional(),
});
export const totalVerdictSchema = baseVerdictSchema.extend({
  market_type: z.literal("total"),
  selection: z.enum(["over", "under"]).optional(),
  current_line: z.number().optional(),
});
export const playerPropVerdictSchema = baseVerdictSchema.extend({
  market_type: z.literal("player_prop"),
  player_id: z.string().min(1).optional(),
  prop_type: z.string().min(1).optional(),
  current_line: z.number().optional(),
});
...
export function expectedValue(
  ...
  return Number(
    (modelProbability * payout - (1 - modelProbability)).toFixed(6)
  );
}
...
function hasEdgeRequirements(verdict: DimeStructuredVerdict): string[] {
  ...
  if (verdict.model_probability === undefined)
    missing.push("model_probability");
...
export function validateDimeStructuredVerdict(
  raw: unknown,
  now = new Date()
): DimeVerdictValidationResult {
  ...
  if (verdict.verdict === "edge_detected") {
    for (const field of hasEdgeRequirements(verdict))
      errors.push(`edge_detected_missing_${field}`);
    if (
      verdict.data_quality === "missing" ||
      verdict.data_quality === "conflict" ||
      verdict.data_quality === "stale"
    )
      errors.push(`edge_detected_invalid_data_quality_${verdict.data_quality}`);
  }

  if (
    verdict.odds_observed_at &&
    ageMs(verdict.odds_observed_at, now) > DIME_ODDS_FRESHNESS_MS &&
    verdict.verdict === "edge_detected"
  ) {
    errors.push("edge_detected_stale_odds");
  }
  if (
    verdict.projection_observed_at &&
    ageMs(verdict.projection_observed_at, now) > DIME_PROJECTION_FRESHNESS_MS &&
    verdict.verdict === "edge_detected"
  ) {
  ...
  if (
    verdict.current_odds !== undefined &&
    verdict.model_probability !== undefined
  ) {
    const implied = americanToImpliedProbability(verdict.current_odds);
    const expectedEdge = Number(
      (
        verdict.model_probability -
        (verdict.no_vig_market_probability ?? implied)
      ).toFixed(6)
    );
    ...
    corrected.calculation_provenance = Array.from(
      new Set([
        ...verdict.calculation_provenance,
        "deterministic_recalculation_v1",
      ])
    );

    if (
      verdict.probability_edge !== undefined &&
      Math.abs(verdict.probability_edge - expectedEdge) > DIME_EDGE_TOLERANCE
    )
      errors.push("probability_edge_mismatch");
    if (
      verdict.expected_value !== undefined &&
      Math.abs(verdict.expected_value - ev) > DIME_EDGE_TOLERANCE
    )
      errors.push("expected_value_mismatch");
    if (
      verdict.fair_odds !== undefined &&
      Math.abs(verdict.fair_odds - fairOdds) > 2
    )
      errors.push("fair_odds_mismatch");
  }

  if (
    verdict.verdict === "need_more_data" &&
    verdict.missing_data_fields.length === 0
  )
    errors.push("need_more_data_requires_missing_fields");
  if (
    verdict.verdict === "data_conflict" &&
    verdict.primary_risk_code === undefined
  )
    errors.push("data_conflict_requires_risk_code");

  return { ok: errors.length === 0, verdict, corrected, errors };
}

const verdictJsonPattern =
  /```(?:json)?\s*\{\s*"schema_version"[\s\S]*?\}\s*```|\{\s*"schema_version"[\s\S]*?\}/g;
...
export function validateDimeResponseText(
  text: string,
  now = new Date()
): DimeVerdictValidationResult {
  const candidates = extractDimeVerdictCandidates(text);
  const hasLegacyEdge =
    text.includes("[EDGE]") || text.includes("edge_detected");
  if (candidates.length === 0) {
    return hasLegacyEdge
      ? { ok: false, errors: ["edge_claim_without_valid_structured_verdict"] }
      : { ok: true, errors: [] };
  }

  const results = candidates.map(candidate =>
    validateDimeStructuredVerdict(candidate, now)
  );
