...
describe("Dime responsible gambling safety", () => {
  it("does not hardcode a US hotline for unknown jurisdiction", () => {
    const result = assessDimeResponsibleGamblingSafety(
      "I am chasing losses and cannot stop",
      "unknown"
    );
  ...
  it("detects prohibited certainty language", () => {
    expect(
      containsProhibitedBettingCertainty("This is a guaranteed lock")
    ).toBe(true);
    expect(
      containsProhibitedBettingCertainty("This is a small edge with risk")
    ).toBe(false);
  });
});
