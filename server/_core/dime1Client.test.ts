...
describe("resolveDime1Config", () => {
  ...
  it("prefers DIME_MODEL_API_SECRET over RUNPOD_API_KEY for the bearer token", () => {
    const config = resolveDime1Config({
      ...RUNPOD_ENV,
      DIME_MODEL_API_SECRET: "private-lb-secret",
    });
  ...
  it("pins the served model from DIME_MODEL_VERSION and falls back to dime-1.0", () => {
    ...
    expect(
      resolveDime1Config({
        ...RUNPOD_ENV,
        DIME_MODEL_VERSION: "dime-1.0-v1.0.0",
      })?.model
    ).toBe("dime-1.0-v1.0.0");
  });

  it("applies the timeout default and override", () => {
    expect(resolveDime1Config(RUNPOD_ENV)?.timeoutMs).toBe(60_000);
    expect(
      resolveDime1Config({ ...RUNPOD_ENV, DIME_MODEL_TIMEOUT_MS: "15000" })
        ?.timeoutMs
    ).toBe(15_000);
    expect(
      resolveDime1Config({ ...RUNPOD_ENV, DIME_MODEL_TIMEOUT_MS: "junk" })
        ?.timeoutMs
    ).toBe(60_000);
  });
});
...
describe("dime1ChatComplete", () => {
  it("POSTs an OpenAI-compatible chat completion and parses the reply", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          ...
          choices: [
            {
              message: { content: "PHI -1.5 is a pass at -120." },
              finish_reason: "stop",
            },
          ],
          usage: {
    ...
    const result = await dime1ChatComplete(
      {
        system: "sys",
        messages: [{ role: "user", content: "Phillies spread?" }],
        maxTokens: 512,
      },
      RUNPOD_ENV
    );
    ...
    expect(url).toBe(
      "https://api.runpod.ai/v2/ep123abc/openai/v1/chat/completions"
    );
  ...
  it("throws Dime1ApiError with the upstream status on non-2xx", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response("worker cold start timeout", { status: 503 })
        )
    );
    await expect(
      dime1ChatComplete(
