...
async function fetchBacktestRows(
  ...
  return rows.map((r: DbRow) => ({
    ...
    bookNoVigProb:
      r.bookNoVigProb !== null ? parseFloat(String(r.bookNoVigProb)) : null,
    result: r.result,
    correct: r.correct,
...
export async function runWalkForwardForMarket(
  ...
  for (const fold of folds) {
    ...
    const leakageViolations = testRows.filter(
      r => r.result === "QUARANTINED"
    ).length;

    const trustStatus: FoldResult["trustStatus"] =
      testStats.sampleSize < config.minSamplePerFold
        ? "INSUFFICIENT_DATA"
        : testStats.accuracy >= 0.7 && testStats.roi > 0
          ? "PASS"
          : "FAIL";
    ...
    auditLog(
      ...
      `Fold ${fold.foldIndex} ${trustStatus}: ${trustReason}`,
      {
        ...
        date_min: fold.testStart,
        date_max: fold.testEnd,
  ...
  const overallAcc =
    totalTestWins + totalTestLosses > 0
      ? totalTestWins / (totalTestWins + totalTestLosses)
      : 0;
  ...
  const avgBrier =
    brierFolds.length > 0
      ? brierFolds.reduce((s, f) => s + f.testBrierScore!, 0) /
        brierFolds.length
      : null;
  const llFolds = foldResults.filter(f => f.testLogLoss !== null);
  const avgLL =
    llFolds.length > 0
      ? llFolds.reduce((s, f) => s + f.testLogLoss!, 0) / llFolds.length
      : null;

  // Stability: std dev of per-fold accuracy and ROI
  const accs = foldResults
    .filter(f => f.testSampleSize >= config.minSamplePerFold)
    .map(f => f.testAccuracy);
  const rois = foldResults
    .filter(f => f.testSampleSize >= config.minSamplePerFold)
    .map(f => f.testRoi);
  ...
  const totalFolds = foldResults.filter(
    f => f.trustStatus !== "INSUFFICIENT_DATA"
  ).length;

  const overallTrustStatus: WalkForwardResult["overallTrustStatus"] =
    totalFolds === 0
      ? "INSUFFICIENT_DATA"
      : passFolds / totalFolds >= 0.7
        ? "PASS"
        : "FAIL";

  const overallTrustReason =
    overallTrustStatus === "INSUFFICIENT_DATA"
  ...
  auditLog(
    ...
    `Walk-forward ${overallTrustStatus}: ${overallTrustReason}`,
    {
      ...
      impact:
        overallTrustStatus === "FAIL"
          ? `${market}_walk_forward_failed`
          : "none",
      action:
        overallTrustStatus === "FAIL" ? "recalibrate_or_investigate" : "none",
    }
  );
...
export async function runWalkForwardAllMarkets(
  ...
  for (const market of markets) {
    const result = await runWalkForwardForMarket(
      market,
      dataStartDate,
      dataEndDate,
      config
  ...
  const insuffCount = results.filter(
    r => r.overallTrustStatus === "INSUFFICIENT_DATA"
  ).length;
  ...
  auditLog(
    ...
    `Walk-forward complete: ${passCount} PASS | ${failCount} FAIL | ${insuffCount} INSUFFICIENT_DATA`,
    {
      ...
      failed: failCount,
      impact:
        failCount > 0 ? "some_markets_below_target" : "all_markets_passed",
