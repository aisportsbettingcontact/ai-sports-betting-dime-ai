...
describe("NRFI grading", () => {
  ...
  it("NRFI and YRFI are complementary (opposite grades)", () => {
    const nrfiOut = gradeNrfi(
      makeInput("nrfi", {
        actualValue: { nrfiResult: "NRFI" },
      })
    );
    const yrfiOut = gradeYrfi(
      makeInput("yrfi", {
        actualValue: { nrfiResult: "NRFI" },
      })
    );
...
describe("checkLeakage", () => {
  ...
  it("safe but unverifiable when startTimeEst is TBD", () => {
    const result = checkLeakage(MODEL_RUN_BEFORE, null, "2025-04-15", "TBD");
    expect(result.safe).toBe(true);
    expect(result.reason).toContain("UNVERIFIABLE_GAME_TIME");
  });

  it("safe but unverifiable when startTimeEst is null", () => {
    const result = checkLeakage(MODEL_RUN_BEFORE, null, "2025-04-15", null);
    expect(result.safe).toBe(true);
    expect(result.reason).toContain("UNVERIFIABLE_GAME_TIME");
  });
});
...
describe("runMarketGate", () => {
  ...
  it("SAFE_TO_PUBLISH when all checks pass and accuracy >= 85%", () => {
    const result = runMarketGate(
      makeStats({ accuracy: 0.87, roi: 0.12 }),
      null,
      null
    );
  ...
  it("PARTIALLY_SAFE when accuracy is 70-85%", () => {
    const result = runMarketGate(
      makeStats({ accuracy: 0.72, roi: 0.05 }),
      null,
      null
    );
  ...
  it("BLOCKED when accuracy < 70%", () => {
    const result = runMarketGate(
      makeStats({ accuracy: 0.6, roi: 0.05 }),
      null,
      null
    );
  ...
  it("BLOCKED when ROI is negative", () => {
    const result = runMarketGate(
      makeStats({ accuracy: 0.75, roi: -0.05 }),
      null,
      null
    );
    expect(result.status).toBe("BLOCKED");
  ...
  it("BLOCKED when sample size < 30", () => {
    const result = runMarketGate(
      makeStats({ gradedCount: 15, wins: 11, losses: 4 }),
      null,
      null
    );
    expect(result.status).toBe("BLOCKED");
    ...
    const results: MarketGateResult[] = [
      {
        ...
        target85Achieved: true,
        target85Realistic: true,
        target85Evidence: "ACHIEVED",
        reason: "SAFE_TO_PUBLISH",
      },
    ];
    ...
    const results: MarketGateResult[] = [
      {
        ...
        target85Achieved: false,
        target85Realistic: true,
        target85Evidence: "NOT_ACHIEVED",
        reason: "BLOCKED",
      },
    ];
    const report = buildPublicationGateReport(results);
...
describe("Segmentation engine utilities", () => {
  ...
  it("getBestSegments returns segments sorted by accuracy descending", () => {
    const segments = [
      makeSegment({ segmentValue: "home", accuracy: 0.7 }),
    ...
    const outputs: GradingOutput[] = [
      ...
      },
      {
        ...gradeFgMl(
          makeInput("fg_ml_home", {
            modelSide: "home",
            actualValue: { fgHomeRuns: 2, fgAwayRuns: 4 },
          })
        ),
      },
      {
        ...gradeFgMl(
          makeInput("fg_ml_home", { actualValue: { isPostponed: true } })
        ),
      },
      {
        ...gradeFgMl(
