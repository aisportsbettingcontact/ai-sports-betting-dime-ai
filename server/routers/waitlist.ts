...
export const waitlistRouter = router({
  ...
  submit: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      console.log(
        `[WaitlistRouter][INPUT] fullName=${input.fullName ?? "(none)"} whyText=${input.whyText ? "(set)" : "(none)"} unitSize=${input.unitSizeMin ?? "?"}-${input.unitSizeMax ?? "?"} step2=${input.step2Completed ?? false} utmSource=${input.utmSource ?? "(none)"}`
      );
      ...
      try {
        ...
        throw new TRPCError({
          ...
          message: "Failed to save waitlist entry. Please try again.",
        });
      }

      if (!result.ok) {
        console.log(
          `[WaitlistRouter][OUTPUT] submit — DUPLICATE email=${input.email}`
        );
      ...
      console.log(
        `[WaitlistRouter][OUTPUT] submit — CREATED id=${result.id} email=${input.email}`
      );
  ...
  enrichStep2: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      console.log(
        `[WaitlistRouter][INPUT] fullName=${input.fullName ?? "(none)"} whyText=${input.whyText ? "(set, len=" + input.whyText.length + ")" : "(none)"} unitSize=${input.unitSize ?? "(none)"}`
      );
      ...
      if (!result.ok) {
        console.log(
          `[WaitlistRouter][OUTPUT] enrichStep2 — NOT_FOUND email=${input.email}`
        );
      ...
      console.log(
        `[WaitlistRouter][OUTPUT] enrichStep2 — UPDATED email=${input.email}`
      );
  ...
  // ── list (owner) ───────────────────────────────────────────────────────────
  list: ownerProcedure.input(zListInput).query(async ({ input }) => {
    console.log(
      `[WaitlistRouter][STEP] list — status=${input.status} search=${input.search ?? "(none)"} limit=${input.limit} offset=${input.offset}`
    );
    ...
    console.log(
      `[WaitlistRouter][OUTPUT] list — total=${result.total} returned=${result.rows.length}`
    );
  ...
  stats: ownerProcedure.query(async () => {
    ...
    console.log(
      `[WaitlistRouter][OUTPUT] stats — total=${stats.total} pending=${stats.pending} approved=${stats.approved} denied=${stats.denied}`
    );
  ...
  updateStatus: ownerProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      console.log(
        `[WaitlistRouter][STEP] updateStatus — id=${input.id} newStatus=${input.status} reviewedBy=${reviewerId}`
      );
      console.log(
        `[WaitlistRouter][INPUT] adminNote=${input.adminNote ? `"${input.adminNote.slice(0, 80)}"` : "(none)"}`
      );
      ...
      console.log(
        `[WaitlistRouter][OUTPUT] updateStatus — id=${updated.id} email=${updated.email} status=${updated.status}`
      );
  ...
  bulkUpdate: ownerProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const reviewerId = ctx.appUser.id;
      console.log(
        `[WaitlistRouter][STEP] bulkUpdate — ids=[${input.ids.join(",")}] newStatus=${input.status} reviewedBy=${reviewerId}`
      );
      ...
      console.log(
        `[WaitlistRouter][OUTPUT] bulkUpdate — updated=${count} status=${input.status}`
      );
  ...
  // ── delete (owner) ─────────────────────────────────────────────────────────
  delete: ownerProcedure.input(zDeleteInput).mutation(async ({ input }) => {
    ...
    if (!deleted) {
      ...
      throw new TRPCError({
        code: "NOT_FOUND",
        message: `Waitlist entry id=${input.id} not found`,
      });
    }
  ...
  // ── exportCsv (owner) ──────────────────────────────────────────────────────
  exportCsv: ownerProcedure.input(zExportInput).query(async ({ input }) => {
