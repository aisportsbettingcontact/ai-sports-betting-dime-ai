...
const BACKTEST_SCRIPT_CANDIDATES = [
  ...
  path.resolve(__dirname, "scripts", "runMlbBacktest2.py"),
  path.resolve(__dirname, "..", "server", "scripts", "runMlbBacktest2.py"),
];
const BACKTEST_SCRIPT =
  BACKTEST_SCRIPT_CANDIDATES.find(p => fs.existsSync(p)) ??
  BACKTEST_SCRIPT_CANDIDATES[0];
...
const CALIBRATION_JSON =
  process.env.MLB_CALIBRATION_PATH ??
  path.join(os.tmpdir(), "mlb_calibration_constants.json");
...
async function persistDriftState(
  ...
  try {
    ...
    console.error(
      `${TAG} [ERROR] persistDriftState failed: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
...
async function seedCalibrationConstants(): Promise<void> {
  ...
  const BASELINE_PARAMS = [
    {
      paramName: "f5_share",
      currentValue: "0.56180000",
      baselineValue: "0.56180000",
      sampleSize: 5103,
      ciLower: "0.55800000",
      ciUpper: "0.56560000",
    },
    {
      paramName: "nrfi_rate",
      currentValue: "0.48820000",
      baselineValue: "0.48820000",
      sampleSize: 5103,
      ciLower: "0.47900000",
      ciUpper: "0.49740000",
    },
    {
      paramName: "k_calibration_factor",
      currentValue: "1.00000000",
      baselineValue: "1.00000000",
      sampleSize: 0,
      ciLower: "0.95000000",
      ciUpper: "1.05000000",
    },
    {
      paramName: "hr_base_rate",
      currentValue: "0.10090238",
      baselineValue: "0.10090238",
      sampleSize: 2438,
      ciLower: "0.09590238",
      ciUpper: "0.10590238",
    },
    {
      paramName: "f5_under_bias",
      currentValue: "0.00000000",
      baselineValue: "0.00000000",
      sampleSize: 0,
      ciLower: "-0.05000000",
      ciUpper: "0.05000000",
    },
    {
      paramName: "fg_ml_home_edge",
      currentValue: "0.00000000",
      baselineValue: "0.00000000",
      sampleSize: 0,
      ciLower: "-0.05000000",
  ...
  for (const param of BASELINE_PARAMS) {
    ...
    seeded++;
    console.log(
      `${TAG_SEED} [OUTPUT] Seeded param=${param.paramName} value=${param.currentValue}`
    );
  }
  console.log(
    `${TAG_SEED} [VERIFY] Seeded ${seeded} params, skipped ${skipped} (already exist)`
  );
}
...
async function computeRollingF5Share(windowSize = WINDOW_SIZE): Promise<{
  ...
  type RowType = {
    ...
    actualF5Total: string | null;
    actualFgTotal: string | null;
  };
  ...
  const meanShare =
    gameData.reduce((s: number, g: { share: number }) => s + g.share, 0) /
    gameData.length;
 ...
 */
export async function checkF5ShareDrift(
  triggerRecal = true
  ...
  console.log(
    `${TAG} [INPUT] BASELINE_F5_SHARE=${BASELINE_F5_SHARE} DRIFT_THRESHOLD=${DRIFT_THRESHOLD} WINDOW_SIZE=${WINDOW_SIZE} MIN_SAMPLE=${MIN_SAMPLE} triggerRecal=${triggerRecal}`
  );

  // ── Step 1: Compute rolling f5_share ─────────────────────────────────────
  console.log(
    `${TAG} [STEP 1] Computing rolling f5_share over last ${WINDOW_SIZE} games`
  );
  const {
    share: rollingF5Share,
    sampleSize,
    games: gameData,
  } = await computeRollingF5Share();

  console.log(
    `${TAG} [STATE] sampleSize=${sampleSize} rollingF5Share=${rollingF5Share ?? "null (insufficient data)"}`
  );
  ...
  const delta = parseFloat(
    Math.abs(rollingF5Share - BASELINE_F5_SHARE).toFixed(6)
  );
  ...
  if (gameData.length > 0) {
    ...
    const std = parseFloat(
      Math.sqrt(
        shares.reduce((s, v) => s + Math.pow(v - rollingF5Share, 2), 0) /
          shares.length
      ).toFixed(6)
    );
    console.log(
      `${TAG} [STATE] Distribution: min=${min} max=${max} std=${std} n=${gameData.length}`
    );
  }
  ...
  console.log(
    `${TAG} [DRIFT] DRIFT DETECTED — delta=${delta} > threshold=${DRIFT_THRESHOLD}`
  );
  console.log(
    `${TAG} [DRIFT] rolling=${rollingF5Share} baseline=${BASELINE_F5_SHARE} direction=${direction} n=${sampleSize}`
  );
...
async function writeDriftEvent(
  ...
  try {
    ...
    console.log(
      `${TAG} [STATE] Drift event written to mlb_model_learning_log (reason=${reason})`
    );
  } catch (err) {
    console.error(
      `${TAG} [ERROR] Failed to write drift event: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
...
export async function triggerRecalibration(
  ...
  console.log(
    `${TAG} [STEP] Reading calibration constants from ${CALIBRATION_JSON}`
  );
  ...
  const newF5Share =
    typeof overall.f5_run_share === "number" ? overall.f5_run_share : null;
  const newNrfiRate =
    typeof overall.nrfi_rate === "number" ? overall.nrfi_rate : null;
  ...
  try {
    ...
    console.log(
      `${TAG} [STATE] Recalibration completion written to mlb_model_learning_log`
    );
  } catch (err) {
    console.error(
      `${TAG} [ERROR] Failed to write recalibration log: ${err instanceof Error ? err.message : String(err)}`
    );
  }

  const elapsed = (Date.now() - startMs) / 1000;
  console.log(
    `${TAG} [OUTPUT] Recalibration complete: newF5Share=${newF5Share} newNrfiRate=${newNrfiRate} constantsPatched=${patchResult.patched} elapsed=${elapsed.toFixed(1)}s`
  );
...
async function runBacktestScript(): Promise<{
  success: boolean;
  error?: string;
  elapsedSec: number;
}> {
  ...
  return new Promise(resolve => {
    ...
    proc.on("close", code => {
      ...
      if (code === 0) {
        console.log(
          `${TAG} [STATE] Backtest completed successfully in ${elapsedSec.toFixed(1)}s`
        );
...
export async function migrateCalibrationConstants(
  ...
  if (!overall) {
    console.warn(
      `${TAG} [WARN] migrateCalibrationConstants: no 'overall' key in calibration JSON`
    );
    return { patched: 0, backup: "" };
  }
  ...
  try {
    ...
    console.error(
      `${TAG} [ERROR] Cannot read MLBAIModel.py: ${err instanceof Error ? err.message : String(err)}`
    );
    return { patched: 0, backup: "" };
  }
  ...
  function patchConstant(key: string, newValue: number, comment: string): void {
    ...
    if (!oldMatch) {
      console.warn(
        `${TAG} [WARN] Could not find constant '${key}' in MLBAIModel.py`
      );
      return;
    }
    ...
    console.log(
      `${TAG} [STATE] Patched '${key}': ${oldValue} → ${newValue.toFixed(4)} (${comment})`
    );
    patched++;
  }
  ...
  if (typeof overall.f5_run_share === "number") {
    patchConstant(
      "f5_share",
      overall.f5_run_share,
      "F5 runs / FG runs 3yr empirical"
    );
  }
  ...
  if (typeof overall.i1_run_share === "number") {
    patchConstant(
      "i1_share",
      overall.i1_run_share,
      "I1 runs / FG runs 3yr empirical"
    );
  }
  if (typeof overall.fg_home_win_rate === "number") {
    patchConstant(
      "fg_home_win_rate",
      overall.fg_home_win_rate,
      "FG ML home win rate 3yr empirical"
    );
    patchConstant(
      "fg_away_win_rate",
      1 - overall.fg_home_win_rate,
      "FG ML away win rate 3yr empirical"
    );
  }
  if (typeof overall.f5_push_rate === "number") {
    patchConstant(
      "f5_push_rate",
      overall.f5_push_rate,
      "F5 push rate 3yr empirical"
    );
  }
  if (typeof overall.fg_rl_away_cover === "number") {
    patchConstant(
      "fg_rl_away_cover",
      overall.fg_rl_away_cover,
      "FG RL away +1.5 cover rate 3yr empirical"
    );
    patchConstant(
      "fg_rl_home_cover",
      1 - overall.fg_rl_away_cover,
      "FG RL home -1.5 cover rate 3yr empirical"
    );
  }
  ...
  try {
    ...
    console.log(
      `${TAG} [OUTPUT] MLBAIModel.py patched: ${patched} constants updated`
    );
  } catch (err) {
    console.error(
      `${TAG} [ERROR] Failed to write MLBAIModel.py: ${err instanceof Error ? err.message : String(err)}`
    );
    // Restore backup
