...
type RawStatRow = Awaited<
  ReturnType<ReturnType<typeof selectStatRows>>
>[number];
...
async function loadStats(
  userId: number
): Promise<{ rows: RawStatRow[]; stats: BetStats }> {
  const rows = await selectStatRows(userId)();
  const stats = aggregateStats(rows as StatRow[], UNIT_SIZE);
  console.log(
    `[loadStats] [OUTPUT] userId=${userId} rows=${rows.length} totalBets=${stats.totalBets}`
  );
  return { rows, stats };
}

const entry = (stats: BetStats, key: string) =>
  stats.byType.find(e => e.key === key);
...
async function cleanup(): Promise<void> {
  ...
  if (ids.length > 0) {
    await db.delete(trackedBetLegs).where(
      inArray(
        trackedBetLegs.betId,
        ids.map(r => r.id)
      )
    );
  }
  ...
  console.log(
    `[cleanup] [STEP] removed ${ids.length} tracked_bets rows (+legs) for userIds ${ALL_USERS.join(",")}`
  );
}
...
async function insertParlay(v: {
  ...
  expect(betId, "$returningId must yield the parent ticket id").toBeGreaterThan(
    0
  );
  ...
  console.log(
    `[insertParlay] [OUTPUT] betId=${betId} legCount=${v.legs.length} result=${v.result}`
  );
  return betId;
}
...
describe.skipIf(SKIP_DB_SUITE)(
  "betTrackerStats.db — stats computed from real tracked_bets rows",
  () => {
    beforeAll(async () => {
      const db = await getDb();
      expect(
        db,
        "real-database suite requires a live DATABASE_URL"
      ).toBeTruthy();
      await cleanup();
    ...
    it('[BTS-1] a parlay files under PARLAY, not under the "ML" its market column actually holds', async () => {
      ...
      console.log(
        `[VERIFY] [BTS-1] ticket read back with market=${ticket.market} betType=${ticket.betType} legCount=${ticket.legCount}`
      );
      ...
      console.log(
        `[VERIFY] [BTS-1] PASS — byType PARLAY wins=${parlay!.wins} risk=${parlay!.totalRisk}U net=${parlay!.netProfit}U`
      );
      ...
      console.log(
        `[VERIFY] [BTS-1] PASS — byType ML uncontaminated: wins=${ml.wins} risk=${ml.totalRisk}U net=${ml.netProfit}U`
      );
    });
    ...
    it("[BTS-2] a straight bet still files under its own market column", async () => {
      ...
      expect(stats.byType.map(e => e.key).sort()).toEqual([
        "ML",
        "PARLAY",
        "RL",
        "TOTAL",
      ...
      console.log(
        `[VERIFY] [BTS-2] PASS — byType keys=${stats.byType.map(e => e.key).join(",")}; TOTAL risk=${total.totalRisk}U; RL pushes=${rl.pushes} risk=${rl.totalRisk}U`
      );
    });
    ...
    it("[BTS-3] the parlay's single stake is counted once, not once per leg", async () => {
      ...
      const legs = await db!
        .select({
          id: trackedBetLegs.id,
          odds: trackedBetLegs.odds,
          result: trackedBetLegs.result,
        })
        .from(trackedBetLegs)
      ...
      console.log(
        `[VERIFY] [BTS-3] PASS — 6 bets / ${legs.length} legs: risk=${stats.totalRisk}U won=${stats.totalWon}U net=${stats.netProfit}U ($${stats.dollarNetProfit})`
      );
    });
    ...
    it("[BTS-4] W/L/PUSH/VOID/PENDING counts match what was inserted", async () => {
      ...
      expect(Object.keys(byResult).sort()).toEqual([
        "LOSS",
        "PENDING",
        "PUSH",
        "VOID",
      ...
      console.log(
        `[VERIFY] [BTS-4] PASS — counts ${JSON.stringify(fromDb)} graded=${stats.gradedBets}`
      );
    });
    ...
    it("[BTS-5] DECIMAL columns arrive as strings and parse into risk/toWin/units", async () => {
      ...
      console.log(
        `[VERIFY] [BTS-5] risk typeof=${typeof win.risk} value=${win.risk}; riskUnits typeof=${typeof loss.riskUnits} nullRow=${String(win.riskUnits)}`
      );
      ...
      console.log(
        `[VERIFY] [BTS-5] PASS — risk=${stats.totalRisk}U won=${stats.totalWon}U lost=${stats.totalLost}U net=${stats.netProfit}U ($${stats.dollarNetProfit})`
      );
    });
    ...
    it("[BTS-6] the equity curve carries one point per P&L-moving bet", async () => {
      ...
      expect(mixed.equityCurve.map(p => p.result)).toEqual([
        "WIN",
        "LOSS",
        "WIN",
      ]);
      ...
      console.log(
        `[VERIFY] [BTS-6] PASS — curve=40 pts for 40 settled; mixed=3 pts for 6 bets (${mixed.equityCurve.map(p => p.cumPL).join(" → ")})`
      );
    });
    ...
    it("[BTS-7] downsampling bounds the curve while ath/maxDrawdown stay computed over the FULL series", async () => {
      ...
      console.log(
        `[VERIFY] [BTS-7] PASS — 40 → ${thinned.length} pts (${thinned.map(p => p.cumPL).join(",")}); ath=${stats.ath} maxDD=${stats.maxDrawdown} over the full series`
      );
    });
  }
);
