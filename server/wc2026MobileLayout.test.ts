...
describe("calculate3WayResult — 3-way EV math", () => {
  it("normalizes book probabilities across all 3 outcomes (sum = 1.0)", () => {
    ...
    const bookSum =
      result.home.bookFairProb +
      result.draw.bookFairProb +
      result.away.bookFairProb;
    const modelSum =
      result.home.modelFairProb +
      result.draw.modelFairProb +
      result.away.modelFairProb;
    expect(bookSum).toBeCloseTo(1.0, 4);
    expect(modelSum).toBeCloseTo(1.0, 4);

    console.log(
      `[TEST][3WAY] CZE vs RSA book fair: H=${(result.home.bookFairProb * 100).toFixed(2)}% D=${(result.draw.bookFairProb * 100).toFixed(2)}% A=${(result.away.bookFairProb * 100).toFixed(2)}% sum=${(bookSum * 100).toFixed(2)}%`
    );
  });
  ...
  it("RSA ML ROI is positive (model +237 vs DK +360 — model sees more value)", () => {
    ...
    console.log(
      `[TEST][3WAY] RSA ML ROI: ${result.away.roiPct.toFixed(2)}% edgePP: ${result.away.edgePP.toFixed(2)}pp`
    );
  });
  ...
  it("DRAW ROI uses all 3 outcomes in denominator (differs from 2-way)", () => {
    ...
    console.log(
      `[TEST][3WAY] CAN vs QAT DRAW ROI: 3-way=${result.draw.roiPct.toFixed(2)}% vs 2-way=${twoWayRoi.toFixed(2)}%`
    );
  });
...
describe("Edge detection — June 18 WC2026 matches", () => {
  it("CZE vs RSA: RSA ML edge detected (model +237 vs DK +360)", () => {
    ...
    expect(result.away.roiPct).toBeGreaterThan(0);
    console.log(
      `[TEST][EDGE] CZE vs RSA — RSA ML: edgePP=${result.away.edgePP.toFixed(2)}pp ROI=${result.away.roiPct.toFixed(2)}%`
    );
  });
  ...
  it("SUI vs BIH: SUI ML edge detected (model -240 vs DK -180)", () => {
    ...
    console.log(
      `[TEST][EDGE] SUI vs BIH — SUI ML: edgePP=${result.home.edgePP.toFixed(2)}pp ROI=${result.home.roiPct.toFixed(2)}%`
    );
  });
  ...
  it("CAN vs QAT: DRAW edge detected (model +303 vs DK +475)", () => {
    ...
    console.log(
      `[TEST][EDGE] CAN vs QAT — DRAW: edgePP=${result.draw.edgePP.toFixed(2)}pp ROI=${result.draw.roiPct.toFixed(2)}%`
    );
  });
  ...
  it("MEX vs KOR: KOR ML edge detected (model +174 vs DK +295)", () => {
    ...
    expect(result.away.roiPct).toBeGreaterThan(0);
    console.log(
      `[TEST][EDGE] MEX vs KOR — KOR ML: edgePP=${result.away.edgePP.toFixed(2)}pp ROI=${result.away.roiPct.toFixed(2)}%`
    );
  });
});
...
describe("TOTAL 2-way ROI", () => {
  it("calculateRoi returns finite number for TOTAL market", () => {
    ...
    console.log(
      `[TEST][TOTAL] CZE vs RSA OVER: edgePP=${overEdgePP.toFixed(2)}pp ROI=${overRoi.toFixed(2)}%`
    );
  });
