...
import { combineLegOdds, MAX_PARLAY_LEGS, MIN_PARLAY_LEGS } from "../../shared/parlayPricing";
...
describe("parlay settlement properties (owner contract: push drops leg + reprices; LOSS dominates)", () => {
  const resultArb = fc.constantFrom<BetResult>("PENDING", "WIN", "LOSS", "PUSH", "VOID");
  const legArb = (index: number) =>
    fc.record({
      ...
      market: fc.constantFrom("ML", "RL", "TOTAL") as fc.Arbitrary<ParlayLeg["market"]>,
      pickSide: fc.constantFrom("AWAY", "HOME", "OVER", "UNDER") as fc.Arbitrary<ParlayLeg["pickSide"]>,
      timeframe: fc.constant("FULL_GAME") as fc.Arbitrary<ParlayLeg["timeframe"]>,
  ...
  it("branch contract holds for every generated ticket", () => {
    fc.assert(
      fc.property(ticketArb, ({ legs, originalOdds }) => {
        ...
        const dropped = legs.filter((l) => l.result === "PUSH" || l.result === "VOID");
