...
console.log('fixture_id          | match_date | kickoff_utc (UTC)    | espn_event_id | venue_id | stadium                    | city          | away @ home');
...
for (const r of rows) {
  ...
  console.log(`${r.fixture_id.padEnd(20)}| ${md} | ${ku} | ${espnId.padEnd(13)} | ${String(venueId).padEnd(8)} | ${stadium} | ${city} | ${matchup}`);
}

// ET conversion check for wc26-r32-079 (Ecuador vs Mexico)
console.log('\n[ET DATE CHECK — wc26-r32-079 Ecuador @ Mexico]');
const r079 = rows.find(r => r.fixture_id === 'wc26-r32-079');
