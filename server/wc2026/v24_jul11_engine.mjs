/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  WC2026 v24.0 — 500X BACKTEST + CORRECT SCORE + RECALIBRATION ENGINE       ║
 * ║  Date: 2026-07-11                                                           ║
 * ║  Scope: Jul 11 QF Matches (099: NOR vs ENG, 100: ARG vs SUI)               ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  PIPELINE (identical to v23 Jul 9 run):                                    ║
 ...
 * ║    6. Model Jul 11 QF matches (NOR vs ENG, ARG vs SUI) with final engine   ║
 * ║    7. Full audit + DB write                                                 ║
 * ║                                                                              ║
 * ║  BOOK ODDS SOURCE: BetExplorer bet365 (scraped 2026-07-11 via CI probe)    ║
 ...
 * ║  LOG: ./wc2026modeling_jul11.txt (APPEND — also to stdout for Railway logs)║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
...
const LOG_FILE = './wc2026modeling_jul11.txt';
const SESSION_ID = `v24-JUL11-500X-BACKTEST-RECAL-${Date.now()}`;
const START_TS = Date.now();
const ENGINE_VERSION = 'v24.0-500X-CORRECT-SCORE-RECALIBRATED-QF-JUL11';
...
// July 11 QF matches to project. Each carries the metadata the Phase-7 write
// needs so nothing is hardcoded to a single fixture: espnId (xG join / row),
// beId+beSlug (BetExplorer event id + slug written to bet_explorer_* columns),
...
const JUL11_BOOK = {
  ...
  'wc26-qf-100': {   // ARG (home) vs SUI (away) — ARG home favorite
    bookHomeMl: -149, bookDraw: 260, bookAwayMl: 475,
    bookSpread: -1.5, bookTotal: 2.5,
...
// ══════════════════════════════════════════════════════════════════════════════
async function main() {
  fs.appendFileSync(LOG_FILE, `\n${'═'.repeat(100)}\n  v24 JUL11 ENGINE SESSION: ${SESSION_ID}\n  STARTED: ${new Date().toISOString()}\n${'═'.repeat(100)}\n`);
  ...
  for (const match of PROJECTION_MATCHES) {
    log('PROJ', `${'━'.repeat(80)}`);
    log('PROJ', `${match.fid}: ${match.home} vs ${match.away} (QF — Jul 11, 2026)`);
    log('PROJ', `${'━'.repeat(80)}`);
    ...
    const book = JUL11_BOOK[match.fid];
  ...
  for (const proj of DRY_RUN ? [] : projections) {
    ...
    // The QF fixture may still carry 'tbd' team slots (bracket scraper lag) —
    // repair to the confirmed R16 winners before writing odds. On the live feed
    // these are already resolved (nor/eng, arg/sui), so this is a no-op safety
    // net; it only writes when a slot is still 'tbd'.
  ...
  const [auditRows] = await conn.query(`
    ...
    FROM wc2026MatchOdds WHERE match_id IN ('wc26-qf-099', 'wc26-qf-100')
  `);
