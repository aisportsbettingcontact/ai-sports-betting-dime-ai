/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  WC2026 v23.0 — 500X BACKTEST + CORRECT SCORE + RECALIBRATION ENGINE       ║
 * ║  Date: 2026-07-09                                                           ║
 * ║  Scope: Jul 9 QF Match (097: FRA vs MAR)                                   ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  PIPELINE (identical to v22 Jul 7 run):                                    ║
 ...
 * ║    6. Model Jul 9 QF match (FRA vs MAR) with final engine                  ║
 * ║    7. Full audit + DB write                                                 ║
 * ║                                                                              ║
 * ║  BOOK ODDS SOURCE: BetExplorer bet365 (scraped 2026-07-09 via CI probe)    ║
 * ║  LOG: ./wc2026modeling_jul9.txt (APPEND — uploaded as Actions artifact)    ║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
...
const LOG_FILE = './wc2026modeling_jul9.txt';
const SESSION_ID = `v23-JUL9-500X-BACKTEST-RECAL-${Date.now()}`;
const START_TS = Date.now();
const ENGINE_VERSION = 'v23.0-500X-CORRECT-SCORE-RECALIBRATED-QF-JUL9';
...
// July 9 QF match to project
const PROJECTION_MATCHES = [
  { fid:'wc26-qf-097', home:'FRA', away:'MAR', espnId:'760510' },
];

// FRESH bet365 book odds (scraped 2026-07-09 13:51 UTC from BetExplorer AJAX
// via CI probe — event jJ5yWPhU, slug france-morocco, bid=549 bet365.us; the
// intl bid=16 rows were absent on the US-served page, same fallback priority
// as the v4 scraper spec). Advance (to-qualify) odds are not offered on
// BetExplorer — bookHomeAdv/bookAwayAdv come from the DB probe of existing
// wc2026MatchOdds columns; when null the engine preserves whatever is already
// in the DB (COALESCE) instead of overwriting with NULL.
const JUL9_BOOK = {
  'wc26-qf-097': {
    bookHomeMl: -175, bookDraw: 290, bookAwayMl: 525,
    bookSpread: -1.5, bookTotal: 2.5,
    bookOver: 103, bookUnder: -120,
    bookBttsY: 100, bookBttsN: -133,
    bookHomeAdv: null, bookAwayAdv: null,                 // ← filled from DB probe
    bookHomeWD: -769, bookAwayWD: 130, bookNoDraw: -400,
    bookHomeSpreadOdds: 160, bookAwaySpreadOdds: -208,
  },
};
...
// ══════════════════════════════════════════════════════════════════════════════
async function main() {
  fs.appendFileSync(LOG_FILE, `\n${'═'.repeat(100)}\n  v23 JUL9 ENGINE SESSION: ${SESSION_ID}\n  STARTED: ${new Date().toISOString()}\n${'═'.repeat(100)}\n`);
  banner(`WC2026 ${ENGINE_VERSION} — 500X BACKTEST → CORRECT SCORE → RECALIBRATION → REINFORCEMENT → JUL 9 QF PROJECTION`);
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PHASE 6: MODEL JULY 9 QF MATCH
  // ══════════════════════════════════════════════════════════════════════════
  banner('PHASE 6/7 — MODEL JULY 9 QF MATCH (FINAL CALIBRATED ENGINE)');
  const projections = [];
  
  for (const match of PROJECTION_MATCHES) {
    log('PROJ', `${'━'.repeat(80)}`);
    log('PROJ', `${match.fid}: ${match.home} vs ${match.away} (QF — Jul 9, 2026)`);
    log('PROJ', `${'━'.repeat(80)}`);
    ...
    const book = JUL9_BOOK[match.fid];
