/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  WC2026 v25.0 — 500X BACKTEST + CORRECT SCORE + RECALIBRATION ENGINE       ║
 * ║  Date: 2026-07-14                                                           ║
 * ║  Scope: Jul 14 SF Match (101: FRA vs ESP)                                  ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  PIPELINE (identical to v24 Jul 11 run):                                   ║
 ...
 * ║    6. Model Jul 14 SF match (FRA vs ESP) with final engine                 ║
 * ║    7. Full audit + DB write                                                 ║
 * ║                                                                              ║
 * ║  ORIENTATION (advancement-map verified): SF-101 (ESPN 760514) home =       ║
 * ║    W(M97)=FRA (won QF-097 FRA v MAR), away = W(M98)=ESP (won QF-098).       ║
 * ║    Matches BetExplorer france-spain (FRA home) + to-advance FRA -150.       ║
 * ║    ESPN URL slug 'spain-france' is text ordering only; canonical home/away  ║
 * ║    comes from wc2026_matches (asserted live before any write).             ║
 * ║                                                                              ║
 * ║  BOOK ODDS SOURCE: BetExplorer bet365 (scraped 2026-07-14 via CI probe)    ║
 ...
 * ║  LOG: ./wc2026modeling_jul14.txt (APPEND — also to stdout for Railway logs)║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
...
const LOG_FILE = './wc2026modeling_jul14.txt';
const SESSION_ID = `v25-JUL14-500X-BACKTEST-RECAL-${Date.now()}`;
const START_TS = Date.now();
const ENGINE_VERSION = 'v25.0-500X-CORRECT-SCORE-RECALIBRATED-SF-JUL14';
...
// July 14 SF match to project. Carries the metadata the Phase-7 write needs so
...
//   760503 (M89 W = FRA) → 760510 (M97) home → 760514 (SF-101) HOME  = France
//   760506 (M93 W = ESP) → 760511 (M98) home → 760514 (SF-101) AWAY  = Spain
// BetExplorer 'france-spain' (FRA home) and to-advance FRA -150 both agree. The
// ESPN URL slug 'spain-france' is text ordering only. A live wc2026_matches
// orientation assertion (Phase 6) hard-fails if the DB disagrees, so a flip can
// never be silently written.
const PROJECTION_MATCHES = [
  // seedDatePT = the PACIFIC-TIME calendar day of kickoff (the app's seeding-day
  // rule); the kickoff TIME is displayed in ET. SF-101 is July 14 in PT. The
  // match_date repair only fires if the seeded DATE differs from seedDatePT, so
  // this is a safe no-op when the bracket already bucketed it on Jul 14.
  ...
  { fid:'wc26-sf-101', home:'FRA', away:'ESP', espnId:'760514',
    beId:'pU0PQ9nR', beSlug:'france-spain', espnSlug:'spain-france',
    homeId:'fra', awayId:'esp', venueCityLike:null, seedDatePT:'2026-07-14' },
];

// FRESH bet365 book odds for SF-101 (FRA home vs ESP away). The six BetExplorer
// markets (1x2, DC, BTTS, OU, AH) are filled from the CI probe
// wc-jul14-probe.yml, which reuses the production scraper's own
// parse_*/_select_primary_* functions against event pU0PQ9nR (slug france-spain).
// Advance (to-qualify) odds are NOT offered on BetExplorer, so bookHomeAdv/
// bookAwayAdv are the owner-provided "To Advance" market — FRA -150 (home),
// ESP +120 (away) — hardcoded below. The model computes its own to-advance odds
...
// ══════════════════════════════════════════════════════════════════════════════
async function main() {
  fs.appendFileSync(LOG_FILE, `\n${'═'.repeat(100)}\n  v25 JUL14 ENGINE SESSION: ${SESSION_ID}\n  STARTED: ${new Date().toISOString()}\n${'═'.repeat(100)}\n`);
  banner(`WC2026 ${ENGINE_VERSION} — 500X BACKTEST → CORRECT SCORE → RECALIBRATION → REINFORCEMENT → JUL 14 SF PROJECTION`);
  ...
  log('INPUT', `Book source: BetExplorer bet365 (scraped 2026-07-14 via CI odds probe wc-jul14-probe.yml)`);
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PHASE 6: MODEL JULY 14 SF MATCH
  // ══════════════════════════════════════════════════════════════════════════
  banner('PHASE 6/7 — MODEL JULY 14 SF MATCH (FINAL CALIBRATED ENGINE)');
  const projections = [];

  for (const match of PROJECTION_MATCHES) {
    log('PROJ', `${'━'.repeat(80)}`);
    log('PROJ', `${match.fid}: ${match.home} vs ${match.away} (SF — Jul 14, 2026)`);
    log('PROJ', `${'━'.repeat(80)}`);
    ...
    const book = JUL14_BOOK[match.fid];

    // ── BOOK COMPLETENESS GUARD — refuse to run on an unfilled placeholder book ──
    // The six BetExplorer markets are hand-filled from the wc-jul14-probe.yml
    ...
    // the engine can never model/publish against placeholder (missing) book lines.
    {
      ...
      if (missing.length) hardFail(`${match.fid}: JUL14_BOOK not filled from probe — null: ${missing.join(', ')}. Run wc-jul14-probe.yml and paste the six BetExplorer markets before modeling.`);
      if (book.bookHomeAdv == null || book.bookAwayAdv == null) hardFail(`${match.fid}: to-advance book lines missing (expected owner-provided FRA -150 / ESP +120)`);
    }

    // ── LIVE ORIENTATION ASSERTION (home/away flip tripwire vs the canonical row) ─
    // The whole pipeline binds odds POSITIONALLY. The intended orientation (FRA
    // home, ESP away) is advancement-map verified, but assert it against the LIVE
    ...
    // written. 'tbd'/empty slots are allowed (Phase 7 repairs them to homeId/awayId).
    {
      ...
      if (h && h !== 'tbd' && h !== match.homeId)
        hardFail(`${match.fid}: LIVE home_team_id='${h}' disagrees with intended home '${match.homeId}' (FRA) — orientation flip; refusing to model/write`);
      if (a && a !== 'tbd' && a !== match.awayId)
        hardFail(`${match.fid}: LIVE away_team_id='${a}' disagrees with intended away '${match.awayId}' (ESP) — orientation flip; refusing to model/write`);
  ...
  for (const proj of DRY_RUN ? [] : projections) {
    ...
    // repair to the confirmed QF winners (fra/esp) before writing odds. On the
    // live feed these are already resolved, and the Phase-6 orientation assertion
    // has already proven any non-tbd slots agree with fra/esp, so this is a no-op
    ...
    // Venue repair is DISABLED for SF-101 (venueCityLike=null): the seeded venue
    ...
    // shown in ET). SF-101 is a Jul-14 match; if a late kickoff pushed its UTC day
    // to Jul 15 it would mis-bucket off the Jul-14 feed. Repair match_date to the
    // PT day (seedDatePT=2026-07-14) WITHOUT touching the (correct) kickoff_utc.
    // The guard only fires when the seeded DATE differs, so it is a no-op if the
    // bracket already bucketed SF-101 on Jul 14.
    if (match.seedDatePT) {
  ...
  const [auditRows] = await conn.query(`
    ...
    FROM wc2026MatchOdds WHERE match_id IN ('wc26-sf-101')
  `);
