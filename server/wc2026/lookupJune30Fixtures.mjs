...
const fids = rows.map(r => r.match_id);
if (fids.length > 0) {
  const ph = fids.map(() => '?').join(',');
  const [mp] = await db.execute(`SELECT match_id, model_version, is_frozen FROM wc2026_model_projections WHERE match_id IN (${ph})`, fids);
  const [bo] = await db.execute(`SELECT match_id, book_source, book_home_ml FROM wc2026_frozen_book_odds WHERE match_id IN (${ph})`, fids);
