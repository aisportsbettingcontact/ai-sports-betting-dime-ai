...
function main() {
  ...
  const p080 = reportData.projections.find(x => x.fixtureId === 'wc26-r32-080');
  ...
  function prob2ml_STRICT(p, label, fixtureId) {
    const pNum = Number(p);
    if (isNaN(pNum)) {
      L.fail('PROB2ML', `${fixtureId} ${label}: p=${p} is NaN`);
      return null;
    }
    if (pNum <= 0 || pNum >= 1) {
      L.fail('PROB2ML', `${fixtureId} ${label}: p=${pNum} out of range (0,1)`);
      return null;
    }
    if (pNum < 0.001 || pNum > 0.999) {
      L.warn('PROB2ML', `${fixtureId} ${label}: p=${pNum} near boundary — ML may be extreme`);
    }
    let ml;
    if (pNum >= 0.5) {
      ml = -(pNum / (1 - pNum)) * 100;
    ...
    if (roundTripErr > 0.005) {
      L.warn('PROB2ML', `${fixtureId} ${label}: round-trip error ${roundTripErr.toFixed(6)} (p=${pNum.toFixed(6)} → ML=${mlRounded} → p_back=${pBack.toFixed(6)})`);
    } else {
      L.atomic('PROB2ML', `${fixtureId} ${label}: P=${pNum.toFixed(6)} → ML=${mlRounded>0?'+':''}${mlRounded} | round-trip err=${roundTripErr.toFixed(6)} ✓`);
    }
    return mlRounded;
  }
  ...
  for (const f of FIXTURES) {
    ...
    const proj = reportData.projections.find(x => x.fixtureId === f.id);
