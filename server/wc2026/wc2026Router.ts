...
import {
  scrapeEspnMatch,
  scrapeEspnScoreboard,
  extractGameId,
} from "./espnMatchScraper";
...
export const wc2026Router = router({
  ...
  matchesByDate: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      console.log(
        `[wc2026.matchesByDate] RESULT date='${input.date}' matches=${matches.length} ids=[${matches.map((f: WcMatch) => f.matchId).join(",")}]`
      );
      if (matches.length === 0) {
        console.log(
          `[wc2026.matchesByDate] EMPTY — no matches found for date='${input.date}'. Checking raw matchDate values...`
        );
        ...
        const sample = await db
          ...
          .limit(5);
        console.log(
          `[wc2026.matchesByDate] SAMPLE rows:`,
      ...
      const teamMap = Object.fromEntries(
        teams.map((t: WcTeam) => [t.teamId, t])
      );
      const venueMap = Object.fromEntries(
        venues.map((v: WcVenue) => [v.venueId, v])
      );
        ...
        for (const row of rows) {
          ...
          if (!seen.has(key)) {
            ...
            if (row.market === "1X2") {
              ...
              else if (row.selection === "no_draw")
                o["noDraw"] = row.americanOdds;
            } else if (row.market === "TOTAL") {
              if (row.selection === "over") {
                o["overLine"] =
                  row.line != null
                    ? parseFloat(row.line as unknown as string)
                    : undefined;
              ...
              if (row.selection === "home") {
                o["homeSpreadLine"] =
                  row.line != null
                    ? parseFloat(row.line as unknown as string)
                    : undefined;
                o["homeSpreadOdds"] = row.americanOdds;
              } else if (row.selection === "away") {
                o["awaySpreadLine"] =
                  row.line != null
                    ? parseFloat(row.line as unknown as string)
                    : undefined;
                o["awaySpreadOdds"] = row.americanOdds;
              }
            } else if (row.market === "DOUBLE_CHANCE") {
              if (row.selection === "home_draw")
                o["homeDrawOdds"] = row.americanOdds;
              else if (row.selection === "away_draw")
                o["awayDrawOdds"] = row.americanOdds;
      ...
      const [dkOddsRows, modelOddsRows, projRows, matchOddsRows] =
        await Promise.all([
          db
            .select()
            ...
            .where(
              and(
                ...
                inArray(wc2026OddsSnapshots.matchId, matchIds)
              )
            )
          ...
          db
            ...
            .where(
              and(
                eq(wc2026OddsSnapshots.bookId, 0),
                inArray(wc2026OddsSnapshots.matchId, matchIds)
              )
            )
            .orderBy(desc(wc2026OddsSnapshots.snapshotTs)),
      ...
      const projMap = Object.fromEntries(
        (projRows as (typeof wc2026ModelProjections.$inferSelect)[]).map(p => [
          p.matchId,
          p,
        ])
      );
      ...
      const matchOddsToBookOdds = (
        ...
        awaySpreadLine:
          r.bookPrimarySpread != null ? -r.bookPrimarySpread : undefined,
      ...
      const wcMapped = matches.map((f: WcMatch) => {
        ...
        return {
          ...
          dkOdds: matchOddsMap[f.matchId]
            ? matchOddsToBookOdds(matchOddsMap[f.matchId]!)
            : (dkMap[f.matchId] ?? null),
          // [v8.0] Use wc2026_model_projections when available, otherwise fall back to book_id=0 snapshot
          modelOdds: proj
            ? projToModelOdds(proj)
            : (modelMap[f.matchId] ?? null),
          projection: proj,
  ...
  latestLineups: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db
        ...
        .orderBy(
          wc2026Lineups.teamId,
          wc2026Lineups.isStarter,
          wc2026Lineups.position
        );
    }),
  ...
  todayWithOdds: publicProcedure.query(async ({ ctx }) => {
    ...
    console.log(
      `[wc2026.todayWithOdds] utcHour=${nowUtc.getUTCHours()} isBeforeCutoff=${isBeforeCutoff} effectiveDate=${today}`
    );
    ...
    // [v8.0 2026-07-02] Book odds now sourced from wc2026MatchOdds (replaces wc2026FrozenBookOdds).
    const [dkOddsRowsT, modelOddsRowsT, projRowsT, matchOddsRowsT] =
      await Promise.all([
        db
          .select()
          .from(wc2026OddsSnapshots)
    ...
    const projMapT = Object.fromEntries(
      (projRowsT as (typeof wc2026ModelProjections.$inferSelect)[]).map(p => [
        p.matchId,
        p,
      ])
    );
    ...
    const twoMapped = matches.map((f: WcMatch) => {
      ...
      return {
        ...
        venue: venueMap[f.venueId] ?? null,
        // [v8.0] Use wc2026MatchOdds book odds when available, otherwise fall back to live DK snapshot
        dkOdds: matchOddsMapT[f.matchId]
          ? matchOddsToBookOddsT(matchOddsMapT[f.matchId]!)
          : (dkMapT[f.matchId] ?? null),
        // [v8.0] Use wc2026_model_projections when available, otherwise fall back to book_id=0 snapshot
        modelOdds: proj
          ? projToModelOddsT(proj)
          : (modelMapT[f.matchId] ?? null),
        projection: proj,
        modelVersion: proj?.modelVersion ?? null,
        isFrozen: proj?.isFrozen ?? false,
        frozenAt: proj?.frozenAt ?? null,
  ...
  espnMatch: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      console.log(
        `[ESPN] espnMatch called | gameId=${gameId} | includePlayerStats=${includePlayerStats} | includeCommentary=${includeCommentary}`
      );
      ...
      try {
        ...
        console.log(
          `[ESPN] espnMatch complete | gameId=${gameId} | elapsed=${elapsed}ms | apiCalls=${data.apiCallCount} | errors=${data.errors.length} | logFile=${data.logFile}`
        );
        ...
        console.error(
          `[ESPN] espnMatch FAILED | gameId=${gameId} | elapsed=${elapsed}ms | error=${errMsg}`
        );
  ...
  espnScoreboard: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        const elapsed = Date.now() - t0;
        console.log(
          `[ESPN] espnScoreboard complete | date=${dateParam} | events=${events.length} | elapsed=${elapsed}ms`
        );
        ...
        const errMsg = err instanceof Error ? err.message : String(err);
        console.error(
          `[ESPN] espnScoreboard FAILED | date=${dateParam} | error=${errMsg}`
        );
  ...
  espnMatchPage: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const gameId = gameIdMatch
        ? gameIdMatch[1]
        : urlOrGameId.replace(/\D/g, "");
      console.log(
        `[ESPN_PAGE] espnMatchPage called | gameId=${gameId} | saveHtml=${saveHtml}`
      );
      const t0 = Date.now();
      try {
        ...
        const errMsg = err instanceof Error ? err.message : String(err);
        console.error(
          `[ESPN_PAGE] espnMatchPage FAILED | gameId=${gameId} | elapsed=${elapsed}ms | error=${errMsg}`
        );
        return { success: false as const, data: null, error: errMsg };
      }
    }),
  ...
  espnIngest: ownerProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const gameId = gameIdMatch
        ...
        : urlOrGameId.replace(/\D/g, "");
      console.log(
        `[ESPN_INGEST] espnIngest called | gameId=${gameId} | dryRun=${dryRun}`
      );
      const t0 = Date.now();
      try {
        ...
        const errMsg = err instanceof Error ? err.message : String(err);
        console.error(
          `[ESPN_INGEST] FAILED | gameId=${gameId} | elapsed=${elapsed}ms | error=${errMsg}`
        );
  ...
  listMatchOdds: ownerProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      console.log(
        `[WC2026_MATCH_ODDS] listMatchOdds START | userId=${userId} username=${username} round=${round}`
      );

      const db = await getDb();
      if (!db) {
        console.error(
          `[WC2026_MATCH_ODDS] listMatchOdds FATAL | DB unavailable | userId=${userId}`
        );
        throw new Error("[WC2026_MATCH_ODDS] Database connection unavailable");
      }
      console.log(
        `[WC2026_MATCH_ODDS] listMatchOdds DB_READY | querying wc2026MatchOdds WHERE world_cup_round='${round}'`
      );
      ...
      if (espnMatchIds.length > 0) {
        ...
        teamNameMap = Object.fromEntries(
          espnRows.map((r: (typeof espnRows)[0]) => [r.matchId, r])
        );
        console.log(
          `[WC2026_MATCH_ODDS] listMatchOdds ENRICH | espnMatchIds=${espnMatchIds.length} resolved=${espnRows.length}`
        );
      }
      ...
      if (invalid.length > 0) {
        console.error(
          `[WC2026_MATCH_ODDS] listMatchOdds INTEGRITY_WARN | ${invalid.length} rows missing match_id`
        );
      }

      // Audit: count rows with all core book_ columns populated
      const fullyPopulated = rows.filter(
        (r: Wc2026MatchOddsRow) =>
          r.bookAwayMl !== null &&
          r.bookHomeMl !== null &&
          r.bookDraw !== null &&
          r.bookPrimarySpread !== null &&
          r.bookTotal !== null &&
          r.bookBttsYes !== null &&
          r.bookBttsNo !== null
      );
      ...
      const enrichedRows = rows.map((r: Wc2026MatchOddsRow) => ({
        ...
        awayTeamAbbrev:
          teamNameMap[r.espnMatchId ?? ""]?.awayTeamAbbrev ?? null,
        homeTeamAbbrev:
          teamNameMap[r.espnMatchId ?? ""]?.homeTeamAbbrev ?? null,
      ...
      return {
        ...
        meta: {
          total: rows.length,
          round,
          fullyPopulated: fullyPopulated.length,
          elapsedMs: elapsed,
        },
      };
    }),
  ...
  updateMatchOdds: ownerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db) {
        console.error(
          `[WC2026_MATCH_ODDS] updateMatchOdds FATAL | DB unavailable | userId=${userId} matchId=${matchId}`
        );
        throw new Error("[WC2026_MATCH_ODDS] Database connection unavailable");
      }
      ...
      if (fields.modelAwayToAdvance !== undefined)
        payload.modelAwayToAdvance = fields.modelAwayToAdvance;
      if (fields.modelHomeToAdvance !== undefined)
        payload.modelHomeToAdvance = fields.modelHomeToAdvance;
      if (fields.modelAwayMl !== undefined)
        payload.modelAwayMl = fields.modelAwayMl;
      if (fields.modelHomeMl !== undefined)
        payload.modelHomeMl = fields.modelHomeMl;
      if (fields.modelDraw !== undefined) payload.modelDraw = fields.modelDraw;
      if (fields.modelNoDraw !== undefined)
        payload.modelNoDraw = fields.modelNoDraw;
      if (fields.modelAwayWd !== undefined)
        payload.modelAwayWd = fields.modelAwayWd;
      if (fields.modelHomeWd !== undefined)
        payload.modelHomeWd = fields.modelHomeWd;
      if (fields.modelPrimarySpread !== undefined)
        payload.modelPrimarySpread = fields.modelPrimarySpread;
      if (fields.modelAwayPrimarySpreadOdds !== undefined)
        payload.modelAwayPrimarySpreadOdds = fields.modelAwayPrimarySpreadOdds;
      if (fields.modelHomePrimarySpreadOdds !== undefined)
        payload.modelHomePrimarySpreadOdds = fields.modelHomePrimarySpreadOdds;
      if (fields.modelTotal !== undefined)
        payload.modelTotal = fields.modelTotal;
      if (fields.modelOverOdds !== undefined)
        payload.modelOverOdds = fields.modelOverOdds;
      if (fields.modelUnderOdds !== undefined)
        payload.modelUnderOdds = fields.modelUnderOdds;
      if (fields.modelBttsYes !== undefined)
        payload.modelBttsYes = fields.modelBttsYes;
      if (fields.modelBttsNo !== undefined)
        payload.modelBttsNo = fields.modelBttsNo;

      if (Object.keys(payload).length === 0) {
        console.warn(
          `[WC2026_MATCH_ODDS] updateMatchOdds NOOP | matchId=${matchId} — no fields to update`
        );
      ...
      if (affectedRows === 0) {
        console.warn(
          `[WC2026_MATCH_ODDS] updateMatchOdds WARN | matchId=${matchId} — 0 rows affected (match may not exist)`
        );
      }
