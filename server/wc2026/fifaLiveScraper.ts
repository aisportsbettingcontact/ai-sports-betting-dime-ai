...
function log(
  ...
  console.log(
    `${prefix} ${icon} ${msg}${detail ? `\n${" ".repeat(55)}↳ ${detail}` : ""}`
  );
}

const FIFA_API_URL =
  "https://api.fifa.com/api/v3/live/football?language=en&count=100";
const FIFA_HEADERS = {
  "User-Agent":
...
type DbStatus =
  "SCHEDULED" | "LIVE" | "HT" | "ET" | "SHOOTOUT" | "FT" | "FT_PEN";
...
function resolveStatus(
...
): { status: DbStatus; minute: string | null } {
  ...
  if (matchStatus === 3) {
    switch (period) {
      ...
      case 3:
        return { status: "ET", minute: normalizeMatchTime(matchTime) };
      case 4:
        return { status: "ET", minute: normalizeMatchTime(matchTime) };
      case 11:
        return { status: "SHOOTOUT", minute: "PENS" };
      default:
...
async function fetchFifaLiveMatches(): Promise<FifaMatchState[]> {
  ...
  if (!resp.ok)
    throw new Error(`FIFA API HTTP ${resp.status}: ${resp.statusText}`);
  ...
  return matches.map(m => {
    const { status, minute } = resolveStatus(
      m.MatchStatus,
      m.Period ?? null,
      m.MatchTime ?? null
    );
...
export async function wc2026LiveSyncHandler(
  req: Request,
  res: Response
): Promise<void> {
  ...
  try {
    ...
    const allMatches = await db
      ...
      .from(wc2026Matches)
      .where(isNotNull(wc2026Matches.fifaMatchId));
    ...
    const fifaToMatch = new Map<string, DbMatch>(
      allMatches.map((f: DbMatch) => [f.fifaMatchId!, f] as [string, DbMatch])
    );
    ...
    try {
      ...
      notifyOwner({
        title: "[HB] fifa-live-sync FAIL",
        content: `FIFA API fetch: ${msg}`.slice(0, 500),
      });
    ...
    log(
      "STATE",
      S(),
      `${relevant.length}/${liveMatches.length} live matches match our tracked matches`
    );
    for (const m of relevant) {
      log(
        "AUDIT",
        S(),
        `fifaId=${m.fifaMatchId} rawStatus=${m.rawMatchStatus} period=${m.rawPeriod} → dbStatus=${m.status} minute=${m.minute ?? "null"} score=${m.homeScore ?? "?"}-${m.awayScore ?? "?"}`
      );
    }
    ...
    for (const liveMatch of relevant) {
      ...
      if (
        ...
        patch.homeScore = liveMatch.homeScore;
      if (
        liveMatch.awayScore !== null &&
        liveMatch.awayScore !== match.awayScore
      )
        patch.awayScore = liveMatch.awayScore;
      if (liveMatch.minute !== match.matchMinute)
        patch.matchMinute = liveMatch.minute;

      // Auto-set advancingTeamId when FT or FT_PEN
      // Also upgrade status to FT_PEN if penalty scores are present
      if (
        (liveMatch.status === "FT" || liveMatch.status === "FT_PEN") &&
        ...
        if (
          ...
          liveMatch.homePenScore === null &&
          liveMatch.awayPenScore === null
        ) {
          patch.advancingTeamId =
            liveMatch.homeScore > liveMatch.awayScore
              ? match.homeTeamId
              : match.awayTeamId;
          log(
            "DB",
            S(),
            `FT winner (score): ${match.matchId} → advancingTeamId=${patch.advancingTeamId}`
          );
        }
        // Case 2: Penalty shootout winner via FIFA Winner field
          ...
          log(
            "DB",
            S(),
            `FT_PEN winner (pens) ${penStr}: ${match.matchId} → advancingTeamId=${patch.advancingTeamId ?? "unknown"}`
          );
        }
        // Case 3: Penalty shootout winner via penalty scores
          ...
          log(
            "DB",
            S(),
            `FT_PEN winner (pen scores ${liveMatch.homePenScore}-${liveMatch.awayPenScore}): ${match.matchId} → advancingTeamId=${patch.advancingTeamId}`
          );
        }
      }
      ...
      try {
        await db
          ...
          .set(patch)
          .where(eq(wc2026Matches.matchId, match.matchId));
    ...
    log(
      "OUTPUT",
      S(),
      `Live sync complete in ${elapsedMs}ms`,
      `updated=${updatedCount} skipped=${skippedCount} fail=${failCount} warn=${warnCount}`
    );
    res.json({
      ok: true,
