/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  WC2026 v22.0 — 500X BACKTEST + CORRECT SCORE + RECALIBRATION ENGINE       ║
 * ║  Date: 2026-07-07                                                           ║
 * ║  Scope: Jul 7 R16 Matches (095: ARG vs EGY, 096: SUI vs COL)              ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  PIPELINE:                                                                  ║
 * ║    1. Pull ESPN data (xG, team stats, match stats, player stats, shot map) ║
 * ║    2. 500X Monte Carlo backtest across 25 variations (22 matches)          ║
 ...
 * ║    6. Model Jul 7 matches (ARG vs EGY, SUI vs COL) with final engine      ║
 * ║    7. Full audit + DB write                                                 ║
 * ║                                                                              ║
 * ║  BOOK ODDS SOURCE: BetExplorer bet365 (scraped 2026-07-07 ~16:30 UTC)     ║
...
const SESSION_ID = `v22-JUL7-500X-BACKTEST-RECAL-${Date.now()}`;
const START_TS = Date.now();
const ENGINE_VERSION = 'v22.0-500X-CORRECT-SCORE-RECALIBRATED-R16-JUL7';
...
// July 7 R16 matches to project
const PROJECTION_MATCHES = [
  { fid:'wc26-r16-095', home:'ARG', away:'EGY', espnId:'760509' },
  { fid:'wc26-r16-096', home:'SUI', away:'COL', espnId:'760508' },
];

// FRESH bet365 book odds (scraped 2026-07-07 ~16:30 UTC from BetExplorer AJAX)
const JUL7_BOOK = {
  'wc26-r16-095': {
    bookHomeMl: -303, bookDraw: 375, bookAwayMl: 1000,
    bookSpread: -1.5, bookTotal: 2.5,
    bookOver: -102, bookUnder: -114,
    bookBttsY: 150, bookBttsN: -200,
    bookHomeAdv: -900, bookAwayAdv: 550,
    bookHomeWD: -2000, bookAwayWD: 225, bookNoDraw: -556,
    bookHomeSpreadOdds: 103, bookAwaySpreadOdds: -120,
  },
  'wc26-r16-096': {
    bookHomeMl: 250, bookDraw: 210, bookAwayMl: 125,
    bookSpread: 0.5, bookTotal: 2.5,
    bookOver: 130, bookUnder: -161,
    bookBttsY: -105, bookBttsN: -125,
    bookHomeAdv: 125, bookAwayAdv: -155,
    bookHomeWD: -161, bookAwayWD: -345, bookNoDraw: -278,
    bookHomeSpreadOdds: -154, bookAwaySpreadOdds: 120,
  },
};
...
// ══════════════════════════════════════════════════════════════════════════════
async function main() {
  fs.appendFileSync(LOG_FILE, `\n${'═'.repeat(100)}\n  v22 JUL7 ENGINE SESSION: ${SESSION_ID}\n  STARTED: ${new Date().toISOString()}\n${'═'.repeat(100)}\n`);
  banner(`WC2026 ${ENGINE_VERSION} — 500X BACKTEST → CORRECT SCORE → RECALIBRATION → REINFORCEMENT → JUL 7 PROJECTIONS`);
  ...
  log('INPUT', `Book source: BetExplorer bet365 (scraped 2026-07-07 ~16:30 UTC via cloud scraper)`);
  ...
  for (const match of PROJECTION_MATCHES) {
    log('PROJ', `${'━'.repeat(80)}`);
    log('PROJ', `${match.fid}: ${match.home} vs ${match.away} (R16 — Jul 7, 2026)`);
    log('PROJ', `${'━'.repeat(80)}`);
    ...
    const book = JUL7_BOOK[match.fid];
  ...
  for (const proj of projections) {
    ...
    await conn.query(`
      UPDATE wc2026MatchOdds SET
        ...
        insert_method = ?, last_inserted_at = NOW(), last_insert_method = ?, odds_source = 'betexplorer+draftkings_manual_advance'
      WHERE match_id = ?
    `, [
  ...
  const [auditRows] = await conn.query(`
    ...
    FROM wc2026MatchOdds WHERE match_id IN ('wc26-r16-095', 'wc26-r16-096')
  `);
