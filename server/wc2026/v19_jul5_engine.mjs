 ...
 * ║  Date: 2026-07-05                                                           ║
 * ║  Scope: Jul 5 R16 Matches (091: BRA vs NOR, 092: MEX vs ENG)              ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  PIPELINE:                                                                  ║
 ...
 * ║    6. Model both Jul 5 matches with final calibrated engine                ║
 * ║    7. Full audit + DB write                                                 ║
 * ║                                                                              ║
 * ║  BOOK ODDS SOURCE: BetExplorer bet365 (scraped 2026-07-05 19:16 UTC)       ║
...
const SESSION_ID = `v19-JUL5-500X-BACKTEST-RECAL-${Date.now()}`;
const START_TS = Date.now();
const ENGINE_VERSION = 'v19.0-500X-CORRECT-SCORE-RECALIBRATED-R16-JUL5';
...
// July 5 R16 matches to project
const PROJECTION_MATCHES = [
  { fid:'wc26-r16-091', home:'BRA', away:'NOR', espnId:'760504' },
  { fid:'wc26-r16-092', home:'MEX', away:'ENG', espnId:'760505' },
];

// FRESH bet365 book odds (scraped 2026-07-05 19:16 UTC from BetExplorer AJAX)
const JUL5_BOOK = {
  'wc26-r16-091': {
    bookHomeMl: -133, bookDraw: 270, bookAwayMl: 375,
    bookSpread: -1.5, bookTotal: 2.5,
    bookOver: -137, bookUnder: 110,
    bookBttsY: -149, bookBttsN: 110,
    bookHomeAdv: -333, bookAwayAdv: 240,
    bookHomeWD: -556, bookAwayWD: 100, bookNoDraw: -345,
    bookHomeSpreadOdds: 200, bookAwaySpreadOdds: -263,
  },
  'wc26-r16-092': {
    bookHomeMl: 200, bookDraw: 220, bookAwayMl: 145,
    bookSpread: -0.5, bookTotal: 2.5,
    bookOver: 138, bookUnder: -175,
    bookBttsY: 100, bookBttsN: -133,
    bookHomeAdv: 100, bookAwayAdv: -137,
    bookHomeWD: -189, bookAwayWD: -278, bookNoDraw: -278,
    bookHomeSpreadOdds: 108, bookAwaySpreadOdds: -137,
  },
};
...
async function main() {
  ...
  for (const match of PROJECTION_MATCHES) {
    ...
    const book = JUL5_BOOK[match.fid];
  ...
  const [auditRows] = await conn.query(`
    ...
    FROM wc2026MatchOdds WHERE match_id IN ('wc26-r16-091','wc26-r16-092')
  `);
