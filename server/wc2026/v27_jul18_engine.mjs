/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  WC2026 v27.0 — 500X BACKTEST + CORRECT SCORE + RECALIBRATION ENGINE       ║
 * ║  Date: 2026-07-18 (3rd place) + 2026-07-19 (Final)                          ║
 * ║  Scope: wc26-3rd-103 (FRA vs ENG) + wc26-final-104 (ESP vs ARG)            ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  PIPELINE (identical to v26 Jul 15 run):                                   ║
 * ║    1. Pull ESPN data (xG, team stats, match stats, player stats, shot map) ║
 * ║    2. 500X Monte Carlo backtest across 25 variations (28 matches)          ║
 ...
 * ║    6. Model 3rd-place + Final with final engine                            ║
 * ║    7. Full audit + DB write                                                 ║
 * ║                                                                              ║
 * ║  ORIENTATION (advancement-map + live wc2026_matches verified):             ║
 * ║    M103 home = L(M101) = FRA (lost SF-101 0-2 ESP), away = L(M102) = ENG   ║
 * ║    (lost SF-102 1-2 ARG). M104 home = W(M101) = ESP, away = W(M102) = ARG. ║
 * ║    BetExplorer france-england / spain-argentina (home first) agree; the    ║
 * ║    ESPN slugs 'england-france'/'argentina-spain' are text ordering only;   ║
 * ║    canonical home/away comes from wc2026_matches (asserted before write).  ║
 * ║                                                                              ║
 * ║  BOOK ODDS SOURCE: BetExplorer bet365 (scraped via CI probe wc-jul18)      ║
 ...
 * ║  LOG: ./wc2026modeling_jul18.txt (APPEND — also to stdout for Railway logs)║
 * ╚══════════════════════════════════════════════════════════════════════════════╝
 */
...
const LOG_FILE = './wc2026modeling_jul18.txt';
const SESSION_ID = `v27-JUL18-500X-BACKTEST-RECAL-${Date.now()}`;
const START_TS = Date.now();
const ENGINE_VERSION = 'v27.0-500X-CORRECT-SCORE-RECALIBRATED-3RD-FINAL-JUL18';
...
const BACKTEST_MATCHES = [
  ...
  // Jul 11 QF results (added for v27 backtest — scores from the live
  // wc2026_matches feed via CI probe; frozen books from v24's JUL11_BOOK):
  { fid:'wc26-qf-099', home:'NOR', away:'ENG', homeScore:1, awayScore:2 },
  // Jul 14-15 SF results (added for v27 backtest — both FT in 90 minutes;
  // frozen books from v25's JUL14_BOOK / v26's JUL15_BOOK):
...
// Jul 18/19 matches to project. Carries the metadata the Phase-7 write needs so
...
//
// ORIENTATION (advancement-map + live wc2026_matches verified 2026-07-17 via CI
// feed probe, wc2026BracketScraper ADVANCEMENT lines 214-217):
//   M103 (3rd place, ESPN 760516): home = L(M101) = FRA (lost SF-101 0-2 ESP),
//     away = L(M102) = ENG (lost SF-102 1-2 ARG). Live row: tbd/tbd (repaired
//     below). BetExplorer 'france-england' (FRA home) agrees.
//   M104 (Final, ESPN 760517): home = W(M101) = ESP, away = W(M102) = ARG.
//     Live row already esp/arg. BetExplorer 'spain-argentina' (ESP home) agrees.
// The ESPN URL slugs 'england-france'/'argentina-spain' are text ordering only.
// A live wc2026_matches orientation assertion (Phase 6) hard-fails if the DB
// disagrees, so a flip can never be silently written.
const PROJECTION_MATCHES = [
  // seedDatePT = the PACIFIC-TIME calendar day of kickoff (the app's seeding-day
  // rule); the kickoff TIME is displayed in ET. Live kickoffs (CI feed probe):
...
// (ESP home vs ARG away). The six BetExplorer markets (1x2, DC, BTTS, OU, AH)
// are filled from the CI probe wc-jul18-probe.yml, which reuses the production
// scraper's own parse_*/_select_primary_* functions against events b9l0F3Bj
// (france-england) and UgbUKPmT (spain-argentina).
//
// TO-ADVANCE: neither the 3rd-place match nor the Final has a "to advance"
// market (there is no next round) — bookHomeAdv/bookAwayAdv stay null and the
// Phase-6 guard treats null adv as expected for these two fids (WARN, not
// hard-fail; the wc2026MatchOdds audit already treats book_*_to_advance as
// WARN-only). The model still computes and writes model_*_to_advance from the
...
const BACKTEST_BOOK = {
  ...
  // Jul 11 QF (frozen from v24_jul11_engine.mjs JUL11_BOOK, probe-scraped pre-match)
  'wc26-qf-099': { bookHomeMl:300, bookDraw:270, bookAwayMl:-111, bookSpread:1.5, bookTotal:2.5, bookOver:-137, bookUnder:110, bookBttsY:-161, bookBttsN:120 },
  // qf-100 (ARG v SUI) book is frozen and ready, but the match has NO
  // BACKTEST_MATCHES entry: its exact final score was not verifiable from the
  // live feed at authoring time (zero-hallucination — never guess a result).
  // Add `{ fid:'wc26-qf-100', home:'ARG', away:'SUI', homeScore:X, awayScore:Y }`
  // once the score is confirmed and it joins the sample automatically.
  'wc26-qf-100': { bookHomeMl:-149, bookDraw:260, bookAwayMl:475, bookSpread:-1.5, bookTotal:2.5, bookOver:120, bookUnder:-149, bookBttsY:110, bookBttsN:-149 },
  // Jul 14-15 SF (frozen from v25 JUL14_BOOK / v26 JUL15_BOOK, probe-scraped pre-match)
  'wc26-sf-101': { bookHomeMl:135, bookDraw:220, bookAwayMl:220, bookSpread:-1.5, bookTotal:2.5, bookOver:-108, bookUnder:-108, bookBttsY:-149, bookBttsN:110 },
  'wc26-sf-102': { bookHomeMl:170, bookDraw:188, bookAwayMl:200, bookSpread:0, bookTotal:2.5, bookOver:130, bookUnder:-161, bookBttsY:-105, bookBttsN:-125 },
};
...
// ══════════════════════════════════════════════════════════════════════════════
async function main() {
  fs.appendFileSync(LOG_FILE, `\n${'═'.repeat(100)}\n  v27 JUL18 ENGINE SESSION: ${SESSION_ID}\n  STARTED: ${new Date().toISOString()}\n${'═'.repeat(100)}\n`);
  banner(`WC2026 ${ENGINE_VERSION} — 500X BACKTEST → CORRECT SCORE → RECALIBRATION → REINFORCEMENT → JUL 18/19 3RD-PLACE + FINAL PROJECTION`);
  ...
  log('INPUT', `Book source: BetExplorer bet365 (scraped via CI odds probe wc-jul18-probe.yml)`);
  ...
  // ══════════════════════════════════════════════════════════════════════════
  banner('PHASE 1/7 — PULL ESPN DATA (ALL PLAYED MATCHES FOR SAMPLED TEAMS)');
  ...
  // ══════════════════════════════════════════════════════════════════════════
  banner(`PHASE 2/7 — 500X BACKTEST (25 VARIATIONS × ${BACKTEST_MATCHES.length} MATCHES = ${25*BACKTEST_MATCHES.length} MODEL RUNS)`);
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PHASE 6: MODEL 3RD-PLACE + FINAL
  // ══════════════════════════════════════════════════════════════════════════
  banner('PHASE 6/7 — MODEL 3RD-PLACE + FINAL (FINAL CALIBRATED ENGINE)');
  const projections = [];

  for (const match of PROJECTION_MATCHES) {
    log('PROJ', `${'━'.repeat(80)}`);
    log('PROJ', `${match.fid}: ${match.home} vs ${match.away} (${ROUND_LABEL[match.fid]} — ${match.seedDatePT})`);
    log('PROJ', `${'━'.repeat(80)}`);
    ...
    const book = JUL18_BOOK[match.fid];

    // ── BOOK COMPLETENESS GUARD — refuse to run on an unfilled placeholder book ──
    // The six BetExplorer markets are hand-filled from the wc-jul18-probe.yml
    ...
    // TO-ADVANCE is intentionally exempt for these two fids: the 3rd-place match
    // and the Final have no to-advance market (no next round), so null adv is
    // expected — WARN only, and the book_*_to_advance columns stay NULL (the
    // Phase-7 audit already treats them as WARN-only).
    {
      ...
      if (missing.length) hardFail(`${match.fid}: JUL18_BOOK not filled from probe — null: ${missing.join(', ')}. Run wc-jul18-probe.yml and paste the six BetExplorer markets before modeling.`);
      if (book.bookHomeAdv == null || book.bookAwayAdv == null)
        log('WARN', `${match.fid}: no to-advance book lines (expected — no such market for ${ROUND_LABEL[match.fid]}); book_*_to_advance will be NULL, model advance odds still computed`);
    }

    // ── LIVE ORIENTATION ASSERTION (home/away flip tripwire vs the canonical row) ─
    // The whole pipeline binds odds POSITIONALLY. The intended orientations
    // (FRA home / ENG away for 3rd-103; ESP home / ARG away for final-104) are
    // advancement-map verified, but assert them against the LIVE wc2026_matches
    // row so a bracket-scraper disagreement can never be silently written.
    // 'tbd'/empty slots are allowed (Phase 7 repairs them to homeId/awayId).
    {
      ...
      if (h && h !== 'tbd' && h !== match.homeId)
        hardFail(`${match.fid}: LIVE home_team_id='${h}' disagrees with intended home '${match.homeId}' (${match.home}) — orientation flip; refusing to model/write`);
      if (a && a !== 'tbd' && a !== match.awayId)
        hardFail(`${match.fid}: LIVE away_team_id='${a}' disagrees with intended away '${match.awayId}' (${match.away}) — orientation flip; refusing to model/write`);
  ...
  for (const proj of DRY_RUN ? [] : projections) {
    ...
    // The fixture may still carry 'tbd' team slots (bracket scraper lag — the
    // live 3rd-103 row is tbd/tbd as of 2026-07-17) — repair to the confirmed
    // SF losers/winners before writing odds. The Phase-6 orientation assertion
    // has already proven any non-tbd slots agree with homeId/awayId, so this is
    // a safety net; it only writes when a slot is still 'tbd'.
    ...
    // shown in ET). Both live rows are already bucketed correctly (3rd-103 on
    // Jul 18, final-104 on Jul 19 — verified via CI feed probe), so this repair
    // is a no-op safety net; it only fires if the seeded DATE ever differs from
    // seedDatePT, and never touches kickoff_utc.
    if (match.seedDatePT) {
    ...
    `, [
      ...
      'knockout', ROUND_LABEL[match.fid], ESPN_TEAM_IDS[match.home], ESPN_TEAM_IDS[match.away],
  ...
  const [auditRows] = await conn.query(`
    ...
    FROM wc2026MatchOdds WHERE match_id IN ('wc26-3rd-103','wc26-final-104')
  `);
