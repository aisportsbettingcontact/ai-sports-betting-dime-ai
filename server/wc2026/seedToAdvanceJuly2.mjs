...
async function main() {
  ...
  for (const s of SEED) {
    log('◀◀ [INPUT]', 'INPUT', `  ${s.fixtureId}: ${s.homeTeam} (home) vs ${s.awayTeam} (away)`);
  ...
  for (const s of SEED) {
    log('▶▶ [STEP]', 'PRE', `Verifying orientation for ${s.fixtureId}`);
    const [rows] = await conn.execute(
      'SELECT fixture_id, home_team_id, away_team_id FROM wc2026_fixtures WHERE fixture_id = ?',
      [s.fixtureId]
    );
    if (!rows.length) {
      log('✗✗ [FAIL]', 'PRE', `  ${s.fixtureId}: NOT FOUND in wc2026_fixtures — ABORT`);
      preflightFail++;
      continue;
    }
    const row = rows[0];
    ...
    if (homeOk && awayOk) {
      log('✅ [PASS]', 'PRE', `  ${s.fixtureId}: Orientation VERIFIED — home=${s.homeTeamId} away=${s.awayTeamId}`);
      preflightPass++;
    } else {
      log('✗✗ [FAIL]', 'PRE', `  ${s.fixtureId}: Orientation MISMATCH — ABORT`);
      preflightFail++;
    }
  }
  ...
  for (const s of SEED) {
    const [rows] = await conn.execute(
      'SELECT fixture_id, book_home_to_advance, book_away_to_advance FROM wc2026MatchOdds WHERE fixture_id = ?',
      [s.fixtureId]
    );
    if (!rows.length) {
      log('⚠️ [WARN]', 'SNAP', `  ${s.fixtureId}: NOT FOUND in wc2026MatchOdds — will INSERT`);
    } else {
      const r = rows[0];
      log('·· [STATE]', 'SNAP', `  ${s.fixtureId}: current book_home_to_advance=${r.book_home_to_advance ?? 'NULL'} book_away_to_advance=${r.book_away_to_advance ?? 'NULL'}`);
    }
  }
  ...
  for (const s of SEED) {
    log('▶▶ [STEP]', 'UPD', `Updating ${s.fixtureId}: home=${s.bookHomeToAdvance > 0 ? '+' : ''}${s.bookHomeToAdvance} away=+${s.bookAwayToAdvance}`);
    try {
      ...
      if (result.affectedRows === 1) {
        log('✅ [PASS]', 'UPD', `  ${s.fixtureId}: UPDATE OK — affectedRows=1`);
        updatePass++;
      } else {
        log('✗✗ [FAIL]', 'UPD', `  ${s.fixtureId}: UPDATE FAILED — affectedRows=${result.affectedRows}`);
        updateFail++;
      }
    } catch (err) {
      log('✗✗ [FAIL]', 'UPD', `  ${s.fixtureId}: EXCEPTION — ${err.message}`);
      updateFail++;
    }
  }
  ...
  for (const s of SEED) {
    ...
    if (!rows.length) {
      log('✗✗ [FAIL]', 'VFY', `  ${s.fixtureId}: NOT FOUND after update`);
      vfyFail++;
      continue;
    }
    const r = rows[0];
    ...
    log('   [ATOMIC]', 'VFY', `  ${s.fixtureId}: book_home_to_advance=${r.book_home_to_advance} | expected=${s.bookHomeToAdvance} | ${homeOk ? 'PASS ✓' : 'FAIL ✗'}`);
    log('   [ATOMIC]', 'VFY', `  ${s.fixtureId}: book_away_to_advance=${r.book_away_to_advance} | expected=${s.bookAwayToAdvance} | ${awayOk ? 'PASS ✓' : 'FAIL ✗'}`);
    if (homeOk && awayOk) {
      log('✅ [PASS]', 'VFY', `  ${s.fixtureId}: VERIFIED — ${s.homeTeam} home=${s.bookHomeToAdvance > 0 ? '+' : ''}${s.bookHomeToAdvance} | ${s.awayTeam} away=+${s.bookAwayToAdvance}`);
      vfyPass++;
    } else {
      log('✗✗ [FAIL]', 'VFY', `  ${s.fixtureId}: MISMATCH after update`);
      vfyFail++;
    }
  }
