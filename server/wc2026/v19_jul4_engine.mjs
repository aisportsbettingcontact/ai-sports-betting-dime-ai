/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║  WC2026 v19.0 — 500X BACKTEST + CORRECT SCORE + RECALIBRATION ENGINE       ║
 * ║  Date: 2026-07-05                                                           ║
 * ║  Scope: Jul 4 R16 Matches (089: PAR vs FRA, 090: CAN vs MAR)              ║
 * ╠══════════════════════════════════════════════════════════════════════════════╣
 * ║  PIPELINE:                                                                  ║
 * ║    1. Pull ESPN data (xG, team stats, match stats, player stats, shot map) ║
 * ║    2. 500X Monte Carlo backtest across 25 variations (16 matches)          ║
 ...
 * ║    6. Model both Jul 4 matches with final calibrated engine                ║
 * ║    7. Full audit + DB write                                                 ║
 * ║                                                                              ║
 * ║  FIXES FROM v18:                                                            ║
 * ║    • Line 525 bug: spread_odds now uses AH odds (not ML)                  ║
 * ║    • JUL4_BOOK updated with fresh bet365 data (scraped 2026-07-04 17:00)  ║
 * ║    • Correct score probability matrix added                                 ║
 * ║    • Double Chance (WD) and No Draw markets added to DB write              ║
 * ║    • Book spread for CAN/MAR corrected to -0.5 (was -2.5)                 ║
 * ║                                                                              ║
...
const SESSION_ID = `v19-500X-BACKTEST-RECAL-${Date.now()}`;
const START_TS = Date.now();
const ENGINE_VERSION = 'v19.0-500X-CORRECT-SCORE-RECALIBRATED-R16';
...
// ══════════════════════════════════════════════════════════════════════════════
// TIER MULTIPLIERS (v19 — refined from v18 backtest performance)
// ══════════════════════════════════════════════════════════════════════════════
...
function deriveAllMarkets(joint, lambdaH, lambdaA, spreadLine) {
  ...
  for (let h=0;h<=MAX_G;h++) for (let a=0;a<=MAX_G;a++) {
    ...
    if (h>a) { pH+=p; pAdvH+=p; }
      ...
      pD+=p;
      // Extra time simulation for advance market
    ...
    if (h>0&&a>0) pBTTS+=p;
    // Spread: half-goal lines have no push
    const margin = h - a;
    if (Math.abs(spreadLine) % 1 !== 0) {
      // Half-goal line (e.g., -1.5, -0.5)
      if (margin > -spreadLine) pHomeSpread+=p;
      else pAwaySpread+=p;
    } else {
      // Whole-goal line — push splits
      if (margin > -spreadLine) pHomeSpread+=p;
  ...
  // Double Chance
  const pHomeWD = pH + pD; // 1X
  const pAwayWD = pA + pD; // X2
  const pNoDraw = pH + pA; // 12
...
function correctScoreGrade(joint, actualHome, actualAway) {
  ...
  // Rank this score among all possible scores
  const allScores = [];
...
// FRESH bet365 book odds (scraped 2026-07-04 17:00 UTC from BetExplorer AJAX)
const JUL4_BOOK = {
  'wc26-r16-089': { bookHomeMl:1600, bookDraw:600, bookAwayMl:-588, bookSpread:-1.5, bookTotal:2.5, bookOver:-161, bookUnder:130, bookBttsY:138, bookBttsN:-189, bookHomeAdv:800, bookAwayAdv:-1600, bookHomeWD:400, bookAwayWD:-5000, bookNoDraw:-1000, bookHomeSpreadOdds:120, bookAwaySpreadOdds:-154 },
  'wc26-r16-090': { bookHomeMl:400, bookDraw:225, bookAwayMl:-120, bookSpread:-0.5, bookTotal:2.5, bookOver:130, bookUnder:-161, bookBttsY:105, bookBttsN:-143, bookHomeAdv:210, bookAwayAdv:-265, bookHomeWD:-110, bookAwayWD:-588, bookNoDraw:-303, bookHomeSpreadOdds:103, bookAwaySpreadOdds:-120 },
};

// Frozen book odds for backtest
const BACKTEST_BOOK = {
...
// ══════════════════════════════════════════════════════════════════════════════
async function main() {
  fs.appendFileSync(LOG_FILE, `\n${'═'.repeat(100)}\n  v19 ENGINE SESSION: ${SESSION_ID}\n  STARTED: ${new Date().toISOString()}\n${'═'.repeat(100)}\n`);
  banner(`WC2026 ${ENGINE_VERSION} — 500X BACKTEST → CORRECT SCORE → RECALIBRATION → REINFORCEMENT → PROJECTIONS`);
  ...
  // Analyze which markets are weakest
  ...
  // Recalibration: average top-3 parameters
  ...
  // Run recalibrated variation through backtest
  const recalResults = [];
  for (const match of BACKTEST_MATCHES) {
    try {
      const gsH = buildGSRows(match.home, xgAll, tsAll, msAll);
  ...
  // Choose best between original best and recalibrated
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // PHASE 6: MODEL BOTH JULY 4 MATCHES
  // ══════════════════════════════════════════════════════════════════════════
  banner('PHASE 6/7 — MODEL JULY 4 R16 MATCHES (FINAL CALIBRATED ENGINE)');
  const projections = [];
  
  for (const match of PROJECTION_MATCHES) {
    log('PROJ', `${'━'.repeat(80)}`);
    log('PROJ', `${match.fid}: ${match.home} vs ${match.away} (R16 — Jul 4, 2026)`);
    log('PROJ', `${'━'.repeat(80)}`);
  ...
  for (const proj of projections) {
    ...
    const homeTeamId = ESPN_TEAM_IDS[match.home];
    const awayTeamId = ESPN_TEAM_IDS[match.away];
    
    // Update wc2026MatchOdds with CORRECT spread odds (not ML)
    await conn.query(`
      UPDATE wc2026MatchOdds SET
        ...
        insert_method = ?, last_inserted_at = NOW(), odds_source = 'betexplorer'
      WHERE match_id = ?
    `, [
  ...
  const [auditRows] = await conn.query(`
    ...
    FROM wc2026MatchOdds WHERE match_id IN ('wc26-r16-089','wc26-r16-090')
  `);
