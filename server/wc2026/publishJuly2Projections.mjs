 ...
 *      and confirm all 3 matchs are returned with non-null modelOdds
...
async function main() {
  ...
  log('STEP', 'INIT', `Matchs: ${MATCH_IDS.join(', ')}`);
  log('STEP', 'INIT', `Log file: ${LOG_FILE}`);
  log('STEP', 'INIT', `Expected values loaded for ${Object.keys(EXPECTED).length} matchs`);
  ...
  for (const fid of MATCH_IDS) {
    subBanner(`AUDIT: ${fid} (${EXPECTED[fid].match})`);
    ...
    log(matchFail === 0 ? 'PASS' : 'FAIL', 'AUDIT',
      `${fid}: ${matchPass}/${checks.length} fields verified | ${matchFail} failures`);
    auditResults[fid] = matchFail === 0 ? 'PASS' : `FAIL(${matchFail})`;
  }
  ...
  `);
  log('FEED', 'FEED', `wc2026_matches for 2026-07-02: ${matchRows.length} rows found`);
  matchRows.forEach(f => {
  ...
  if (matchRows.length !== 3) {
    log('FAIL', 'FEED', `Expected 3 matchs for 2026-07-02, got ${matchRows.length}`);
  } else {
    log('PASS', 'FEED', '3 matchs confirmed for 2026-07-02 ✓');
  }

  const feedMatchIds = matchRows.map(f => f.match_id);
  const placeholders = feedMatchIds.map(() => '?').join(',');
  ...
  log('OUT', 'FINAL_REPORT', `Matchs published: ${MATCH_IDS.join(', ')}`);
  ...
  for (const fid of MATCH_IDS) {
    const exp = EXPECTED[fid];
    log('OUT', 'FINAL_REPORT', `  ${fid} (${exp.match}): λH=${exp.lambdaH.toFixed(4)} λA=${exp.lambdaA.toFixed(4)} Proj:${exp.projH.toFixed(2)}-${exp.projA.toFixed(2)}`);
