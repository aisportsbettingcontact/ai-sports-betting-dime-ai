...
const g1 = (o: Partial<MlbProviderGame> = {}) =>
  raysRedSoxGame1({
    gamePk: PK_G1,
    officialDate: NS_DATE,
    startUtc: `${NS_DATE}T17:35:00Z`,
    rescheduledFrom: "2126-05-09",
    ...o,
  });
...
async function cleanup() {
  ...
  await db
    .delete(games)
    .where(inArray(games.mlbGamePk, [PK_G1, PK_G2, PK_TMP, PK_MOVED]));
}

describe.skipIf(SKIP_DB_IN_CI)(
  "MLB doubleheader DB invariants (real database)",
  () => {
    beforeAll(async () => {
      await cleanup();
    });
    ...
    it("[DH-DB-1] both doubleheader games coexist as distinct rows", async () => {
      ...
      const { inserted, applyErrors } = await applyMlbScheduleSyncPlan(
        db!,
        plan
      );
      ...
      expect(new Set(rows.map(r => r.mlbGamePk))).toEqual(
        new Set([PK_G1, PK_G2])
      );
    ...
    it("[DH-DB-3] gamePk unique index rejects duplicate provider identity", async () => {
      ...
      await expectDuplicateKey(
        db!.insert(games).values({
          ...
          sport: "MLB",
          gameNumber: 1,
          mlbGamePk: PK_G1, // PK_G1 already owns a row
        })
      );
    });
    ...
    it("[DH-DB-4] re-ingestion updates only the matching provider event", async () => {
      ...
      const { updated, applyErrors } = await applyMlbScheduleSyncPlan(
        db!,
        plan
      );
      expect(applyErrors).toEqual([]);
      ...
      const rows = await loadNsRows(db!);
      expect(rows.find(r => r.mlbGamePk === PK_G1)?.startTimeEst).toBe(
        "2:05 PM"
      );
      expect(rows.find(r => r.mlbGamePk === PK_G2)?.startTimeEst).toBe(
        "7:10 PM"
      );
    ...
    it("[DH-DB-6] a stale snapshot cannot regress terminal status", async () => {
      ...
      expect(
        (await loadNsRows(db!)).find(r => r.mlbGamePk === PK_G1)?.gameStatus
      ).toBe("final");
    ...
    it("[DH-DB-8] two-phase apply survives a gameNumber permutation (swap-deadlock hardening)", async () => {
      ...
      const restore = planMlbScheduleSync([g1(), g2()], await loadNsRows(db!));
      expect(
        (await applyMlbScheduleSyncPlan(db!, restore)).applyErrors
      ).toEqual([]);
    });
    ...
    it("[DH-DB-9] same-pk reschedule end-to-end: old-date row relocates, no blocked insert (the 824766 pattern)", async () => {
      ...
      const allRows: DbGameRow[] = [
        ...windowRows,
        ...oldRows.map((r: (typeof oldRows)[number]) => ({
          ...r,
          mlbGamePk: r.mlbGamePk != null ? Number(r.mlbGamePk) : null,
        })),
      ];
