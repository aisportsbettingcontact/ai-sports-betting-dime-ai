...
async function insertUser(
  ...
  console.log(
    `[insertUser] [INPUT] suffix=${suffix} role=${role} email=${email}`
  );
...
// ── Context factory ────────────────────────────────────────────────────────────
function createContextWithCookie(
  jwt: string,
...
afterAll(async () => {
  ...
  console.log(
    "[afterAll] [STEP] Deleted all test users with username LIKE 'testuser_tvdb_%'"
  );
});
...
describe.skipIf(SKIP_DB_IN_CI)(
  "tokenVersion.db — DB-level and procedure-level force-logout invariants",
  () => {
    // ── DB-LEVEL incrementTokenVersion ───────────────────────────────────────────
    ...
    it("[TV-1..4] incrementTokenVersion increments DB tokenVersion atomically", async () => {
      ...
      console.log(
        `[INPUT] userId=${user.id} initial tokenVersion=${user.tokenVersion}`
      );
      ...
      expect(newTv).toBe(2);
      console.log(
        `[VERIFY] [TV-2] PASS — incrementTokenVersion returned newTv=${newTv}`
      );
      ...
      console.log(
        `[VERIFY] [TV-3] PASS — DB tokenVersion=${freshRow?.tokenVersion} after increment`
      );
      ...
      console.log(
        `[VERIFY] [TV-4] PASS — second increment: DB tokenVersion=${freshRow2?.tokenVersion}`
      );
    });
    ...
    it("[JR-1,2] stale tv JWT rejected; fresh tv JWT accepted by appUserProcedure (real DB)", async () => {
      ...
      console.log(
        `[INPUT] userId=${user.id} tokenVersion=${user.tokenVersion}`
      );
      ...
      const newTv = await incrementTokenVersion(user.id);
      console.log(
        `[STATE] DB tokenVersion incremented to ${newTv} — oldJwt is now stale`
      );
      ...
      await expect(
        staleCaller.metrics.sessionHeartbeat()
      ).rejects.toMatchObject({ code: "UNAUTHORIZED" });
      console.log(
        `[VERIFY] [JR-1] PASS — stale JWT (tv=${user.tokenVersion}) rejected by appUserProcedure with UNAUTHORIZED`
      );
      ...
      console.log(
        `[VERIFY] [JR-2] PASS — fresh JWT (tv=${newTv}) accepted by appUserProcedure; heartbeat.ok=${heartbeatResult.ok}`
      );
    });
    ...
    it("[JR-3] JWT without tv claim (tv=null) is accepted — null skips the tokenVersion check", async () => {
      ...
      const jwtWithoutTv = await new SignJWT({
        sub: String(user.id),
        role: "user",
        type: "app_user",
      })
      ...
      const newTv = await incrementTokenVersion(user.id);
      console.log(
        `[STATE] DB tokenVersion incremented to ${newTv} — JWT without tv should still be accepted`
      );
      ...
      console.log(
        `[VERIFY] [JR-3] PASS — JWT without tv accepted (null tv skips tokenVersion check); me().id=${meResult?.id}`
      );
    });
    ...
    it("[FL-1,2] forceLogoutUser increments tokenVersion and returns newTokenVersion", async () => {
      ...
      console.log(
        `[INPUT] owner.id=${owner.id} target.id=${target.id} target.tokenVersion=${target.tokenVersion}`
      );

      const ownerJwt = await signAppUserToken(
        owner.id,
        "owner",
        owner.tokenVersion
      );
      ...
      // [FL-1] forceLogoutUser returns success=true and newTokenVersion=oldTv+1
      const result = await ownerCaller.appUsers.forceLogoutUser({
        id: target.id,
      });
      expect(result.success).toBe(true);
      expect(result.newTokenVersion).toBe(target.tokenVersion + 1);
      console.log(
        `[VERIFY] [FL-1] PASS — forceLogoutUser returned success=true newTokenVersion=${result.newTokenVersion}`
      );
      ...
      console.log(
        `[VERIFY] [FL-2] PASS — DB tokenVersion=${freshRow?.tokenVersion} === newTokenVersion=${result.newTokenVersion}`
      );
    });
    ...
    it("[FL-3] forceLogoutUser throws BAD_REQUEST when owner tries to logout themselves", async () => {
      ...
      console.log(
        `[VERIFY] [FL-3] PASS — BAD_REQUEST thrown for self-logout attempt`
      );
    });
    ...
    it("[FL-4] forceLogoutUser throws NOT_FOUND for unknown userId", async () => {
      ...
      console.log(
        `[VERIFY] [FL-4] PASS — NOT_FOUND thrown for unknown userId=${nonExistentId}`
      );
    });
    ...
    it("[FL-5,6] end-to-end: old JWT rejected after forceLogoutUser; new JWT accepted", async () => {
      ...
      const oldTargetJwt = await signAppUserToken(
        target.id,
        "user",
        target.tokenVersion
      );
      ...
      const ownerCtx = createContextWithCookie(ownerJwt);
      const ownerCaller = appRouter.createCaller(ownerCtx);
      const forceResult = await ownerCaller.appUsers.forceLogoutUser({
        id: target.id,
      });
      ...
      console.log(
        `[VERIFY] [FL-5] PASS — old JWT (tv=${target.tokenVersion}) rejected by appUserProcedure after forceLogoutUser`
      );
      ...
      expect(heartbeatResult.ok).toBe(true);
      console.log(
        `[VERIFY] [FL-6] PASS — new JWT (tv=${newTv}) accepted by appUserProcedure after forceLogoutUser; heartbeat.ok=${heartbeatResult.ok}`
      );
    });
    ...
    it("[FA-1..3] incrementAllTokenVersions increments all users except the excluded owner", async () => {
      ...
      console.log(
        `[INPUT] owner.id=${owner.id} tv=${ownerTvBefore} | target.id=${target.id} tv=${targetTvBefore}`
      );
      ...
      console.log(
        `[VERIFY] [FA-1] PASS — incrementAllTokenVersions affected ${count} users`
      );
      ...
      console.log(
        `[VERIFY] [FA-1] PASS — target tokenVersion: ${targetTvBefore} → ${targetAfter?.tokenVersion}`
      );
      ...
      console.log(
        `[VERIFY] [FA-3] PASS — owner tokenVersion unchanged: ${ownerAfter?.tokenVersion} === ${ownerTvBefore}`
      );
    });
  }
);
