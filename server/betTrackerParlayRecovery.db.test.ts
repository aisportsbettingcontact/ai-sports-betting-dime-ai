...
async function insertTicket(db: Db, spec: TicketSpec): Promise<number> {
  ...
  if (id == null)
    throw new Error(
      `[insertTicket] ticket for userId=${spec.userId} did not persist`
    );
  console.log(
    `[insertTicket] [OUTPUT] userId=${spec.userId} betId=${id} legCount=${spec.legCount} originalOdds=${spec.originalOdds}`
  );
  return id;
}
...
describe.skipIf(SKIP_DB_SUITE)(
  "bet tracker parlay recovery — real-database invariants",
  () => {
    beforeAll(async () => {
      await cleanup();
    });
    ...
    afterAll(async () => {
      await cleanup();
      console.log(
        `[afterAll] [STEP] removed legs LIKE '${MARK}%' and tickets for userIds ${USER_IDS.join(",")}`
      );
    });
    ...
    it("[PR-1] a leg 7 days old is missed by the today/yesterday sweep and found only by the all-dates sweep", async () => {
      ...
      console.log(
        `[INPUT] betId=${betId} strandedLeg=${strandedId}@${ptDate(7)} todayLeg=${todayLegId}@${ptDate(0)}`
      );
      ...
      console.log(
        `[VERIFY] [PR-1] PASS — window sweep found ${windowIds.length} leg(s), not the ${ptDate(7)} one`
      );
      ...
      console.log(
        `[VERIFY] [PR-1] PASS — all-dates sweep found both legs including the stranded ${ptDate(7)} one`
      );
    ...
    it("[PR-2] a ticket with zero open legs is reachable only by the still-PENDING query, and folds to the divided-down price", async () => {
      ...
      console.log(
        `[INPUT] betId=${betId} legs=WIN/WIN/PUSH originalOdds=600 risk=100 riskUnits=2`
      );
      ...
      console.log(
        `[VERIFY] [PR-2] PASS — no open legs, so no leg-driven sweep would ever touch ticket ${betId}`
      );
      ...
      console.log(
        `[VERIFY] [PR-2] PASS — still-PENDING/legCount>0 query found ticket ${betId}`
      );
      ...
      console.log(
        `[VERIFY] [PR-2] PASS — folded to WIN @ ${after.odds} (toWin=${after.toWin}, units=${after.toWinUnits})`
      );
      ...
      console.log(
        `[VERIFY] [PR-2] PASS — re-fold wrote nothing (odds still ${twice.odds})`
      );
    });
    ...
    it("[PR-3] an early stranded leg never trips the parent 36h alarm; only the leg-level join sees it", async () => {
      ...
      console.log(
        `[INPUT] betId=${betId} parentDate=${utcDate(0)} strandedLeg=${strandedLegId}@${utcDate(5)}`
      );
    ...
    `);
      const parentStuck =
        (parentRows as unknown as Array<Array<Record<string, unknown>>>)[0] ??
        [];
      expect(parentStuck).toHaveLength(0);
      console.log(
        `[VERIFY] [PR-3] PASS — parent alarm silent: ticket ${betId} is dated ${utcDate(0)}, inside the ${STUCK_THRESHOLD_HOURS}h window`
      );
      ...
      const stuckLegs = (await findStuckLegs(STUCK_THRESHOLD_HOURS)).filter(
        l => l.betId === betId
      );
      ...
      expect(stuckLegs[0].hoursPending).toBeGreaterThanOrEqual(
        STUCK_THRESHOLD_HOURS
      );
      console.log(
        `[VERIFY] [PR-3] PASS — leg-level join found leg ${strandedLegId} at ${Math.floor(stuckLegs[0].hoursPending)}h pending`
      );
      ...
      await db
        ...
        .set({ result: "LOSS" })
        .where(eq(trackedBets.id, betId));
      const afterSettle = (await findStuckLegs(STUCK_THRESHOLD_HOURS)).filter(
        l => l.betId === betId
      );
      expect(afterSettle).toHaveLength(0);
      console.log(
        `[VERIFY] [PR-3] PASS — leg alarm clears once the ticket itself is no longer PENDING`
      );
    });
    ...
    it("[PR-4] a WIN set outside the grader survives a re-fold that computes PENDING", async () => {
      ...
      console.log(
        `[STATE] settleParlay says PENDING (${settlement.reason}) — the guard has to refuse this write`
      );
      ...
      console.log(
        `[VERIFY] [PR-4] PASS — ticket ${betId} examined and left at WIN @ ${after.odds}, nothing written`
      );
    });
  }
);
