...
describe("computeDesiredRoleIds", () => {
  ...
  it("entitled sharp member → baseline + legacy + SHARP", () => {
    expect([...computeDesiredRoleIds(sharpUser, REG, NOW)].sort()).toEqual(
      [SUB, LEGACY, SHARP].sort()
    );
  });
  ...
  it("unknown plan slug → baseline only, never a guessed role", () => {
    ...
    expect([...computeDesiredRoleIds(mystery, REG, NOW)].sort()).toEqual(
      [SUB, LEGACY].sort()
    );
  });
  ...
  it("expiry in the past → nothing (roles will be revoked)", () => {
    expect(
      computeDesiredRoleIds({ ...sharpUser, expiryDate: NOW - 1 }, REG, NOW)
        .size
    ).toBe(0);
  });
  ...
  it("expiry exactly now is NOT expired (matches appUserProcedure's strict >)", () => {
    expect(
      computeDesiredRoleIds({ ...sharpUser, expiryDate: NOW }, REG, NOW).size
    ).toBeGreaterThan(0);
  });

  it("hasAccess=false or pendingSetup=true → nothing", () => {
    expect(
      computeDesiredRoleIds({ ...sharpUser, hasAccess: false }, REG, NOW).size
    ).toBe(0);
    expect(
      computeDesiredRoleIds({ ...sharpUser, pendingSetup: true }, REG, NOW).size
    ).toBe(0);
  });
...
describe("wiring contracts", () => {
  const syncSrc = fs.readFileSync(
    path.join(__dirname, "discordRoleSync.ts"),
    "utf8"
  );
  ...
  it("every entitlement path still routes through the sync module", () => {
    ...
    const oauth = fs.readFileSync(
      path.join(__dirname, "..", "discordAuth.ts"),
      "utf8"
    );
    const stripeRouter = fs.readFileSync(
      path.join(__dirname, "..", "routers", "stripe.ts"),
      "utf8"
    );
  ...
  it("resetPassword auto-logs-in (seamless claim → Connect Discord)", () => {
    const appUsersSrc = fs.readFileSync(
      path.join(__dirname, "..", "routers", "appUsers.ts"),
      "utf8"
    );
    const block = appUsersSrc.slice(
      appUsersSrc.indexOf("resetPassword: publicProcedure"),
      appUsersSrc.indexOf("setManualDiscordId:")
    );
    expect(block).toContain(
      "signAppUserToken(input.uid, user.role, newTokenVersion)"
    );
