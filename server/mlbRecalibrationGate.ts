...
export function resolveRecalMode(env: Record<string, string | undefined>): {
  ...
  if (env.MLB_RECAL_MODE === "autopatch") {
    return {
      mode: "autopatch",
      source: "MLB_RECAL_MODE=autopatch (legacy self-patching OVERRIDE)",
    };
  }
...
export function parseProposalEnvelope(
  paramChanges: string | null
): ProposalEnvelope | null {
  if (!paramChanges) return null;
  try {
    const parsed = JSON.parse(paramChanges) as ProposalEnvelope;
    if (!parsed || typeof parsed !== "object" || !parsed.gate?.status)
      return null;
    return parsed;
  } catch {
    return null;
...
export function validateApproval(
  ...
  if (proposal.gate.status !== "PROPOSED") {
    return {
      ok: false,
      reason: `NOT_PENDING: proposal is ${proposal.gate.status}, only PROPOSED can be decided`,
    };
  }
  if (request.decidedBy === proposal.gate.proposedBy) {
    return {
      ok: false,
      reason:
        "SELF_APPROVAL_FORBIDDEN: the proposing agent may not decide its own recalibration",
    };
  }
  if (request.role !== "owner") {
    return {
      ok: false,
      reason: `UNAUTHORIZED_APPROVER: role '${request.role}' may not decide model recalibrations`,
    };
  }
  if (!request.rationale || request.rationale.trim().length === 0) {
    return {
      ok: false,
      reason: "MISSING_RATIONALE: a decision must record why (audit trail)",
    };
  }
...
export async function countOpenQuarantines(
  ...
  const rows = await db
    ...
    .where(
      sql`${mlbGameBacktest.result} = 'QUARANTINED' AND ${mlbGameBacktest.gameDate} >= ${cutoff}`
    );
...
export async function listRecalibrationProposals(
  limit = 20
...
export async function decideRecalibration(
  ...
  if (rows.length === 0)
    return { ok: false, reason: `NOT_FOUND: learning-log row ${proposalId}` };
  const envelope = parseProposalEnvelope(rows[0].paramChanges);
  if (!envelope)
    return {
      ok: false,
      reason:
        "LEGACY_ROW: pre-gate row has no proposal envelope — nothing to decide",
    };

  const openQuarantines =
    request.decision === "APPROVED" ? await countOpenQuarantines(nowMs) : 0;
  const verdict = validateApproval(request, envelope, openQuarantines);
  if (!verdict.ok) {
    console.warn(
      `${TAG} [VERIFY] decision REJECTED by gate: proposal=${proposalId} ${verdict.reason}`
    );
