...
const h = vi.hoisted(() => {
  const productsCreate = vi.fn(async (_p: unknown) => ({
    id: "prod_TEST123",
    livemode: false,
  }));
  const productsUpdate = vi.fn(async () => ({}));
  const pricesCreate = vi.fn(async (_p: unknown) => ({
    id: "price_TEST123",
    livemode: false,
  }));
  const couponsCreate = vi.fn(async (_p: unknown) => ({
    id: "coupon_TEST123",
  }));
...
describe("provisionPlan", () => {
  it("creates a Stripe Product then Price and persists both", async () => {
    ...
    const [prodParams, prodOpts] = h.productsCreate.mock.calls[0] as [
      Record<string, unknown>,
      { idempotencyKey?: string },
    ];
    expect(prodParams).toMatchObject({
      name: "Pro Weekly",
      metadata: { dime_plan_slug: "pro-weekly" },
    });
    ...
    const [priceParams] = h.pricesCreate.mock.calls[0] as [
      Record<string, unknown>,
    ];
  ...
  it("rejects an amount below the Stripe minimum (50c)", async () => {
    await expect(
      provisionPlan({
        name: "Too Cheap",
        prices: [{ amountCents: 10, interval: "month", intervalCount: 1 }],
      })
  ...
  it("clamps a recurring interval to ≤ 1 year (year × 5 → 1)", async () => {
    await provisionPlan({
      name: "Yearly",
      prices: [{ amountCents: 49900, interval: "year", intervalCount: 5 }],
    });
    const [priceParams] = h.pricesCreate.mock.calls[0] as [
  ...
  it("creates multiple intervals under one product, with a promo coupon per promo'd interval", async () => {
      ...
      prices: [
        {
          ...
          interval: "month",
          intervalCount: 1,
          promo: { type: "percent", value: 50, code: "MONTH50" },
        },
        {
          amountCents: 4900,
          interval: "week",
    ...
    const [promoParams] = h.promotionCodesCreate.mock.calls[0] as [
      { code: string; coupon: string },
    ];
    ...
    expect(priceRows[0]).toMatchObject({
      ...
      promoValue: 50,
      promoCode: "MONTH50",
      stripeCouponId: "coupon_TEST123",
    });
    expect(priceRows[1]).toMatchObject({
      isDefault: false,
      promoType: "percent",
      promoValue: 25,
      promoCode: null,
  ...
  it("surfaces a clear 'schema out of date' message (not raw SQL) when a column is missing", async () => {
    ...
    h.insertError = Object.assign(
      new Error("Failed query: insert into `subscription_plans` ..."),
      {
...
describe("describeDbError", () => {
  ...
  it("unwraps a driver error nested under .cause (Drizzle)", () => {
    const d = describeDbError({
      ...
      cause: {
        code: "ER_BAD_FIELD_ERROR",
        errno: 1054,
        sqlMessage: "Unknown column 'restockAmount' in 'field list'",
      },
    });
    expect(d.schemaMismatch).toBe(true);
  ...
  it("flags a missing table (ER_NO_SUCH_TABLE / 1146)", () => {
    expect(
      describeDbError({
        errno: 1146,
        sqlMessage: "Table 'db.subscription_plans' doesn't exist",
      }).schemaMismatch
    ).toBe(true);
  });
  it("does NOT flag an unrelated error (duplicate key) as a schema mismatch", () => {
    const d = describeDbError({
      code: "ER_DUP_ENTRY",
      errno: 1062,
      sqlMessage: "Duplicate entry 'x' for key 'slug'",
    });
