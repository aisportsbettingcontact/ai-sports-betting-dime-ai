...
const DK_GROUND_TRUTH: Record<string, MatchOdds> = {
  "wc26-g-025": {
    home_team_id: "cze",
    away_team_id: "rsa",
    dk: { home: -120, away: 380, draw: 260 },
  },
  "wc26-g-027": {
    home_team_id: "sui",
    away_team_id: "bih",
    dk: { home: -180, away: 500, draw: 310 },
  },
  "wc26-g-028": {
    home_team_id: "can",
    away_team_id: "qat",
    dk: { home: -350, away: 1000, draw: 475 },
  },
  "wc26-g-026": {
    home_team_id: "mex",
    away_team_id: "kor",
    dk: { home: 105, away: 295, draw: 230 },
  },
};
...
function simulateDisplayLayer(dkOdds: {
  home: number;
  away: number;
  draw: number;
}) {
...
describe("WC2026 Feed — June 18 Display Orientation & Odds Mapping", () => {
  ...
  describe("3-way no-vig ROI: all 3 outcomes (H/D/A) incorporated", () => {
    it("wc26-g-025 CZE vs RSA: vig ~3%, fair probs sum to 1.000", () => {
      const { dk } = DK_GROUND_TRUTH["wc26-g-025"];
      const { fairHome, fairDraw, fairAway, vigPct } = calculate3WayNoVig(
        dk.home,
        dk.draw,
        dk.away
      );
