...
async function insertTestUser(suffix: string): Promise<{
  ...
  console.log(
    `[insertTestUser] [INPUT] suffix=${suffix} email=${email} username=${username}`
  );
  ...
  if (!rows.length)
    throw new Error(
      `[insertTestUser] Failed to retrieve inserted user for suffix=${suffix}`
    );
  const user = rows[0];
  console.log(
    `[insertTestUser] [OUTPUT] id=${user.id} email=${user.email} tokenVersion=${user.tokenVersion}`
  );
  return user;
}
...
afterAll(async () => {
  ...
  console.log(
    "[afterAll] [STEP] Deleted all test users with username LIKE 'testuser_pr_%'"
  );
});
...
describe.skipIf(SKIP_DB_IN_CI)(
  "passwordReset — requestPasswordReset + resetPassword invariants (real DB)",
  () => {
    // ── requestPasswordReset — token generation ────────────────────────────────
    ...
    it("[RQ-1] returns { success: true } for a valid email", async () => {
      ...
      console.log(
        `[INPUT] emailOrUsername=${user.email} origin=http://localhost:3000`
      );
      ...
      console.log(
        `[VERIFY] [RQ-1] PASS — requestPasswordReset returned success=true for email=${user.email}`
      );
    });
    ...
    it("[RQ-2] returns { success: true } for a valid username", async () => {
      ...
      console.log(
        `[INPUT] emailOrUsername=${user.username} origin=http://localhost:3000`
      );
      const result = await caller.appUsers.requestPasswordReset({
      ...
      expect(result.success).toBe(true);
      console.log(
        `[VERIFY] [RQ-2] PASS — requestPasswordReset returned success=true for username=${user.username}`
      );
    });
    ...
    it("[RQ-3] returns { success: true } for unknown email (anti-enumeration)", async () => {
      ...
      console.log(
        `[INPUT] emailOrUsername=${unknownEmail} — user does not exist`
      );
      ...
      expect(result.success).toBe(true);
      console.log(
        `[VERIFY] [RQ-3] PASS — success=true for unknown email (anti-enumeration — no error thrown)`
      );
    });
    ...
    it("[RQ-4..8] DB token stored correctly + sendPasswordResetEmail called with correct args", async () => {
      ...
      expect(
        dbUser,
        "DB user must exist after requestPasswordReset"
      ).not.toBeNull();

      // [RQ-4] passwordResetToken is non-null
      expect(dbUser!.passwordResetToken).not.toBeNull();
      console.log(
        `[VERIFY] [RQ-4] PASS — passwordResetToken is non-null: ${dbUser!.passwordResetToken?.slice(0, 16)}...`
      );
      ...
      console.log(
        `[VERIFY] [RQ-5] PASS — passwordResetToken is 64-char hex: ${tokenHex.slice(0, 16)}...`
      );
      ...
      expect(expiresAt).toBeGreaterThanOrEqual(
        expectedExpiry - RESET_TOKEN_TOLERANCE_MS
      );
      expect(expiresAt).toBeLessThanOrEqual(
        afterCall + THIRTY_MIN_MS + RESET_TOKEN_TOLERANCE_MS
      );
      console.log(
        `[VERIFY] [RQ-6] PASS — expiresAt=${new Date(expiresAt).toISOString()} ≈ now + 30min`
      );
      ...
      console.log(
        `[VERIFY] [RQ-7] PASS — sendPasswordResetEmail called with toEmail=${callArgs.toEmail} username=${callArgs.username}`
      );
      ...
      expect(
        tokenMatch,
        "resetUrl must contain a 64-char hex token param"
      ).not.toBeNull();
      ...
      console.log(
        `[VERIFY] [RQ-8] PASS — resetUrl contains raw token; SHA-256(rawToken)===storedHash verified`
      );
    });
    ...
    it("[RQ-9] @username prefix is stripped before lookup", async () => {
      ...
      console.log(
        `[INPUT] emailOrUsername=${atUsername} — @ prefix should be stripped`
      );
      ...
      expect(dbUser!.passwordResetToken).not.toBeNull();
      console.log(
        `[VERIFY] [RQ-9] PASS — @username prefix stripped; token stored for userId=${user.id}`
      );
    });
    ...
    it("[RP-1..8] full reset chain: new hash, cost>=10, tokenVersion++, old hash cleared", async () => {
      ...
      const rawToken = tokenMatch![1];
      console.log(
        `[STATE] rawToken extracted from resetUrl: ${rawToken.slice(0, 16)}...`
      );
      ...
      console.log(
        `[STATE] tvBefore=${tvBefore} oldPasswordHash=${oldPasswordHash.slice(0, 20)}...`
      );

      // Step 2: Execute resetPassword
      console.log(
        `[INPUT] uid=${user.id} token=${rawToken.slice(0, 16)}... password=${NEW_PASSWORD}`
      );
      ...
      expect(
        afterReset,
        "DB user must exist after resetPassword"
      ).not.toBeNull();
      const newPasswordHash = afterReset!.passwordHash;
      console.log(
        `[STATE] newPasswordHash=${newPasswordHash.slice(0, 20)}... tokenVersion=${afterReset!.tokenVersion}`
      );

      // [RP-2] New password is bcrypt-hashed
      const isBcrypt =
        newPasswordHash.startsWith("$2b$") ||
        newPasswordHash.startsWith("$2a$");
      expect(isBcrypt).toBe(true);
      console.log(
        `[VERIFY] [RP-2] PASS — newPasswordHash is bcrypt: ${newPasswordHash.slice(0, 7)}`
      );
      ...
      expect(
        costMatch,
        "Could not parse bcrypt cost from new hash"
      ).not.toBeNull();
      const costRounds = parseInt(costMatch![1], 10);
      expect(costRounds).toBeGreaterThanOrEqual(10);
      console.log(
        `[VERIFY] [RP-3] PASS — bcrypt cost rounds=${costRounds} >= 10 (OWASP minimum)`
      );
      ...
      console.log(
        `[VERIFY] [RP-4] PASS — bcrypt.compare(NEW_PASSWORD, newHash)=true`
      );
      ...
      console.log(
        `[VERIFY] [RP-6] PASS — passwordResetExpiresAt cleared to null`
      );

      // [RP-7] tokenVersion is incremented (old sessions invalidated)
      expect(afterReset!.tokenVersion).toBe(tvBefore + 1);
      console.log(
        `[VERIFY] [RP-7] PASS — tokenVersion incremented: ${tvBefore} → ${afterReset!.tokenVersion}`
      );
      ...
      console.log(
        `[VERIFY] [RP-8] PASS — old password no longer authenticates against new hash`
      );
    });
    ...
    it("[RE-1] throws BAD_REQUEST for unknown uid", async () => {
      ...
      console.log(
        `[VERIFY] [RE-1] PASS — BAD_REQUEST thrown for unknown uid=${fakeUid}`
      );
    });
    ...
    it("[RE-2] throws BAD_REQUEST for wrong token (hash mismatch)", async () => {
      ...
      console.log(
        `[INPUT] uid=${user.id} wrongToken=${wrongToken.slice(0, 16)}... (mismatch)`
      );
      ...
      console.log(
        `[VERIFY] [RE-2] PASS — BAD_REQUEST thrown for token hash mismatch`
      );
    });
    ...
    it("[RE-3] throws BAD_REQUEST when no reset is pending (no token in DB)", async () => {
      ...
      console.log(
        `[VERIFY] [RE-3] PASS — BAD_REQUEST thrown when no reset is pending`
      );
    });
    ...
    it("[RE-4..5] throws BAD_REQUEST for expired token; expired token is cleared from DB", async () => {
      ...
      invalidateAppUserByIdCache(user.id);
      console.log(
        `[INPUT] uid=${user.id} expiredAt=${new Date(expiredAt).toISOString()} (1ms in the past)`
      );
      ...
      console.log(
        `[VERIFY] [RE-4] PASS — BAD_REQUEST thrown for expired token`
      );
      ...
      console.log(
        `[VERIFY] [RE-5] PASS — expired token cleared from DB: passwordResetToken=null passwordResetExpiresAt=null`
      );
    });
  }
);
...
describe("passwordReset — input validation (no DB)", () => {
  it("[RE-6] Zod rejects password shorter than 8 chars (BAD_REQUEST)", async () => {
    ...
    console.log(
      `[INPUT] uid=1 token=valid-format password="Short1!" (7 chars)`
    );
    ...
    console.log(
      `[VERIFY] [RE-6] PASS — BAD_REQUEST thrown for password shorter than 8 chars`
    );
  });
  ...
  it("[RE-7] Zod rejects token not matching 64-char hex format (BAD_REQUEST)", async () => {
    ...
    console.log(
      `[VERIFY] [RE-7] PASS — BAD_REQUEST thrown for invalid token format`
    );
  });
});
