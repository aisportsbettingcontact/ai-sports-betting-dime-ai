  ...
  return db
    ...
    .where(
      and(
        ...
        eq(trackedBetLegs.gameDate, date)
      )
    );
}
...
async function sweepPendingStraights(db: Db, date: string) {
  return db
    .select({ id: trackedBets.id, legCount: trackedBets.legCount })
    .from(trackedBets)
    .where(
      and(
...
async function cleanup(): Promise<void> {
  ...
  if (legParents.size > 0) {
    await db
      .delete(trackedBetLegs)
      .where(inArray(trackedBetLegs.betId, Array.from(legParents)));
  }
...
describe.skipIf(SKIP_DB_SUITE)(
  "betTracker lifecycle — deletion, isolation and orphans (real database)",
  () => {
    beforeAll(async () => {
      const db = await getDb();
      ...
      for (const id of NS_USERS)
        await insertUser(db!, id, id === U_OWNER ? "owner" : "user");
      ...
      await db!.insert(trackedBets).values([
        ...Array.from({ length: 3 }, (_, i) =>
          straight(U_PAGER, D5, i === 1 ? "LOSS" : "WIN", -110 - i)
        ),
        ...Array.from({ length: 9 }, (_, i) =>
          straight(U_PAGER, D4, i % 3 === 0 ? "LOSS" : "WIN", -120 - i)
        ),
        straight(U_PAGER, D3, "WIN", -130),
        ...Array.from({ length: 7 }, (_, i) =>
          straight(U_PAGER, D2, i % 2 === 0 ? "WIN" : "LOSS", -140 - i)
        ),
        ...Array.from({ length: 3 }, (_, i) =>
          straight(U_PAGER, D1, "LOSS", -150 - i)
        ),
      ...
      pagerParlayId = (
        await (
          await callerFor(U_PAGER, "user")
        ).betTracker.createParlay(parlayInput(D_PARLAY, 8802100))
      ).id;
      neighborParlayId = (
        await (
          await callerFor(U_NEIGHBOR, "user")
        ).betTracker.createParlay(parlayInput(D_PARLAY, 8802200))
      ).id;
    ...
    it("[DEL-1] delete removes the ticket and every leg row it owned", async () => {
      ...
      const { id: betId } = await caller.betTracker.createParlay(
        parlayInput(D_PARLAY, 8802300)
      );
      ...
      expect(
        before.every((l: { result: string }) => l.result === "PENDING")
      ).toBe(true);
      ...
      const live = new Set(
        (
          await db!
            .select({ id: trackedBets.id })
            .from(trackedBets)
            .where(inArray(trackedBets.id, parents))
    ...
    it("[ORPH-1,2] a parent-only delete strands legs the sweep still grades and the alarm cannot see", async () => {
      ...
      expect(
        swept.filter((l: { betId: number }) => l.betId === betId)
      ).toHaveLength(3);
      ...
      console.log(
        `[ORPH] betId=${betId} — swept legs=3, ticketsExamined=0, visible to findStuckLegs=false`
      );
    ...
    it("[ISO-1] a per-user query returns only that user's rows", async () => {
      const db = await getDb();
      const pagerRows = await db!
        .select({ id: trackedBets.id, userId: trackedBets.userId })
        .from(trackedBets)
        .where(eq(trackedBets.userId, U_PAGER));
      const neighborRows = await db!
        .select({ id: trackedBets.id, userId: trackedBets.userId })
        .from(trackedBets)
        .where(eq(trackedBets.userId, U_NEIGHBOR));

      expect(pagerRows).toHaveLength(31); // 23 settled + 7 unsettled + 1 ticket
      expect(neighborRows).toHaveLength(3); // 2 settled + 1 ticket
      expect(
        pagerRows.every((r: { userId: number }) => r.userId === U_PAGER)
      ).toBe(true);
      expect(
        neighborRows.every((r: { userId: number }) => r.userId === U_NEIGHBOR)
      ).toBe(true);

      const overlap = new Set(neighborRows.map((r: { id: number }) => r.id));
      expect(
        pagerRows.filter((r: { id: number }) => overlap.has(r.id))
      ).toEqual([]);
      ...
      const page = await (
        await callerFor(U_NEIGHBOR, "user")
      ).betTracker.listWithStatsPaginated({ limit: 200, isHistorical: true });
      expect(page.bets.map(b => b.id).sort()).toEqual(
        neighborRows.map((r: { id: number }) => r.id).sort()
      );
    });

    it("[ISO-2] a non-privileged caller cannot scope a read to another user", async () => {
      await expect(
        (await callerFor(U_NEIGHBOR, "user")).betTracker.listWithStatsPaginated(
          { targetUserId: U_PAGER, limit: 10, isHistorical: true }
        )
      ...
      const asOwner = await (
        await callerFor(U_OWNER, "owner")
      ).betTracker.listWithStatsPaginated({
        targetUserId: U_NEIGHBOR,
        limit: 200,
        isHistorical: true,
      });
    ...
    it("[ISO-3] legs are reachable only through their own owner's ticket ids", async () => {
      ...
      expect(
        pagerLegs.filter((id: number) => neighborLegs.includes(id))
      ).toEqual([]);
      ...
      const attached = await db!
        .select()
        .from(trackedBetLegs)
        .where(inArray(trackedBetLegs.betId, [pagerParlayId]));
      expect(attached.map((l: { betId: number }) => l.betId)).toEqual([
        pagerParlayId,
        pagerParlayId,
        pagerParlayId,
      ]);
      expect(
        attached.some((l: { betId: number }) => l.betId === neighborParlayId)
      ).toBe(false);
      ...
      expect(new Set(byShape.map((l: { betId: number }) => l.betId))).toEqual(
        new Set([pagerParlayId, neighborParlayId])
      );
      ...
      expect(ticket.legs.map((l: { id: number }) => l.id).sort()).toEqual(
        pagerLegs.sort()
      );
    });
    ...
    it("[LC-1,2] the straight-bet sweep selection excludes the parlay parent", async () => {
      ...
      const { id: ticketId } = await caller.betTracker.createParlay(
        parlayInput(D_FILTER, 8802600)
      );
      ...
      const [ticketRow] = await db!
        .select()
        .from(trackedBets)
        .where(eq(trackedBets.id, ticketId));
      const [straightRow] = await db!
        .select()
        .from(trackedBets)
        .where(eq(trackedBets.id, straightId));
      ...
      const ours = (rows: { id: number }[]) =>
        rows
          .map(r => r.id)
          .filter(id => createdBetIds.has(id))
          .sort((a, b) => a - b);
      ...
      const unfiltered = await db!
        ...
        .where(
          and(
            eq(trackedBets.result, "PENDING"),
            eq(trackedBets.gameDate, D_FILTER)
          )
        );
      expect(ours(unfiltered)).toEqual(
        [straightId, ticketId].sort((a, b) => a - b)
      );
    });
    ...
    it("[PAGE-1,2] paging the settled set emits every row exactly once, in order", async () => {
      ...
      const truth = (
        await db!
          ...
          .where(
            and(
              eq(trackedBets.userId, U_PAGER),
              inArray(trackedBets.result, ["WIN", "LOSS"])
            )
          )
      ...
      for (;;) {
        const page = await caller.betTracker.listWithStatsPaginated({
          settledOnly: true,
          limit,
          cursor,
          isHistorical: true,
        ...
        if (++guard > 20)
          throw new Error("pager did not terminate — cursor is not advancing");
        if (!page.hasNextPage) {
    ...
    it("[PAGE-3] settledOnly narrows the list without narrowing stats", async () => {
      const caller = await callerFor(U_PAGER, "user");
      const settled = await caller.betTracker.listWithStatsPaginated({
        settledOnly: true,
        limit: 200,
        isHistorical: true,
      });
      const all = await caller.betTracker.listWithStatsPaginated({
        limit: 200,
        isHistorical: true,
      });
      ...
      expect(
        settled.bets.every(b => b.result === "WIN" || b.result === "LOSS")
      ).toBe(true);
    ...
    it("[DEN-1] a subscriber cannot restate a bet in dollars", async () => {
      ...
      await expect(
        caller.betTracker.update({ id, risk: 5000 })
      ).rejects.toMatchObject({ code: "BAD_REQUEST" });
      await expect(
        caller.betTracker.update({ id, toWin: 9999 })
      ).rejects.toMatchObject({ code: "BAD_REQUEST" });
    ...
    it("[DEN-4] straight create derives toWinUnits when the client omits it", async () => {
      ...
      const bet = await caller.betTracker.create({
        ...
        odds: -110,
        risk: 100,
        riskUnits: 2, // no toWinUnits sent
      });
