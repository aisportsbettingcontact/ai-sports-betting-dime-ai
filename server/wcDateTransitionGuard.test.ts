...
function buildOptimizedOddsWhereClause(
  bookId: number,
  matchIds: string[]
): string {
...
describe("[FIX 2026-06-24] WC date-transition guard — shouldShowSkeleton / shouldShowEmptyState", () => {
  it("[VERIFY] Initial load: isLoading=true → show skeleton, not empty state", () => {
    const isLoading = true,
      ...
      len = 0;
    console.log(
      "[INPUT] Initial load: isLoading=true isFetching=true dataLength=0"
    );
    const skeleton = shouldShowSkeleton(isLoading, isFetching, len);
    const empty = shouldShowEmptyState(isLoading, isFetching, len);
    console.log(
      `[STATE] shouldShowSkeleton=${skeleton} shouldShowEmptyState=${empty}`
    );
  ...
  it("[VERIFY] Date transition (PROJECTIONS): isFetching=true, dataLength=0 → skeleton, NOT blank", () => {
    ...
    const isLoading = false,
      ...
      len = 0;
    console.log(
      "[INPUT] Date transition: isLoading=false isFetching=true dataLength=0"
    );
    const skeleton = shouldShowSkeleton(isLoading, isFetching, len);
    const empty = shouldShowEmptyState(isLoading, isFetching, len);
    console.log(
    ...
    expect(empty).toBe(false);
    console.log(
      "[VERIFY] PASS — Date transition shows skeleton, not blank/empty"
    );
  });
  ...
  it("[VERIFY] Date transition (LINEUPS): isFetching=true, dataLength=0 → skeleton, NOT blank", () => {
    const isLoading = false,
      ...
      len = 0;
    console.log(
      "[INPUT] Lineups date transition: isLoading=false isFetching=true dataLength=0"
    );
    const skeleton = shouldShowSkeleton(isLoading, isFetching, len);
    const empty = shouldShowEmptyState(isLoading, isFetching, len);
    console.log(
    ...
    expect(empty).toBe(false);
    console.log(
      "[VERIFY] PASS — Lineups date transition shows skeleton, not blank"
    );
  });
  ...
  it("[VERIFY] Date transition (SPLITS): isFetching=true, dataLength=0 → skeleton, NOT blank", () => {
    const isLoading = false,
      ...
      len = 0;
    console.log(
      "[INPUT] Splits date transition: isLoading=false isFetching=true dataLength=0"
    );
    const skeleton = shouldShowSkeleton(isLoading, isFetching, len);
    const empty = shouldShowEmptyState(isLoading, isFetching, len);
    console.log(
    ...
    expect(empty).toBe(false);
    console.log(
      "[VERIFY] PASS — Splits date transition shows skeleton, not blank"
    );
  });
  ...
  it("[VERIFY] Matches loaded: isFetching=false, dataLength=6 → show matches", () => {
    const isLoading = false,
      ...
      len = 6;
    console.log(
      "[INPUT] Loaded: isLoading=false isFetching=false dataLength=6"
    );
    const skeleton = shouldShowSkeleton(isLoading, isFetching, len);
    const empty = shouldShowEmptyState(isLoading, isFetching, len);
    console.log(
  ...
  it("[VERIFY] Genuine empty day: isFetching=false, dataLength=0 → show empty state", () => {
    ...
    const isLoading = false,
      ...
      len = 0;
    console.log(
      "[INPUT] Genuine empty: isLoading=false isFetching=false dataLength=0"
    );
    const skeleton = shouldShowSkeleton(isLoading, isFetching, len);
    const empty = shouldShowEmptyState(isLoading, isFetching, len);
    console.log(
    ...
    console.log(
      "[VERIFY] PASS — Genuine empty day shows 'No World Cup matches'"
    );
  });
  ...
  it("[VERIFY] Background refetch with matches: isFetching=true, dataLength=6 → show matches (no flicker)", () => {
    ...
    const isLoading = false,
      ...
      len = 6;
    console.log(
      "[INPUT] Background refetch: isLoading=false isFetching=true dataLength=6"
    );
    const skeleton = shouldShowSkeleton(isLoading, isFetching, len);
    const empty = shouldShowEmptyState(isLoading, isFetching, len);
    console.log(
    ...
    expect(empty).toBe(false);
    console.log(
      "[VERIFY] PASS — Background refetch with existing matches does not trigger skeleton"
    );
  });
  ...
  it("[VERIFY] Regression proof: pre-fix logic showed blank on date transition", () => {
    ...
    const isLoading = false,
      ...
      len = 0;
    console.log(
      "[INPUT] Regression: pre-fix isLoading=false isFetching=true dataLength=0"
    );
    ...
    console.log(
      `[STATE] PRE-FIX: showSkeleton=${preFix_showSkeleton} showEmpty=${preFix_showEmpty}`
    );
    ...
    console.log(
      `[STATE] POST-FIX: showSkeleton=${postFix_showSkeleton} showEmpty=${postFix_showEmpty}`
    );
    expect(postFix_showSkeleton).toBe(true);
    expect(postFix_showEmpty).toBe(false);
    console.log(
      "[VERIFY] PASS — Regression proof: pre-fix showed blank, post-fix shows skeleton"
    );
  });
});
...
describe("[FIX 2026-06-24] WC backend query optimization — match_id IN filter", () => {
  it("[VERIFY] Optimized query filters by match_id IN (...) — not full table scan", () => {
    ...
    console.log(
      "[OUTPUT] Optimized query targets 6 match_ids instead of 3,724+ rows"
    );
    console.log(
      "[VERIFY] PASS — Backend query optimization confirmed: O(N) → O(1) per date"
    );
  });
  ...
  it("[VERIFY] Empty match_ids guard: no matches on date → short-circuit with 1=0", () => {
    ...
    console.log(
      "[VERIFY] PASS — Empty match_ids produces short-circuit WHERE clause"
    );
  });
});
