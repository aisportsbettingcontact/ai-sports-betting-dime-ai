...
function shouldShowEmptyState(
  ...
  return (
    !shouldShowSkeleton(gamesLoading, gamesFetching, sortedDatesLength) &&
    sortedDatesLength === 0
  );
}
...
describe("[FIX 2026-06-24] Date-transition guard — shouldShowSkeleton / shouldShowEmptyState", () => {
  ...
  it("[VERIFY] Date transition: gamesLoading=false, gamesFetching=true, sortedDates=[] → show skeleton (NOT empty state)", () => {
    ...
    console.log(
      '[VERIFY] PASS — Date transition shows skeleton, not false "No games found"'
    );
  });
  ...
  it("[VERIFY] Background refetch with games: gamesFetching=true, sortedDates=10 → show games (not skeleton)", () => {
    ...
    console.log(
      "[VERIFY] PASS — Background refetch with existing games does not trigger skeleton"
    );
  });
  ...
  it("[VERIFY] Sport switch: gamesLoading=false, gamesFetching=true, sortedDates=0 → show skeleton", () => {
    ...
    expect(shouldShowEmptyState(false, true, 0)).toBe(false);
    console.log(
      "[VERIFY] PASS — Sport switch shows skeleton, not false empty state"
    );
  });
  ...
  it("[VERIFY] Pre-fix behavior would show empty state during date transition (regression proof)", () => {
    ...
    const preFix_showSkeleton = (
      gamesLoading: boolean,
      sortedDatesLength: number
    ): boolean => gamesLoading;
    const preFix_showEmpty = (
      gamesLoading: boolean,
      sortedDatesLength: number
    ): boolean => !gamesLoading && sortedDatesLength === 0;
    ...
    console.log(
      "[VERIFY] PASS — Regression proof: pre-fix showed empty state, post-fix shows skeleton"
    );
  });
});
