...
async function insertTestUser(
  ...
  await db.insert(appUsers).values({
    ...
    expiryDate:
      overrides.expiryDate !== undefined ? overrides.expiryDate : null,
    tokenVersion: 1,
    pendingSetup: false,
  });
  ...
  console.log(
    `[insertTestUser] [OUTPUT] userId=${userId} tokenVersion=${tokenVersion}`
  );
...
afterAll(async () => {
  ...
  console.log(
    "[afterAll] [STEP] Deleted all test users with username LIKE 'testuser_login_%'"
  );
});
...
beforeEach(() => {
  loginRateMap.clear();
  console.log(
    "[beforeEach] [STEP] loginRateMap cleared — rate-limit state reset"
  );
});
...
describe.skipIf(SKIP_DB_IN_CI)(
  ...
  () => {
    ...
    it("[SL-1..9] stayLoggedIn=true issues 90-day persistent cookie with correct flags", async () => {
      ...
      console.log(
        `[STATE] setCookies[0].options=${JSON.stringify(setCookies[0]?.options)}`
      );
      ...
      console.log(
        `[VERIFY] [SL-1] PASS — cookie name="${setCookies[0]!.name}"`
      );
      ...
      console.log(
        `[VERIFY] [SL-3] PASS — maxAge=${opts.maxAge} (${NINETY_DAYS_MS}ms = 90 days)`
      );
      ...
      console.log(
        `[VERIFY] [SL-9] PASS — result.user id=${result.user.id} email=${result.user.email} username=${result.user.username} role=${result.user.role}`
      );
    });
    ...
    it("[SC-1..6] stayLoggedIn=false issues session cookie with maxAge=undefined", async () => {
      ...
      expect(setCookies[0]!.name).toBe(APP_USER_COOKIE);
      console.log(
        `[VERIFY] [SC-1] PASS — cookie name="${setCookies[0]!.name}"`
      );
      ...
      console.log(
        `[VERIFY] [SC-3] PASS — maxAge=undefined (session cookie, expires on browser close)`
      );
    ...
    it("[JW-1..6] JWT claims are correct: sub, role, type, tv, alg, exp", async () => {
      ...
      console.log(
        `[VERIFY] [JW-1] PASS — sub="${payload.sub}" === userId="${userId}"`
      );
      ...
      console.log(
        `[VERIFY] [JW-4] PASS — tv=${payload.tv} === tokenVersion=${tokenVersion}`
      );
      ...
      console.log(
        `[VERIFY] [JW-6] PASS — exp drift=${drift}ms < ${EXP_TOLERANCE_MS}ms tolerance`
      );
    });
    ...
    it("[TLS-1,2] HTTPS request produces secure=true, sameSite=none", async () => {
      ...
      console.log(
        `[INPUT] emailOrUsername=${email} protocol=https stayLoggedIn=true`
      );
    ...
    it("[UN-1] login by username (without @) issues cookie correctly", async () => {
      ...
      console.log(
        `[VERIFY] [UN-1] PASS — username login issues cookie maxAge=${setCookies[0]!.options.maxAge}`
      );
    });
    ...
    it("[UN-2] @username (with @ prefix) is treated as email lookup — throws UNAUTHORIZED", async () => {
      ...
      console.log(
        `[INPUT] emailOrUsername=@${username} — contains '@' so isEmail=true → email lookup → null → UNAUTHORIZED`
      );
      ...
      console.log(
        `[VERIFY] [UN-2] PASS — @username treated as email lookup → UNAUTHORIZED (correct production behavior)`
      );
    });
    ...
    it("[ER-1] throws UNAUTHORIZED for unknown email/username", async () => {
      ...
      console.log(
        `[INPUT] emailOrUsername=nobody_xyz_${Date.now()}@test.invalid (does not exist)`
      );
      ...
      console.log(
        `[VERIFY] [ER-1] PASS — UNAUTHORIZED thrown for unknown user`
      );
    });
    ...
    it("[ER-2] throws UNAUTHORIZED for wrong password", async () => {
      ...
      console.log(
        `[VERIFY] [ER-2] PASS — UNAUTHORIZED thrown for wrong password`
      );
    });
    ...
    it("[ER-3] throws FORBIDDEN for hasAccess=false user", async () => {
      ...
      console.log(
        `[VERIFY] [ER-3] PASS — FORBIDDEN thrown for hasAccess=false`
      );
    });
    ...
    it("[ER-4] throws FORBIDDEN for expired user (expiryDate in the past)", async () => {
      ...
      console.log(
        `[INPUT] emailOrUsername=${email} expiryDate=${pastMs} (${new Date(pastMs).toISOString()} — expired)`
      );
      ...
      console.log(
        `[VERIFY] [ER-4] PASS — FORBIDDEN thrown for expired account`
      );
    });
  }
);
