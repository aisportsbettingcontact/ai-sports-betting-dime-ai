...
export interface NhlModelEdge {
  ...
  classification:
    "ELITE EDGE" | "STRONG EDGE" | "PLAYABLE EDGE" | "SMALL EDGE" | "NO EDGE";
  conf: "HIGH" | "MOD" | "LOW";
}
...
export async function runNhlModelForGame(
  input: NhlModelEngineInput
): Promise<NhlModelResult> {
  const enginePath = path.join(__dirname, "nhl_model_engine.py");
  const inputJson = JSON.stringify(input);

  console.log(
    `[NhlModelEngine] ► Spawning Python engine for: ${input.away_team} @ ${input.home_team}`
  );
  console.log(
    `[NhlModelEngine]   Away goalie: ${input.away_goalie ?? "TBD"} (GSAx=${input.away_goalie_gsax.toFixed(2)} GP=${input.away_goalie_gp})`
  );
  console.log(
    `[NhlModelEngine]   Home goalie: ${input.home_goalie ?? "TBD"} (GSAx=${input.home_goalie_gsax.toFixed(2)} GP=${input.home_goalie_gp})`
  );
  console.log(
    `[NhlModelEngine]   Market: PL_odds=${input.mkt_away_pl_odds}/${input.mkt_home_pl_odds} Total=${input.mkt_total} ML=${input.mkt_away_ml}/${input.mkt_home_ml}`
  );
  ...
  return new Promise<NhlModelResult>(resolve => {
    ...
    const timeout = setTimeout(() => {
      ...
      resolve({
        ...
        mkt_pl_away_odds: 0,
        mkt_pl_home_odds: 0,
        mkt_pl_away_cover_pct: 0,
        mkt_pl_home_cover_pct: 0,
        mkt_total_over_odds: 0,
        mkt_total_under_odds: 0,
        away_win_pct: 0,
        home_win_pct: 0,
        away_pl_cover_pct: 0,
        home_pl_cover_pct: 0,
 ...
 */
export async function runNhlModelBatch(
  inputs: NhlModelEngineInput[]
): Promise<NhlModelResult[]> {
  ...
  console.log(
    `[NhlModelEngine] ► Batch spawning Python engine for ${inputs.length} game(s)`
  );
  ...
  return new Promise<NhlModelResult[]>(resolve => {
        ...
        inputs.map(
          inp =>
            ({
              ...
              away_goalie: inp.away_goalie,
              home_goalie: inp.home_goalie,
              proj_away_goals: 0,
              proj_home_goals: 0,
              away_puck_line: "+1.5",
              away_puck_line_odds: 0,
    ...
    proc.on("close", code => {
      ...
      if (code !== 0) {
        console.error(
          `[NhlModelEngine] ✗ Batch Python exited with code ${code}`
        );
        resolve(
          inputs.map(
            inp =>
      ...
      try {
        ...
        console.log(
          `[NhlModelEngine] ✅ Batch complete: ${results.filter(r => r.ok).length}/${results.length} games succeeded`
        );
        resolve(results);
      } catch (parseErr) {
...
> {
  ...
  for (const abbrev of [awayAbbrev, homeAbbrev]) {
    ...
    if (!stats) {
      throw new Error(
        `[NhlModelEngine] No stats found for team ${abbrev} — cannot run model without complete data`
      );
    }
