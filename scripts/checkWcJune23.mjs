...
const rows = await db
  ...
  .from(wc2026Matches)
  .where(inArray(wc2026Matches.matchId, ['wc26-g-045','wc26-g-046','wc26-g-047','wc26-g-048']));

console.log('[CHECK] June 23 WC Matchs:');
for (const r of rows) {
  console.log(`  ${r.matchId}: ${r.homeTeamId} ${r.homeScore ?? 'null'}-${r.awayScore ?? 'null'} ${r.awayTeamId} | status=${r.status}`);
}
process.exit(0);
