 ...
 * ═══════════════════════════════════════════════════════════════════════════════
 * STEP 1 OF 2 — SCHEMA MIGRATION + MATCH + FROZEN BOOK ODDS SEED
 ...
 *   [C] Idempotent match insert (delete + re-insert if exists)
...
async function run() {
  ...
  // ── PHASE D: IDEMPOTENT MATCH INSERT ────────────────────────────────────
  banner('PHASE D — MATCH INSERT (IDEMPOTENT)');

  logStep(`Check for existing match: ${MATCH_ID}`);
  const [existing] = await conn.execute(
    'SELECT match_id, match_date, kickoff_utc FROM wc2026_matches WHERE match_id = ?',
    [MATCH_ID]
  );
  logState(`Existing rows for ${MATCH_ID}: ${existing.length}`);

  if (existing.length > 0) {
    logWarn(`Match ${MATCH_ID} already exists — executing idempotent delete + re-insert`);
    logState(`Existing record: ${JSON.stringify(existing[0])}`);

    logStep(`Delete existing frozen_book_odds for ${MATCH_ID}`);
    const [delOdds] = await conn.execute(
      'DELETE FROM wc2026_frozen_book_odds WHERE match_id = ?', [MATCH_ID]
    );
    logState(`Deleted ${delOdds.affectedRows} frozen_book_odds rows`);
    logPass(`Frozen book odds cleared for ${MATCH_ID}`);

    logStep(`Delete existing model_projections for ${MATCH_ID}`);
    const [delProj] = await conn.execute(
      'DELETE FROM wc2026_model_projections WHERE match_id = ?', [MATCH_ID]
    );
    logState(`Deleted ${delProj.affectedRows} model_projections rows`);
    logPass(`Model projections cleared for ${MATCH_ID}`);

    logStep(`Delete existing match record: ${MATCH_ID}`);
    const [delFix] = await conn.execute(
      'DELETE FROM wc2026_matches WHERE match_id = ?', [MATCH_ID]
    );
    logState(`Deleted ${delFix.affectedRows} match rows`);
    logPass(`Match record cleared for ${MATCH_ID}`);
  } else {
    logState(`No existing match found — proceeding with fresh insert`);
  }

  logStep(`Insert match: ${MATCH_ID} | ${HOME_TEAM.toUpperCase()}(H) vs ${AWAY_TEAM.toUpperCase()}(A)`);
  ...
  try {
    ...
    logPass(`Match inserted: ${MATCH_ID}`);
  } catch (e) {
    fatal(`Failed to insert match ${MATCH_ID}`, e);
  }
  ...
  banner('PHASE F — FULL VERIFICATION (MATCH + ODDS)');

  logStep('Query match row back from DB for verification');
  const [verFix] = await conn.execute(
    `SELECT match_id, match_date, kickoff_utc, stage, home_team_id, away_team_id,
     ...
     FROM wc2026_matches WHERE match_id = ?`,
    [MATCH_ID]
  );
  if (verFix.length === 0) fatal(`Match ${MATCH_ID} not found in DB after insert`);
  const fx = verFix[0];
  logOutput(`Match row: ${JSON.stringify(fx)}`);

  const fixChecks = [
    ['match_id',    fx.match_id,    MATCH_ID],
  ...
  subBanner('MATCH FIELD VALIDATION');
  for (const [field, actual, expected] of fixChecks) {
    if (String(actual) === String(expected)) {
      logPass(`match.${field}: expected="${expected}" actual="${actual}"`);
    } else {
      logFail(`match.${field}: expected="${expected}" actual="${actual}" — MISMATCH`);
    }
  }
  ...
  if (verOdds.length === 0) fatal(`Frozen book odds for ${MATCH_ID} not found in DB after insert`);
  const od = verOdds[0];
  ...
  if (failCount > 0) {
    ...
    logPass(`Match ${MATCH_ID} seeded and frozen correctly`);
  ...
  logOutput(`match_id=${MATCH_ID} | stage=${STAGE} | date=${MATCH_DATE} | kickoff=${KICKOFF_UTC}`);
