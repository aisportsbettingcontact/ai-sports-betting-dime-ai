    ...
    for side in ["away", "home"]:
        ...
        if pitchers:
            ...
            try:
                ...
                ip_full = int(ip_parts[0]) + (
                    int(ip_parts[1]) / 3 if len(ip_parts) > 1 else 0
                )
            except Exception:
                ip_full = 0
...
def compute_calibration(graded_games: list) -> dict:
    ...
    complete = [
        ...
        if g.get("fg_total") is not None
        and g.get("f5_total") is not None
        and g.get("i1_total") is not None
    ]
    ...
    nrfi_rate = (
        sum(1 for g in nrfi_games if g["nrfi"]) / len(nrfi_games)
        if nrfi_games
        else 0.516
    )
    ...
    print(
        f"[STATE] f5_share={f5_share:.4f} i1_share={i1_share:.4f} nrfi_rate={nrfi_rate:.4f}"
    )
    ...
    print(
        f"[STATE] Per-inning weights: {', '.join(f'I{k}={v:.4f}' for k, v in sorted(inning_weights.items()))}"
    )
    ...
    print(
        f"[STATE] Team NRFI rates computed for {len(team_nrfi_rates)} teams (min 20 games)"
    )
    ...
    print(
        f"[STATE] FG ML: away_win={away_win_rate:.4f} home_win={home_win_rate:.4f} pushes={pushes} n={len(fg_ml_games)}"
    )
    ...
    away_rl_cover = (
        sum(1 for m in margins if m + 1.5 > 0) / len(margins) if margins else 0.5
    )
    home_rl_cover = (
        sum(1 for m in margins if m + 1.5 < 0) / len(margins) if margins else 0.5
    )
    rl_push_rate = (
        sum(1 for m in margins if m + 1.5 == 0) / len(margins) if margins else 0.0
    )
    print(
        f"[STATE] FG RL: away +1.5 cover={away_rl_cover:.4f} home -1.5 cover={home_rl_cover:.4f} push={rl_push_rate:.4f} n={len(rl_games)}"
    )
    ...
    print(
        f"[STATE] F5 ML: away_win={f5_away_win_rate:.4f} pushes={f5_pushes} push_rate={f5_push_rate:.4f} n={len(f5_ml_games)}"
    )
    ...
    f5_away_rl_cover = (
        sum(1 for m in f5_margins if m + 0.5 > 0) / len(f5_margins)
        if f5_margins
        else 0.5
    )
    f5_home_rl_cover = (
    ...
    print(
        f"[STATE] F5 RL: away +0.5 cover={f5_away_rl_cover:.4f} home -0.5 cover={f5_home_rl_cover:.4f} push={f5_rl_push_rate:.4f} n={len(f5_rl_games)}"
    )

    # ── Season-level breakdown ─────────────────────────────────────────────
    by_season = defaultdict(
        lambda: {"nrfi": 0, "yrfi": 0, "fg_total": [], "f5_total": [], "i1_total": []}
    )
    for g in complete:
    ...
    for season, data in by_season.items():
        ...
        season_stats[season] = {
            ...
            "avg_fg": round(statistics.mean(data["fg_total"]), 4)
            if data["fg_total"]
            else None,
            "avg_f5": round(statistics.mean(data["f5_total"]), 4)
            if data["f5_total"]
            else None,
            "avg_i1": round(statistics.mean(data["i1_total"]), 4)
            if data["i1_total"]
            else None,
            "f5_share": round(
                statistics.mean(data["f5_total"]) / statistics.mean(data["fg_total"]), 4
            )
            if data["fg_total"] and data["f5_total"]
            else None,
            "i1_share": round(
                statistics.mean(data["i1_total"]) / statistics.mean(data["fg_total"]), 4
            )
            if data["fg_total"] and data["i1_total"]
            else None,
    ...
    for s, stats in sorted(season_stats.items()):
        print(
            f"  {s}: n={stats['n_games']} nrfi={stats['nrfi_rate']} avg_fg={stats['avg_fg']} f5_share={stats['f5_share']} i1_share={stats['i1_share']}"
        )
    ...
    for g in complete:
        for side, score_key, f5_key, i1_key in [
            ...
            team = g.get(side)
            if team:
                if g.get(score_key) is not None:
                    team_avg_runs[team].append(g[score_key])
                if g.get(f5_key) is not None:
                    team_avg_f5[team].append(g[f5_key])
                if g.get(i1_key) is not None:
                    team_avg_i1[team].append(g[i1_key])
    ...
    for team in set(list(team_avg_runs.keys()) + list(team_avg_f5.keys())):
        if len(team_avg_runs.get(team, [])) >= 20:
            team_scoring[team] = {
                ...
                "avg_f5": round(statistics.mean(team_avg_f5[team]), 4)
                if team_avg_f5.get(team)
                else None,
                "avg_i1": round(statistics.mean(team_avg_i1[team]), 4)
                if team_avg_i1.get(team)
                else None,
    ...
    return {
        ...
        "overall": {
            ...
            "f5_home_win_rate": round(f5_home_wins / len(f5_ml_games), 4)
            if f5_ml_games
            else 0,
...
def main():
    ...
    complete_games = [
        g
        for g in all_games
        if g.get("awayFinalScore") is not None
        and g.get("homeFinalScore") is not None
        and g.get("awayF5Score") is not None
        and g.get("homeF5Score") is not None
    ]
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {
            executor.submit(fetch_boxscore, g["gamePk"]): g for g in complete_games
        }
        for i, future in enumerate(as_completed(futures)):
            ...
            if (i + 1) % batch_size == 0:
                print(
                    f"  [STATE] Fetched {i + 1}/{len(complete_games)} | ok={fetched} failed={failed}"
                )
                time.sleep(0.3)
    ...
    print(
        f"[OUTPUT] Phase 2 complete: graded={len(graded_games)} errors={grade_errors}"
    )
    ...
    for g in graded_games[:3]:
        print(
            f"  {g['gameDate']} {g['away_team']}@{g['home_team']}: "
    ...
    with open(OUTPUT_JSON, "w") as f:
        json.dump(
            {"graded_games": graded_games, "summary": calibration["overall"]},
            f,
            indent=2,
        )
    ...
    for season in sorted(ss.keys()):
        s = ss[season]
        lines.append(
            f"  {season}: n={s['n_games']} nrfi={s['nrfi_rate']} avg_fg={s['avg_fg']} avg_f5={s['avg_f5']} avg_i1={s['avg_i1']} f5_share={s['f5_share']} i1_share={s['i1_share']}"
        )
    ...
    for team, data in sorted(ts.items(), key=lambda x: -x[1]["avg_runs"]):
        lines.append(
            f"  {team:4s} = avg_runs={data['avg_runs']} avg_f5={data['avg_f5']} avg_i1={data['avg_i1']} n={data['n_games']}"
        )
    ...
    print("\n[VERIFY] PASS — 3-season backtest complete")
    print(
        f"[VERIFY] graded={len(graded_games)} calibration_constants={len(calibration['overall'])} team_nrfi={len(tn)} team_f5={len(tf)}"
    )
