...
content = open(DB_FILE).read()  # noqa: SIM115
...
# ════════════════════════════════════════════════════════════════════════════════
old_match = re.search(
    r"export async function createAppUser\([^)]+\)[^{]*\{[^}]+\}", content, re.DOTALL
)
if old_match:
    ...
    if fn_end != -1:
        ...
        if "withCircuitBreaker" not in fn_body:
            ...
            if new_fn_body != fn_body:
                print(
                    "  [WARN] createAppUser — complex body, using targeted line replacement"
                )
        else:
...
# Write result
# ════════════════════════════════════════════════════════════════════════════════
open(DB_FILE, "w").write(content)  # noqa: SIM115
...
if patches_applied == 0:
    print(
        "[WARN] No patches were applied — check if functions were already patched or text changed"
    )
    sys.exit(1)
else:
    print("[VERIFY] PASS")
