...
// Key: match_id → { home_score, away_score, note }
// We'll determine match_ids from the DB first, then map scores to correct home/away orientation
...
async function main() {
  ...
  // Step 1: Fetch all June 21 matches with their team orientations
  const [matches] = await conn.execute(
    `SELECT match_id, home_team_id, away_team_id, kickoff_utc, status, home_score, away_score
     FROM wc2026_matches
     WHERE match_date = '2026-06-21'
     ORDER BY kickoff_utc`
  );

  console.log(`${TAG} [INPUT] June 21 matches in DB (${matches.length}):`);
  for (const f of matches) {
    const hs = f.home_score !== null ? f.home_score : 'NULL';
    const as_ = f.away_score !== null ? f.away_score : 'NULL';
    console.log(`${TAG}   ${f.match_id}: ${f.home_team_id} (H) vs ${f.away_team_id} (A) | status=${f.status} | score=${hs}-${as_}`);
  }

  if (matches.length !== 4) {
    console.error(`${TAG} [FAIL] Expected 4 June 21 matches, found ${matches.length}. Aborting.`);
    await conn.end();
    process.exit(1);
  }

  // Step 2: Match each confirmed result to the correct match by team IDs
  let updateCount = 0;
  const updates = [];
  ...
  for (const result of CONFIRMED_RESULTS) {
    ...
    // Find match where (home=team1 AND away=team2) OR (home=team2 AND away=team1)
    const fx = matches.find(f =>
      (f.home_team_id === team1 && f.away_team_id === team2) ||
      (f.home_team_id === team2 && f.away_team_id === team1)
    );

    if (!fx) {
      console.error(`${TAG} [FAIL] No match found for ${label} (team1=${team1}, team2=${team2})`);
      await conn.end();
      process.exit(1);
    }
    ...
    if (fx.home_team_id === team1) {
      ...
      homeScore = score1;
      awayScore = score2;
      console.log(`${TAG} [STEP] ${label} → ${fx.match_id}: ${team1}(H)=${score1} ${team2}(A)=${score2} [orientation: team1=home]`);
    } else {
      // team2 is home (team1 is away)
      homeScore = score2;
      awayScore = score1;
      console.log(`${TAG} [STEP] ${label} → ${fx.match_id}: ${team2}(H)=${score2} ${team1}(A)=${score1} [orientation: team2=home]`);
    }

    updates.push({ match_id: fx.match_id, homeScore, awayScore, label });
  }
  ...
  for (const u of updates) {
    const [res] = await conn.execute(
      ...
      [u.homeScore, u.awayScore, u.match_id]
    );
    if (res.affectedRows !== 1) {
      console.error(`${TAG} [FAIL] Update failed for ${u.match_id} (affectedRows=${res.affectedRows})`);
      await conn.end();
      process.exit(1);
    }
    console.log(`${TAG} [OUTPUT] ${u.match_id}: home_score=${u.homeScore} away_score=${u.awayScore} status=FT → affectedRows=${res.affectedRows} ✅`);
    updateCount++;
  }

  // Step 4: Verify final state
  console.log(`\n${TAG} [VERIFY] Re-reading June 21 matches from DB after update...`);
  ...
  for (const f of verify) {
    const expected = updates.find(u => u.match_id === f.match_id);
    if (!expected) { console.error(`${TAG} [FAIL] Unexpected match in verify: ${f.match_id}`); allPass = false; continue; }
    ...
    const icon = pass ? '✅' : '❌';
    console.log(`${TAG} ${icon} ${f.match_id}: ${f.home_team_id}(H) ${f.home_score}-${f.away_score} ${f.away_team_id}(A) | status=${f.status} | ${expected.label}`);
  }
