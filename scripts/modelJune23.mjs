 ...
 * modelJune23.mjs
 *
 * June 23, 2026 WC2026 Model — v3 Champion Parameters
 ...
 *   wc26-g-046: DR Congo (H) vs Colombia (A)     — 10:00 PM EST
 *
 * LOGGING: [MODEL_J23] [INPUT/STEP/STATE/OUTPUT/VERIFY]
 */
...
// ── Elo Ratings (2026 pre-tournament, updated with group stage results) ───
// Source: FIFA/Elo ratings as of June 2026 pre-tournament estimates
const ELO = {
  uzb: 1580,  // Uzbekistan — first WC, estimated ~1580
  por: 2050,  // Portugal — strong contender
  gha: 1680,  // Ghana
  eng: 1970,  // England
  pan: 1620,  // Panama
  cro: 1900,  // Croatia — 2018 finalist
  cod: 1650,  // DR Congo
  col: 1820,  // Colombia
};
...
const FIXTURES = [
  ...
  { id: 'wc26-g-046', home: 'cod', away: 'col', homeName: 'DR Congo',   awayName: 'Colombia',  kickoffEST: '10:00 PM EST' },
];
...
async function main() {
  ...
  for (const fix of FIXTURES) {
    ...
    // DK book odds
    const dkH = dkMap[`${fix.id}:1X2:home`]?.odds ?? dkMap[`${fix.id}:MONEYLINE:HOME`]?.odds ?? null;
    const dkD = dkMap[`${fix.id}:1X2:draw`]?.odds ?? dkMap[`${fix.id}:MONEYLINE:DRAW`]?.odds ?? null;
    const dkA = dkMap[`${fix.id}:1X2:away`]?.odds ?? dkMap[`${fix.id}:MONEYLINE:AWAY`]?.odds ?? null;
    const dkO = dkMap[`${fix.id}:TOTAL:over`]?.odds ?? null;
    const dkU = dkMap[`${fix.id}:TOTAL:under`]?.odds ?? null;
    const dkLine = dkMap[`${fix.id}:TOTAL:over`]?.line ?? 2.5;
    ...
    // For total edge, use the DK line's corresponding model prob
    const modelOverProb = dkLine === 3.5 ? sim.pO35 : dkLine === 1.5 ? sim.pO15 : sim.pO25;
    const modelUnderProb = dkLine === 3.5 ? sim.pU35 : dkLine === 1.5 ? sim.pU15 : sim.pU25;
    const edgeO = dkO ? ((modelOverProb - impliedProb(dkO)) * 100).toFixed(2) : null;
    const edgeU = dkU ? ((modelUnderProb - impliedProb(dkU)) * 100).toFixed(2) : null;
    ...
    console.log(`${TAG}   O/U 2.5: O=${fmt(ou25O)} (${(sim.pO25*100).toFixed(1)}%) U=${fmt(ou25U)}`);
    console.log(`${TAG}   O/U 3.5: O=${fmt(ou35O)} (${(sim.pO35*100).toFixed(1)}%) U=${fmt(ou35U)}`);
    console.log(`${TAG}   DK Line=${dkLine}: O=${fmt(dkO)} U=${fmt(dkU)} | Edge O=${edgeO !== null ? edgeO+'pp' : 'N/A'} U=${edgeU !== null ? edgeU+'pp' : 'N/A'}`);
    console.log(`${TAG} [OUTPUT] BTTS: ${(sim.btts*100).toFixed(1)}% | AvgGoals: ${projTotal.toFixed(3)}`);
    console.log(`${TAG} [OUTPUT] Lean: ${modelLean === 'H' ? fix.homeName : modelLean === 'A' ? fix.awayName : 'Draw'} (${(leanProb*100).toFixed(1)}%)`);
    ...
    projections.push({
      ...
      sim, projTotal, projSpread,
      edgeH, edgeD, edgeA, edgeO, edgeU,
      dkH, dkD, dkA, dkO, dkU, dkLine,
      modelLean, leanProb,
  ...
  // ── Seed into wc2026_odds_snapshots (book_id=0) ───────────────────────────
  console.log(`\n${TAG} ${'─'.repeat(60)}`);
  ...
  for (const p of projections) {
    const { fix } = p;
    const dkLine = p.dkLine;

    // Determine which total line to use for the feed
    const totalLine = dkLine;
    const overOdds = dkLine === 3.5 ? p.ou35O : dkLine === 1.5 ? p.ou15O : p.ou25O;
    const underOdds = dkLine === 3.5 ? p.ou35U : dkLine === 1.5 ? p.ou15U : p.ou25U;
    const overProb = dkLine === 3.5 ? p.sim.pO35 : dkLine === 1.5 ? p.sim.pO15 : p.sim.pO25;
    const underProb = dkLine === 3.5 ? p.sim.pU35 : dkLine === 1.5 ? p.sim.pU15 : p.sim.pU25;

    const markets = [
      // 1X2
      { market: '1X2', selection: 'home',  americanOdds: p.mlH, impliedProb: p.pH, line: null },
      { market: '1X2', selection: 'draw',  americanOdds: p.mlD, impliedProb: p.pD, line: null },
      { market: '1X2', selection: 'away',  americanOdds: p.mlA, impliedProb: p.pA, line: null },
      // TOTAL
      { market: 'TOTAL', selection: 'over',  americanOdds: overOdds,  impliedProb: overProb,  line: totalLine },
      { market: 'TOTAL', selection: 'under', americanOdds: underOdds, impliedProb: underProb, line: totalLine },
      // DOUBLE_CHANCE
      { market: 'DOUBLE_CHANCE', selection: 'home_draw', americanOdds: p.dc1X, impliedProb: p.pH + p.pD, line: null },
      { market: 'DOUBLE_CHANCE', selection: 'away_draw', americanOdds: p.dcX2, impliedProb: p.pD + p.pA, line: null },
    ];
    ...
    for (const mkt of markets) {
      try {
        ...
        console.log(`${TAG}   [STATE] ✅ ${fix.id} ${mkt.market}:${mkt.selection}${lineStr} → ${oddsStr} (${(mkt.impliedProb*100).toFixed(1)}%)`);
      } catch (err) {
        seedErrors++;
        console.error(`${TAG}   [ERROR] ❌ ${fix.id}:${mkt.market}:${mkt.selection} → ${err.message}`);
      }
    }
  }
  ...
  // ── Final Summary ─────────────────────────────────────────────────────────
  console.log(`\n${TAG} ${'='.repeat(72)}`);
  console.log(`${TAG} [VERIFY] model_projections: ${upserted}/4 | odds_snapshots: ${seedInserted}/28 | errors: ${seedErrors}`);
  console.log(`${TAG} [VERIFY] DB rows confirmed: ${verifyRows.length}/28`);
  console.log(`${TAG} [VERIFY] ${verifyRows.length === 28 && seedErrors === 0 ? '✅ PASS — all 28 rows clean' : '❌ FAIL — check errors above'}`);

  console.log(`\n${TAG} ╔${'═'.repeat(70)}╗`);
  console.log(`${TAG} ║  JUNE 23, 2026 WC2026 — SHARP ORIGINATED LINES (v3 Champion)        ║`);
  console.log(`${TAG} ╠${'═'.repeat(70)}╣`);
  for (const p of projections) {
    const { fix } = p;
    console.log(`${TAG} ║  ${fix.homeName} vs ${fix.awayName} — ${fix.kickoffEST}`);
    ...
    console.log(`${TAG} ║    DK Line=${p.dkLine}: O=${fmt(p.dkO)} U=${fmt(p.dkU)} | Edge O=${p.edgeO !== null ? p.edgeO+'pp' : 'N/A'} U=${p.edgeU !== null ? p.edgeU+'pp' : 'N/A'}`);
    ...
    if (p.dkH !== null) {
      console.log(`${TAG} ║    DK Book: H=${fmt(p.dkH)} D=${fmt(p.dkD)} A=${fmt(p.dkA)} | O${p.dkLine}=${fmt(p.dkO)} U${p.dkLine}=${fmt(p.dkU)}`);
      console.log(`${TAG} ║    Edges:   H=${p.edgeH}pp D=${p.edgeD}pp A=${p.edgeA}pp | O${p.dkLine}=${p.edgeO}pp U${p.dkLine}=${p.edgeU}pp`);
    }
    console.log(`${TAG} ╠${'═'.repeat(70)}╣`);
  }
  console.log(`${TAG} ╚${'═'.repeat(70)}╝`);
  ...
  if (seedErrors > 0 || verifyRows.length !== 28) {
    process.exit(1);
  }
  process.exit(0);
}
