...
# ── Grade a single game ────────────────────────────────────────────────────────
def grade_game(game: dict, dk: dict | None, c: dict, version: str) -> dict:
    ...
    f5_total = (
        (int(f5_away) + int(f5_home))
        if (f5_away is not None and f5_home is not None)
        else None
    )
    ...
    dk_away_rl_odds = (
        str(dk.get("dkAwayRunLineOdds")) if dk.get("dkAwayRunLineOdds") else None
    )
    dk_home_rl_odds = (
        str(dk.get("dkHomeRunLineOdds")) if dk.get("dkHomeRunLineOdds") else None
    )
    ...
    grades["fg_over"] = {
        ...
        "roi": _roi(probs["p_fg_over"], dk_over_odds)
        if over_correct is not None
        else None,
        "brier": _brier(probs["p_fg_over"], over_correct)
        if over_correct is not None
        else None,
    ...
    grades["fg_under"] = {
        ...
        "roi": _roi(probs["p_fg_under"], dk_under_odds)
        if under_correct is not None
        else None,
        "brier": _brier(probs["p_fg_under"], under_correct)
        if under_correct is not None
        else None,
    }
    # F5 ML Home
    if f5_total is not None:
        ...
        grades["f5_ml_home"] = {
            ...
            "roi": None,
            "brier": _brier(probs["p_f5_home"], int(int(f5_home) > int(f5_away)))
            if not f5_push
            else None,
        }
        grades["f5_ml_away"] = {
            ...
            "roi": None,
            "brier": _brier(probs["p_f5_away"], int(int(f5_away) > int(f5_home)))
            if not f5_push
            else None,
        }
        grades["f5_total"] = {
...
def _roi(p: float, ml_str) -> float | None:
    if not ml_str:
        return None
    try:
...
def _brier(p: float, outcome: int | None) -> float | None:
    if outcome is None:
        return None
...
def aggregate(results: list[dict]) -> dict:
    ...
    for mkt in MARKETS:
        ...
        agg[mkt] = {
            ...
            "roi_count": roi_count,
            "avg_brier": round(total_brier / brier_count, 6)
            if brier_count > 0
            else None,
...
def main():
    ...
    for version, c in [("v1", V1), ("v2", V2)]:
        ...
        for i, game in enumerate(games):
            if i % 1000 == 0 and i > 0:
                print(
                    f"  [STATE] {version}: {i}/{len(games)} | {time.time() - t0:.1f}s elapsed"
                )
            ...
            try:
                ...
                if errors[version] <= 3:
                    print(
                        f"  [ERROR] {version} {game['away_team']}@{game['home_team']} {game['gameDate']}: {e}"
                    )
        print(
            f"  [OUTPUT] {version}: graded={len(all_results[version])} errors={errors[version]} | {time.time() - t0:.1f}s"
        )
    ...
    print(
        "Method: Analytical (no Monte Carlo) — uses team scoring tendencies + calibration priors"
    )
    print("=" * 110)
    print(
        f"{'Market':<16} {'v1 Acc':>8} {'v2 Acc':>8} {'Δ Acc':>8} {'v1 n':>7} {'v2 n':>7} "
    ...
    for mkt in MARKETS:
        ...
        print(
            f"{mkt:<16} {v1a_s} {v2a_s} {da_s} {v1['graded']:>7} {v2['graded']:>7} "
    ...
    fg_mae_lift = (
        (c1.get("fg_total_mae", 0) - c2.get("fg_total_mae", 0))
        if (c1.get("fg_total_mae") and c2.get("fg_total_mae"))
        else None
    )
    f5_mae_lift = (
        (c1.get("f5_total_mae", 0) - c2.get("f5_total_mae", 0))
        if (c1.get("f5_total_mae") and c2.get("f5_total_mae"))
        else None
    )

    print(
        f"\n[OUTPUT] FG Total MAE: v1={c1.get('fg_total_mae', 'N/A')} v2={c2.get('fg_total_mae', 'N/A')} "
        f"Δ={fg_mae_lift:+.4f} (n={c2.get('n_fg', 0)})"
        if fg_mae_lift
        else f"\n[OUTPUT] FG Total MAE: v1={c1.get('fg_total_mae', 'N/A')} v2={c2.get('fg_total_mae', 'N/A')}"
    )
    print(
        f"[OUTPUT] F5 Total MAE: v1={c1.get('f5_total_mae', 'N/A')} v2={c2.get('f5_total_mae', 'N/A')} "
        f"Δ={f5_mae_lift:+.4f} (n={c2.get('n_f5', 0)})"
        if f5_mae_lift
        else f"[OUTPUT] F5 Total MAE: v1={c1.get('f5_total_mae', 'N/A')} v2={c2.get('f5_total_mae', 'N/A')}"
    )
    ...
    for season in seasons:
        ...
        print(
            f"  {season}: n={len(sr)} | FG ML Home={fg_acc * 100:.1f}% | F5 ML Home={f5_acc * 100:.1f}% | "
            f"FG RL Away={rl_acc * 100:.1f}% | NRFI={nrfi_acc * 100:.1f}%"
            if all([fg_acc, f5_acc, nrfi_acc, rl_acc])
            else f"  {season}: n={len(sr)}"
        )

    # ── Key constant delta impact ──────────────────────────────────────────────
    print("\n[OUTPUT] Key constant delta impacts:")
    print(
        f"  fg_rl_away: v1={V1['fg_rl_away_cover']:.4f} → v2={V2['fg_rl_away_cover']:.4f} | "
        f"v1 acc={agg_v1.get('fg_rl_away', {}).get('accuracy', 0) * 100:.1f}% → "
        f"v2 acc={agg_v2.get('fg_rl_away', {}).get('accuracy', 0) * 100:.1f}%"
    )
    print(
        f"  f5_ml_home: v1={V1['f5_home_win_rate']:.4f} → v2={V2['f5_home_win_rate']:.4f} | "
        f"v1 acc={agg_v1.get('f5_ml_home', {}).get('accuracy', 0) * 100:.1f}% → "
        f"v2 acc={agg_v2.get('f5_ml_home', {}).get('accuracy', 0) * 100:.1f}%"
    )
    print(
        f"  nrfi:       v1={V1['nrfi_rate']:.4f} → v2={V2['nrfi_rate']:.4f} | "
        f"v1 acc={agg_v1.get('nrfi', {}).get('accuracy', 0) * 100:.1f}% → "
        f"v2 acc={agg_v2.get('nrfi', {}).get('accuracy', 0) * 100:.1f}%"
    )
    print(
        f"  F5_RUN_SHARE: v1={V1['F5_RUN_SHARE']:.4f} → v2={V2['F5_RUN_SHARE']:.4f} | "
    ...
    report = {
        ...
        "constant_deltas": {
            k: {
                "v1": V1.get(k),
                "v2": V2.get(k),
                "delta": round(V2.get(k, 0) - V1.get(k, 0), 4),
            }
            for k in V1
        },
    }
