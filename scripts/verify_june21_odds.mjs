...
async function verify() {
  ...
  // Get the LATEST DK (book_id=68) 1X2 odds for all 4 June 21 matches
  const [rows] = await conn.query(`
    SELECT s.match_id, s.selection, s.american_odds, s.snapshot_ts,
  ...
  // Show latest per match/selection
  const seen = new Set();
  ...
  for (const r of rows) {
    ...
    if (!seen.has(k)) {
      ...
      console.log(`  ${r.match_id} | selection=${r.selection.padEnd(4)} [${label.padEnd(18)}] odds=${odds}`);
    }
  }
  ...
  const [modelRows] = await conn.query(`
    ...
    WHERE s.match_id IN ('wc26-g-037','wc26-g-038','wc26-g-039','wc26-g-040')
      AND s.book_id = 0
      AND s.market = '1X2'
    ORDER BY s.match_id, s.selection
  `);
  ...
  if (dcRows.length === 0) {
    ...
    for (const r of dcRows) {
      ...
      const odds = r.american_odds > 0 ? `+${r.american_odds}` : `${r.american_odds}`;
      console.log(`  ${r.match_id} | selection=${r.selection.padEnd(10)} [${label.padEnd(22)}] odds=${odds}`);
    }
  }
