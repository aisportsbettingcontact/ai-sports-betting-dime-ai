...
export function computeMissingGames(
  finals: DatasetGame[],
  dbGamePks: Set<number>
): DatasetGame[] {
...
async function loadDbGamePks(pool: mysql.Pool): Promise<Set<number>> {
  const [rows] = await pool.query(
    "SELECT game_pk FROM mlb_games WHERE season = ?",
    [SEASON]
  );
...
async function deltaVerify(
  pool: mysql.Pool,
  pks: number[],
  datasetByPk: Map<number, DatasetGame>
): Promise<boolean> {
  ...
  const [rows] = await pool.query(
    `SELECT game_pk, away_score, home_score FROM mlb_games WHERE game_pk IN (?)`,
    [pks]
  );
  ...
  for (const pk of pks) {
    ...
    if (!dbRow) {
      log(
        `[VERIFY] score check FAIL — game_pk=${pk} missing from mlb_games after load`
      );
      scoreMismatches++;
      continue;
    }
    ...
    if (
      Number(dbRow.away_score) !== dataset.awayScore ||
      Number(dbRow.home_score) !== dataset.homeScore
    ) {
      log(
  ...
  log(
    `[VERIFY] score cross-foot: ${pks.length - scoreMismatches}/${pks.length} OK`
  );
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (missing.length === 0) {
      log(
        `up to date — mlb_games already has all ${finals.length} finals for season ${SEASON}`
      );
    } else {
      log(
        `missing ${missing.length} final(s): ${missing.map(g => g.gamePk).join(", ")}`
      );

      // (d) crawl only the missing feeds
      const shardPath = writeDeltaShard(missing);
      try {
        run("crawl_feeds (delta shard)", "python3", [
          join(CRAWL_DIR, "crawl_feeds.py"),
          "--shard",
          shardPath,
        ]);
      } finally {
      ...
      run("transform", "python3", [
        join(ETL_DIR, "transform.py"),
        "--season",
        String(SEASON),
      ]);
      ...
      log(
        `delta refresh complete — loaded and verified ${missing.length} game(s)`
      );
    }
  } catch (err: any) {
