...
Companion to `EVENT-TAXONOMY.md` (which holds the _strategy_: North Star, metric
constellation, journey, power-user model). **This** doc is the _catalog_: every trackable
...
1. **Curated allowlist, exhaustively enumerated** — we track a large, named set of _meaningful_
...
`source_user_id` _(server-derived, authoritative)_ · `session_id` · `tab_id` · `feature_id` ·
`surface` · `outcome` · `occurred_at_utc` _(client)_ · `received_at_utc` _(server, authoritative)_ ·
`environment` · `app_version` · `is_test` · `props_json` _(allowlisted scalars)_.
...
That is the point of the block: _screen time per page_ becomes _screen time per page per device_;
_power users_ can be _mobile-first power users_; _most-important features_ can be _most-important on
mobile vs desktop_.
...
_mobile_ emission differs from _desktop_.

### 3.0 Lifecycle & session — _total active screen time_ (per device)
...
| `device_context_captured` | once per session, on first heartbeat               | no  | _(full §1b block)_      | the canonical device snapshot for the session                   |
...
### 3.1 Navigation & screen-time — _screen time per page, where time goes_ (per device)
...
### 3.2 Auth & identity — _last sign in_ (per device)
...
### 3.9 Feature lifecycle (cross-surface) — _most-important features / friction_
...
## 4. Actions — `action_performed { action_name }` — _total & unique actions_ (per device)
...
- **Nav/Global:** `pane_switched` · `tab_tapped` _(mobile)_ · `search_performed` ·
...
Every owner-named metric now has a device cut: _last sign in_ → **last sign in per device**;
_total active screen time_ → **per device**; _where users spend time_ → **per device**;
_most-important features_ → **per device**; _power users_ → **mobile-first vs desktop-first**.
