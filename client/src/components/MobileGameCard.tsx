...
export const MobileGameCard = React.memo(
  function MobileGameCard(props: MobileGameCardProps) {
    const {
      game,
      awayAbbr,
      homeAbbr,
    ...
    const formatGameClock = (raw: string | null | undefined): string => {
      ...
      const compound = s.match(
        /^(q?\d|\d+(?:st|nd|rd|th)?(?:\s+(?:period|half))?|half(?:time)?)\s+(\d{1,2}:\d{2})$/i
      );
      if (compound) {
    ...
    const bkAwaySpreadStr = !isNaN(awayBookSpread)
      ? mbAwaySpreadOdds
        ? `${spreadSign(awayBookSpread)} (${mbAwaySpreadOdds})`
        : spreadSign(awayBookSpread)
      : "—";
    const bkHomeSpreadStr = !isNaN(homeBookSpread)
      ? mbHomeSpreadOdds
        ? `${spreadSign(homeBookSpread)} (${mbHomeSpreadOdds})`
    ...
    const bkOverStr = !isNaN(bookTotal)
      ? mbOverOdds
        ? `o${bkTotalStr} (${mbOverOdds})`
        : `o${bkTotalStr}`
      : "o—";
    const bkUnderStr = !isNaN(bookTotal)
      ? mbUnderOdds
        ? `u${bkTotalStr} (${mbUnderOdds})`
        : `u${bkTotalStr}`
      : "u—";
    ...
    const mdlDisplayTotal =
      isNhlGame && !isNaN(bookTotal) ? bookTotal : modelTotal;
    ...
    const mdlOverTotalStr = !hasModelData
      ...
      : !isNaN(mdlDisplayTotal)
        ? (isNhlGame || isMlbGame) && game.modelOverOdds
          ? `${mdlTotalStr} (${fmtOddsSign(game.modelOverOdds)})`
          : mdlTotalStr
        : "—";
    const mdlUnderTotalStr = !hasModelData
      ? "—"
      : !isNaN(mdlDisplayTotal)
        ? (isNhlGame || isMlbGame) && game.modelUnderOdds
          ? `${mdlTotalStr} (${fmtOddsSign(game.modelUnderOdds)})`
          : mdlTotalStr
        : "—";
    ...
    const awayMlEdgePP =
      !isNaN(bkAwayMlProb) && !isNaN(mdlAwayMlProb)
        ? mdlAwayMlProb - bkAwayMlProb
        : NaN;
    const homeMlEdgePP =
      !isNaN(bkHomeMlProb) && !isNaN(mdlHomeMlProb)
    ...
    const awayMlEdgeDetected: boolean =
      hasModelData &&
      !isNaN(awayMlEdgePP) &&
      awayMlEdgePP > ML_EDGE_THRESHOLD_PP;
    const homeMlEdgeDetected: boolean =
      hasModelData &&
      !isNaN(homeMlEdgePP) &&
      homeMlEdgePP > ML_EDGE_THRESHOLD_PP;
    if (process.env.NODE_ENV === "development") {
      console.log(
    ...
    const gatedSpreadEdgePP: number = (() => {
      if (spreadEdgeIsAway === true)
        return isNaN(awaySpreadEdgePP) ? NaN : awaySpreadEdgePP;
      if (spreadEdgeIsAway === false)
        return isNaN(homeSpreadEdgePP) ? NaN : homeSpreadEdgePP;
    ...
    const gatedTotalEdgePP: number = (() => {
      ...
      if (totalEdgeIsOver === false)
        return isNaN(underEdgePP) ? NaN : underEdgePP;
      return NaN; // null = no Option B edge
    })();
    ...
    const bestEdgePP: number = (() => {
      const vals = [gatedSpreadEdgePP, gatedTotalEdgePP, gatedMlEdgePP].filter(
        v => !isNaN(v)
      );
    ...
    const MktCard = ({
      ...
      awayBookLine: string;
      awayBookJuice: string;
      awayModelLine: string;
      awayModelJuice: string;
      awayModelHasEdge: boolean;
      homeBookLine: string;
      homeBookJuice: string;
      homeModelLine: string;
      homeModelJuice: string;
      homeModelHasEdge: boolean;
      isML?: boolean;
      ...
      const SubCol = ({
        ...
        const juiceColor = isBook
          ...
          : hasEdge
            ? "#45E0A8"
            : "#FFFFFF"; // MODEL: neon if edge, white if not
        return (
          <div
            style={{
              display: "flex",
      ...
      return (
        ...
        >
          ...
          {/* Away row */}
          <TeamRow
            bookLine={awayBookLine}
            bookJuice={awayBookJuice}
            modelLine={awayModelLine}
            modelJuice={awayModelJuice}
            modelHasEdge={awayModelHasEdge}
          />
          {/* Divider */}
          <div style={{ height: 1, background: "#FFFFFF", margin: "0 4px" }} />
          {/* Home row */}
          <TeamRow
            bookLine={homeBookLine}
            bookJuice={homeBookJuice}
            modelLine={homeModelLine}
            modelJuice={homeModelJuice}
            modelHasEdge={homeModelHasEdge}
          />
      ...
      >
        ...
        {(() => {
          ...
          const spreadRoiPP = awayHasEdge
            ? isNaN(awaySpreadEdgePP)
              ? NaN
              : awaySpreadEdgePP
            : homeHasEdge
              ? isNaN(homeSpreadEdgePP)
                ? NaN
                : homeSpreadEdgePP
          ...
          return (
            <MktCard
              ...
              awayBookJuice={
                mbAwaySpreadOdds ? String(mbAwaySpreadOdds) : "—110"
              }
              ...
              homeBookJuice={
                mbHomeSpreadOdds ? String(mbHomeSpreadOdds) : "—110"
              }
    ...
    return (
      ...
      >
        ...
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "clamp(96px, 24.4vw, 108px) 1fr",
            width: "100%",
            minHeight: 0,
          }}
        >
         ...
         without clipping, nowrap text can paint past the panel boundary onto BOOK/MODEL headers. */}
          ...
          >
            ...
            >
              ...
              >
                {isAppAuthed && (
                  <button
                    ...
                    style={{
                      ...
                      display: "flex",
                      alignItems: "center",
                      color: isFavorited ? "#45E0A8" : "#FFFFFF",
                      filter: isFavorited ? "none" : "none",
                  ...
                  >
                    <svg
                      ...
                      height="10"
                      viewBox="0 0 24 24"
                      fill={isFavorited ? "#45E0A8" : "none"}
                      stroke={isFavorited ? "#45E0A8" : "#FFFFFF"}
                      strokeWidth="2"
                      strokeLinecap="round"
                ...
                {isLive ? (
                  ...
                  <span
                    className="flex items-center gap-0.5 font-black tracking-widest uppercase"
                    style={{
                      color: "#45E0A8",
                      fontSize: "10px",
                      whiteSpace: "nowrap",
                  ...
                  >
                    <span
                      ...
                      style={{ background: "#45E0A8", flexShrink: 0 }}
                    />
                    <span>LIVE</span>
                  </span>
                ) : isFinal ? (
           ...
           PART 2: overflow:hidden REMOVED from abbr span.
             ...
             This is the ONLY bulletproof pattern: hold the space unconditionally. */}
              <span
                ...
                style={{
                  ...
                  visibility:
                    (isLive || isFinal) && hasScores ? "visible" : "hidden",
                }}
              >
                {game.awayScore ?? "0"}
              </span>
            ...
            {/* Divider */}
            <div
              style={{ height: 1, background: "hsl(var(--border) / 0.4)" }}
            />
