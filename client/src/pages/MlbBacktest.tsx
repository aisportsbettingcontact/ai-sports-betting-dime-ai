...
function accBadge(acc: number, sample: number): ReactElement {
  ...
  if (acc >= TARGET_ACCURACY)
    return (
      <Badge className="bg-primary text-primary-foreground border-primary text-xs gap-1">
        <Check className="w-3 h-3" /> {pct(acc)}
      </Badge>
    );
  if (acc >= BREAKEVEN_ACCURACY)
    return (
      <Badge className="bg-background text-foreground border-border text-xs">
        {pct(acc)}
      </Badge>
    );
  return (
...
export default function MlbBacktest() {
  ...
  const [sortBy, setSortBy] = useState<"accuracy" | "roi" | "sample">(
    "accuracy"
  );
  ...
  const runBacktestMutation =
    trpc.mlbBacktest.runHistoricalBacktest.useMutation({
      onSuccess: data => {
        toast.success(
          `Backtest complete: ${data.processed} dates processed, ${data.errors} errors`
        );
        reportQuery.refetch();
  ...
  const SortIcon = ({ col }: { col: typeof sortBy }) => {
    if (sortBy !== col)
      return <Minus className="w-3 h-3 text-foreground inline ml-1" />;
    return sortDir === "desc" ? (
      ...
      <div className="min-h-[calc(100dvh-3.5rem)] bg-background text-foreground p-4 md:p-6 lg:p-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-foreground text-sm mt-1">
              Live 2026 validation · All markets · Target: ≥70% accuracy on
              filtered picks
            </p>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Select
              ...
              onValueChange={v => setMinEdge(Number(v))}
            >
            ...
            <Button
              ...
              onClick={() => {
                reportQuery.refetch();
                timeSeriesQuery.refetch();
                edgeBucketsQuery.refetch();
              }}
              className="bg-background border-border text-foreground hover:bg-background h-9"
            >
              <RefreshCw
                className={`w-4 h-4 ${reportQuery.isFetching ? "animate-spin" : ""}`}
              />
            </Button>
            {/* Owner: run backtest */}
            {isOwner && (
        ...
        {reportQuery.isLoading ? (
          ...
          <div className="text-foreground text-center py-8">
            Failed to load backtest data. {reportQuery.error.message}
          </div>
        ) : report && summary ? (
          <>
            {/* KPI row */}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="bg-background border border-border mb-4 flex-wrap h-auto gap-1 p-1">
                <TabsTrigger
                  value="overview"
                  className="data-[state=active]:bg-primary data-[state=active]:text-primary-foreground text-foreground text-xs sm:text-sm"
                >
                  Overview
                </TabsTrigger>
                <TabsTrigger
              ...
              <TabsContent value="overview">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-background border-b border-border">
                        <th className="text-left px-3 py-2.5 text-foreground font-medium text-xs w-36">
                          Market
                        </th>
                        ...
                        <th
                          className="text-center px-3 py-2.5 text-foreground font-medium text-xs cursor-pointer hover:text-foreground"
                        ...
                        <th className="text-center px-3 py-2.5 text-foreground font-medium text-xs">
                          Avg Edge
                        </th>
                        <th className="text-center px-3 py-2.5 text-foreground font-medium text-xs">
                          Filtered Acc
                    ...
                    <tbody>
                      {sortedMarkets.map((m, i) => {
                        ...
                        return (
                          <tr
                            key={m.market}
                            className={`border-b border-border hover:bg-background transition-colors ${i % 2 === 0 ? "bg-background" : "bg-background"}`}
                          >
                            ...
                            <td className="px-3 py-2.5 text-center">
                              <span
                                className={`font-mono font-bold text-xs ${accColor(m.accuracy)}`}
                              >
                            ...
                            <td className="px-3 py-2.5 text-center">
                              <span
                                className={`font-mono text-xs ${roiColor(m.roi)}`}
                              >
                                {sample >= 5
                                  ? `${m.roi >= 0 ? "+" : ""}${pct(m.roi)}`
                                  : "—"}
                            ...
                            <td className="px-3 py-2.5 text-center">
                              {accBadge(
                                m.filteredAccuracy,
                                m.filteredWins + m.filteredLosses
                              )}
                            </td>
                            <td className="px-3 py-2.5 text-center">
                ...
                <div className="mt-4 mb-6">
                    ...
                    {[
                      ...
                      >
                        <div className="flex items-center justify-between mb-1">
                          <span
                            className={`font-bold text-sm ${t.status === "LIVE" ? "text-primary-foreground" : "text-foreground"}`}
                          >
                            {t.market}
                          </span>
                          <Badge
                          ...
                          >
                            {t.status === "LIVE" ? (
                              <>
                                <Check className="w-3 h-3" /> {pct(t.acc)}
                              </>
                            ) : (
                              `~ ${pct(t.acc)}`
                            )}
                        ...
                        <div
                          className={`text-xs font-mono mb-2 ${t.status === "LIVE" ? "text-primary-foreground" : "text-foreground"}`}
                        >
                          {t.threshold}
                        </div>
                        <div className="flex items-center gap-3 text-xs">
                          <span
                            className={`font-bold ${t.status === "LIVE" ? "text-primary-foreground" : "text-primary"}`}
                          >
                            {t.w}W
                          </span>
                          <span
                            className={`font-bold ${t.status === "LIVE" ? "text-primary-foreground" : "text-foreground"}`}
                          >
                            {t.l}L
                          </span>
                          <span
                            className={`font-bold ml-auto ${t.status === "LIVE" ? "text-primary-foreground" : t.roi >= 0 ? "text-primary" : "text-foreground"}`}
                          >
                            ROI: {t.roi >= 0 ? "+" : ""}
                            {pct(t.roi)}
                          </span>
                  ...
                  <div className="mt-3 p-3 rounded-lg bg-background border border-border">
                    <div className="text-xs text-foreground font-semibold mb-1 flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3" /> Calibration In
                      Progress
                    </div>
                    <div className="text-xs text-foreground">
                      ...
                      Bias = −0.52 Ks/start (under-projecting). Calibration
                      factors updated: OVER ×1.05, UNDER ×1.03. Re-run model to
                      validate. &nbsp;|&nbsp;
                      <span className="text-foreground font-medium">
                        HR Props:
                      </span>{" "}
                      Over-predicting P(HR) by +3.57pp. HR_CALIBRATION_FACTOR
                      reduced 0.875 → 0.720. Re-run model to validate.
                    </div>
                  </div>
                </div>
                  ...
                  <ResponsiveContainer width="100%" height={280}>
                    ...
                    >
                      ...
                      <Bar dataKey="accuracy" radius={[3, 3, 0, 0]}>
                        {sortedMarkets
                          .filter(m => m.wins + m.losses >= 5)
                          .map(m => (
                            <Cell
                              key={m.market}
                              fill={
                                m.accuracy >= TARGET_ACCURACY
                                  ? "#45E0A8"
                                  : m.accuracy >= BREAKEVEN_ACCURACY
                                    ? "#FFFFFF"
                                    : "#FFFFFF"
                              }
                            />
              ...
              <TabsContent value="roi">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
                  ...
                  <span className="text-foreground text-xs">
                    Cumulative ROI over time (flat-bet $100/game)
                  </span>
                </div>
                    ...
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-background border-b border-border">
                        <tr>
                          ...
                          <th className="text-center px-3 py-2 text-foreground font-medium">
                            W
                          </th>
                          <th className="text-center px-3 py-2 text-foreground font-medium">
                            L
                      ...
                      <tbody>
                        {[...timeSeriesQuery.data].reverse().map((d, i) => (
                          <tr
                            key={d.date}
                            className={`border-b border-border ${i % 2 === 0 ? "bg-background" : "bg-background"}`}
                          >
                            <td className="px-3 py-1.5 text-foreground font-mono">
                              {d.date}
                            </td>
                            <td className="px-3 py-1.5 text-center text-primary font-mono">
                              {d.wins}
                            </td>
                            <td className="px-3 py-1.5 text-center text-foreground font-mono">
                              {d.losses}
                            </td>
                            <td
                              className={`px-3 py-1.5 text-center font-mono ${accColor(d.accuracy)}`}
                            >
                              {pct(d.accuracy)}
                            </td>
                            <td
                              className={`px-3 py-1.5 text-center font-mono ${roiColor(d.cumulativeRoi)}`}
                            >
                              {d.cumulativeRoi >= 0 ? "+" : ""}
                              {pct(d.cumulativeRoi)}
                            </td>
              ...
              <TabsContent value="calibration">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
                  ...
                  <span className="text-foreground text-xs">
                    Edge bucket → actual win rate (well-calibrated =
                        ...
                        <Bar dataKey="accuracy" radius={[3, 3, 0, 0]}>
                          {(edgeBucketsQuery.data ?? []).map(b => (
                            <Cell
                              key={b.bucket}
                              fill={
                                b.accuracy >= TARGET_ACCURACY
                                  ? "#45E0A8"
                                  : b.accuracy >= BREAKEVEN_ACCURACY
                                    ? "#FFFFFF"
                                    : "#FFFFFF"
                              }
                            />
                        ...
                        <tbody>
                          {(edgeBucketsQuery.data ?? []).map((b, i) => (
                            ...
                            >
                              ...
                              <td
                                className={`px-3 py-1.5 text-center font-mono font-bold ${accColor(b.accuracy)}`}
                              >
                                {pct(b.accuracy)}
                              </td>
                              <td className="px-3 py-1.5 text-center text-foreground font-mono">
              ...
              <TabsContent value="kprops">
                {kPropsQuery.isLoading ? (
                  ...
                  <div className="text-foreground text-center py-8">
                    No K-Props backtest data available.
                  </div>
                )}
              </TabsContent>
              ...
              <TabsContent value="hrprops">
                {hrPropsQuery.isLoading ? (
                  ...
                  <div className="text-foreground text-center py-8">
                    No HR Props backtest data available.
                  </div>
                )}
              </TabsContent>
            </Tabs>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <KpiCard
          label="MAE"
          value={data.mae.toFixed(2)}
          sub="Mean absolute error (Ks)"
          color={data.mae < 2 ? "text-primary" : "text-foreground"}
          icon={<Target className="w-4 h-4" />}
        />
        <KpiCard
          label="Bias"
        ...
        <KpiCard
          ...
          color={data.rmse < 2.5 ? "text-primary" : "text-foreground"}
          icon={<BarChart2 className="w-4 h-4" />}
        />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="bg-background border-border">
          ...
          <CardContent className="px-4 pb-3">
            <div
              className={`text-2xl font-bold font-mono ${accColor(data.overAccuracy)}`}
            >
              {pct(data.overAccuracy)}
            </div>
            <div className="text-foreground text-xs mt-1">
              {data.overWins}W / {data.overLosses}L
            </div>
          </CardContent>
        </Card>
        <Card className="bg-background border-border">
          ...
          <CardContent className="px-4 pb-3">
            <div
              className={`text-2xl font-bold font-mono ${accColor(data.underAccuracy)}`}
            >
              {pct(data.underAccuracy)}
            </div>
            <div className="text-foreground text-xs mt-1">
              {data.underWins}W / {data.underLosses}L
            </div>
          </CardContent>
        </Card>
      </div>
            ...
            <tbody>
              {data.byEdgeTier.map((t, i) => (
                ...
                >
                  ...
                  <td
                    className={`px-3 py-1.5 text-center font-mono font-bold ${accColor(t.accuracy)}`}
                  >
                    {pct(t.accuracy)}
                  </td>
                  <td className="px-3 py-1.5 text-center text-foreground font-mono">
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <KpiCard
          label="Total Predictions"
          value={String(data.totalPredictions)}
          sub="HR Props modeled"
          color="text-foreground"
          icon={<Activity className="w-4 h-4" />}
        />
        <KpiCard
          label="OVER Accuracy"
          value={pct(data.overAccuracy)}
          sub={`${data.overWins}W / ${data.overLosses}L`}
          color={accColor(data.overAccuracy)}
          icon={<Target className="w-4 h-4" />}
        />
        <KpiCard
          ...
          sub={
            data.calibrationBias > 0.02
              ? "Over-predicting HR rate"
              : data.calibrationBias < -0.02
                ? "Under-predicting HR rate"
                : "Well-calibrated"
          }
          color={
            Math.abs(data.calibrationBias) < 0.02
              ? "text-primary"
              : "text-foreground"
          }
          icon={<TrendingUp className="w-4 h-4" />}
        ...
        <KpiCard
          ...
          color={
            data.highEdgeCount >= 5
              ? accColor(data.highEdgeAccuracy)
              : "text-foreground"
          }
          icon={<CheckCircle2 className="w-4 h-4" />}
        />
        ...
        <Card className="bg-background border-border">
          ...
          <CardContent className="px-4 pb-3">
            <div className="text-2xl font-bold font-mono text-foreground">
              {pct(data.avgModelPHr, 2)}
            </div>
            <div className="text-foreground text-xs mt-1">
              Mean model probability across all players
            </div>
          </CardContent>
        </Card>
        <Card className="bg-background border-border">
          ...
          <CardContent className="px-4 pb-3">
            <div className="text-2xl font-bold font-mono text-primary">
              {pct(data.avgActualHrRate, 2)}
            </div>
            <div className="text-foreground text-xs mt-1">
              Actual HR hit rate in sample
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        <h3 className="text-foreground font-semibold text-sm mb-3">
          Win Rate by Model P(HR) Bucket
        </h3>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart
            data={data.byProbBucket}
            margin={{ top: 5, right: 10, left: -10, bottom: 5 }}
          >
            <CartesianGrid
              strokeDasharray="3 3"
