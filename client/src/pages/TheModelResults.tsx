...
> {
  ...
  componentDidCatch(error: Error, info: { componentStack: string }) {
    console.error(
      `[SectionErrorBoundary:${this.props.label}] ERROR MESSAGE:`,
      error.message
    );
    console.error(
      `[SectionErrorBoundary:${this.props.label}] STACK:`,
      error.stack
    );
    console.error(
      `[SectionErrorBoundary:${this.props.label}] COMPONENT STACK:`,
      info.componentStack
    );
  }
  render() {
...
function accuracyColor(acc: number | null): string {
  ...
  if (acc >= 0.5) return "var(--foreground)";
...
function EdgeLeaderboardTable({
  ...
  side: "away" | "home" | "both";
  setSide: (v: "away" | "home" | "both") => void;
  withOutcome: boolean;
  setWithOutcome: (v: boolean) => void;
  sortBy: "edge" | "date" | "brier";
  setSortBy: (v: "edge" | "date" | "brier") => void;
  limit: number;
  ...
  const sorted = [...rows].sort((a, b) => {
    ...
    if (sortBy === "brier") {
      const bA =
        (market === "f5" ? a.brierF5Ml : a.brierFgMl) != null
          ? parseFloat((market === "f5" ? a.brierF5Ml : a.brierFgMl)!)
          : 999;
      const bB =
        (market === "f5" ? b.brierF5Ml : b.brierFgMl) != null
          ? parseFloat((market === "f5" ? b.brierF5Ml : b.brierFgMl)!)
          : 999;
      return bA - bB;
    }
    return 0;
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          ...
          {[0, 1, 2, 3, 5].map(v => (
            <button
              ...
              style={{
                background:
                  minEdge === v ? "var(--primary)" : "var(--background)",
                border: "1px solid var(--border)",
                borderRadius: 3,
                cursor: "pointer",
                color:
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          ...
          {(["both", "away", "home"] as const).map(s => (
            <button
              ...
              onClick={() => setSide(s)}
              style={{
                background: side === s ? "var(--primary)" : "var(--background)",
                border: "1px solid var(--border)",
                borderRadius: 3,
                cursor: "pointer",
                color:
        ...
        <button
          ...
          style={{
            background: withOutcome ? "var(--primary)" : "var(--background)",
            border: "1px solid var(--border)",
            borderRadius: 3,
            cursor: "pointer",
            color: withOutcome
        ...
        >
          ...
          {(["edge", "date", "brier"] as const).map(s => (
            <button
              ...
              style={{
                background:
                  sortBy === s ? "var(--primary)" : "var(--background)",
                border: "1px solid var(--border)",
                borderRadius: 3,
                cursor: "pointer",
                color:
        ...
        >
          ...
          <tbody>
            {sorted.map((row, i) => {
              ...
              const ml =
                market === "f5"
                  ...
                  : isAway
                    ...
                    : row.homeML;
              const awayScore =
                market === "f5" ? row.actualF5AwayScore : row.actualAwayScore;
              const homeScore =
                market === "f5" ? row.actualF5HomeScore : row.actualHomeScore;
              const scoreStr =
              ...
              const correct =
                market === "f5" ? row.f5MlCorrect : row.fgMlCorrect;
              ...
              return (
                <tr
                  ...
                  style={{
                    borderBottom: "1px solid var(--border)",
                    background:
                      i % 2 === 0 ? edgeBg(row.edgePct) : "transparent",
                  }}
                >
                  <td
                    style={{
                  ...
                  <td style={{ padding: "5px 10px", whiteSpace: "nowrap" }}>
                    ...
                    <span
                      style={{ color: "var(--foreground)", margin: "0 4px" }}
                    >
                      @
                    </span>
                    <span style={{ color: "var(--foreground)" }}>
                  ...
                  >
                    {row.modelWinPct.toFixed(1)}%
                  </td>
                  <td
                    style={{ padding: "5px 10px", color: "var(--foreground)" }}
                  >
                    {row.bookImpliedPct.toFixed(1)}%
                  </td>
                  <td style={{ padding: "5px 10px", fontWeight: 700 }}>
                    <span style={{ color: edgeColor(row.edgePct) }}>
                      {row.edgePct > 0 ? "+" : ""}
                      {row.edgePct.toFixed(2)}pp
                    </span>
                  </td>
                  <td
                    style={{ padding: "5px 10px", color: "var(--foreground)" }}
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {[100, 200, 500].map(v => (
            <button
              ...
              style={{
                background:
                  limit === v ? "var(--primary)" : "var(--background)",
                border: "1px solid var(--border)",
                borderRadius: 3,
                cursor: "pointer",
                color:
...
function EdgeScatterPlot({
  ...
  if (scatterData.length === 0) {
    return (
      ...
      >
        No games with outcomes yet. Results will appear after games are
        ingested.
      </div>
    );
  }
  ...
  return (
    <>
      ...
      >
        {[
          {
            label: "GAMES W/ OUTCOME",
            value: scatterData.length,
            color: "var(--foreground)",
          },
          {
            label: "WINS",
            value: scatterData.filter(d => d.y === 1).length,
            color: "var(--primary)",
          },
          {
            label: "LOSSES",
            value: scatterData.filter(d => d.y === 0).length,
            color: "var(--foreground)",
          },
          {
            label: "WIN RATE",
            value: `${((scatterData.filter(d => d.y === 1).length / scatterData.length) * 100).toFixed(1)}%`,
            color: "var(--foreground)",
          },
          {
      ...
      >
        ...
        >
          {market === "f5" ? "F5" : "FG"} ML EDGE (pp) vs OUTCOME — Positive
          slope = model alpha confirmed
        </div>
          ...
          <ScatterChart margin={{ top: 10, right: 20, bottom: 30, left: 20 }}>
              ...
              content={({ payload }) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ color: "#45E0A8" }}>
                      Edge: {d.x > 0 ? "+" : ""}
                      {d.x}pp
                    </div>
                    <div style={{ color: d.y === 1 ? "#45E0A8" : "#FFFFFF" }}>
                      {d.y === 1 ? "WIN" : "LOSS"} ({d.side})
                    </div>
                  </div>
                );
              }}
            ...
            {regression && (
              <ReferenceLine
                segment={[
                  {
                    x: regression.xMin,
                    y:
                      regression.slope * regression.xMin + regression.intercept,
                  },
                  {
                    x: regression.xMax,
                    y:
                      regression.slope * regression.xMax + regression.intercept,
                  },
                ]}
                ...
                strokeDasharray="6 3"
                label={{
                  value: `slope: ${regression.slope > 0 ? "+" : ""}${regression.slope.toFixed(4)}`,
                  position: "insideTopRight",
                  fill: regression.slope > 0 ? "#45E0A8" : "#FFFFFF",
                  fontSize: 10,
                }}
              />
            )}
...
function BrierHeatmap({
  ...
  selectedCell: {
    date: string;
    market: "fgMl" | "f5Ml" | "nrfi" | "fgTotal" | "f5Total";
  } | null;
  setSelectedCell: (
    v: {
      date: string;
    ...
    <div>
      <div style={{ overflowX: "auto" }}>
        ...
        >
          ...
          <span
            style={{ marginLeft: 8, color: "var(--primary)", fontSize: 10 }}
          >
            ...
            {[...heatmapData.heatmap].reverse().map(row => (
              ...
              >
                ...
                {(
                  ...
                  const v = row[field];
                  const nullCount = row[
                    field.replace("avg", "null") as keyof typeof row
                  ] as number;
                  const marketKey = (
                    field === "avgFgMl"
                      ? "fgMl"
                  ...
                  const isSelected =
                    selectedCell?.date === row.date &&
                    selectedCell?.market === marketKey;
                  return (
                    <td
                      key={field}
                      onClick={() =>
                        setSelectedCell(
                          isSelected
                            ? null
                            : { date: row.date, market: marketKey }
                        )
                      }
                      style={{
                        padding: "5px 10px",
      ...
      {selectedCell && (
        ...
        >
          ...
          >
            ...
            >
              DRILL-DOWN — {selectedCell.date} /{" "}
              {selectedCell.market.toUpperCase()}
            </div>
            <button
              type="button"
                    ...
                    mktKey === "f5Ml"
                      ...
                      : mktKey === "fgMl"
                        ...
                        : mktKey === "nrfi"
                          ? g.nrfiCorrect
                          : null;
                  const scoreAway = (mktKey as string).startsWith("f5")
                    ? g.actualF5AwayScore
                    : g.actualAwayScore;
                  const scoreHome = (mktKey as string).startsWith("f5")
                    ? g.actualF5HomeScore
                    : g.actualHomeScore;
                  return (
                    <tr
                    ...
                    >
                      ...
                      >
                        {g.awayTeam as string} @ {g.homeTeam as string}
                      </td>
                      <td
                        style={{
                          padding: "5px 8px",
...
function RollingAccuracyPanel({
  ...
  if (isLoading)
    return (
      <div
        style={{ fontSize: 11, color: "var(--foreground)", padding: "8px 0" }}
      >
        Loading rolling accuracy…
      </div>
    );
      ...
      >
        ...
        >
          ...
          <div style={{ textAlign: "center" }}>
            ...
            >
              {edgeOver != null
                ? `${edgeOver > 0 ? "+" : ""}${(edgeOver * 100).toFixed(1)}pp`
                : "—"}
            </div>
          </div>
          <div style={{ textAlign: "center" }}>
          ...
          <div>
            {isPending ? (
              ...
              <ResultBadge
                result={prop.backtestResult}
                correct={prop.modelCorrect}
              />
            )}
          </div>
        </div>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            ...
            >
              vs {isAway ? prop.homeTeam : prop.awayTeam}
            </span>
          </div>
          <div
            style={{
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <div style={{ textAlign: "center", minWidth: 52 }}>
            ...
            >
              {bookLine != null ? bookLine.toFixed(1) : "—"}
            </div>
          </div>
          {prop.actualKs != null && (
            <div style={{ textAlign: "center" }}>
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <div
                style={{
                  ...
                  fontWeight: 700,
                  color:
                    Math.abs(modelErr) <= 0.5
                      ? "var(--primary)"
                      : Math.abs(modelErr) <= 1.5
                        ? "var(--foreground)"
                        : "var(--foreground)",
                  fontFamily:
                    '"Familjen Grotesk", system-ui, -apple-system, sans-serif',
          ...
          <div>
            <VerdictBadge
              ...
              bestEdge={prop.bestEdge}
              bestMlStr={prop.bestMlStr}
            />
          </div>
          <div>
            {isPending ? (
...
export default function TheModelResults() {
  ...
  const {
    ...
    { sport: "MLB" },
    {
      enabled: !!appUser && isOwner && brierSubTab === "heatmap",
      refetchOnWindowFocus: false,
    }
  );
  ...
  const { data: drilldownData, isLoading: drilldownLoading } =
    trpc.mlbSchedule.getBrierDrilldown.useQuery(
      ...
      {
        enabled: !!appUser && isOwner && selectedCell != null,
        refetchOnWindowFocus: false,
      }
    );
  ...
  const {
    data: fgEdgeData,
    isLoading: fgEdgeLoading,
    refetch: refetchFgEdge,
  } = trpc.mlbSchedule.getFgEdgeLeaderboard.useQuery(
    {
      minEdge: fgMinEdge,
      side: fgSide,
      withOutcome: fgWithOutcome,
      limit: fgLimit,
    },
    {
      enabled: !!appUser && isOwner && marketTab === "fullgame",
      refetchOnWindowFocus: false,
    }
  );
  ...
  const {
    data: f5EdgeData,
    isLoading: f5EdgeLoading,
    refetch: refetchF5Edge,
  } = trpc.mlbSchedule.getF5EdgeLeaderboard.useQuery(
    {
      minEdge: f5MinEdge,
      side: f5Side,
      withOutcome: f5WithOutcome,
      limit: f5Limit,
    },
    {
      enabled: !!appUser && isOwner && marketTab === "first5",
      refetchOnWindowFocus: false,
    }
  );
  ...
  const {
    data: kDailyData,
    isLoading: kDailyLoading,
    refetch: refetchKDaily,
  ...
  const {
    data: kLast7Data,
    isLoading: kLast7Loading,
    refetch: refetchKLast7,
  ...
  // K-Props calibration
  const { data: kCalibData, isLoading: kCalibLoading } =
    trpc.strikeoutProps.getCalibrationMetrics.useQuery(undefined, {
      enabled: !!appUser && isOwner && marketTab === "kprops",
      refetchOnWindowFocus: false,
    });
  ...
  const { data: hrGamesData } = trpc.games.list.useQuery(
    ...
    {
      enabled: !!appUser && isOwner && marketTab === "hrprops",
      refetchOnWindowFocus: false,
    }
  );
  const hrGameIds = useMemo(
    () => (hrGamesData ?? []).map((g: { id: number }) => g.id),
    [hrGamesData]
  );
  const { data: hrPropsData, isLoading: hrPropsLoading } =
    trpc.hrProps.getByGames.useQuery(
      { gameIds: hrGameIds },
      {
        enabled:
          !!appUser &&
  ...
  const reingestMutation = trpc.mlbSchedule.triggerOutcomeIngestion.useMutation(
    {
      onSuccess: (data, vars) => {
        toast.success(
          `Re-ingested ${vars.dateStr}: ${data.written} written, ${data.errors} errors`
        );
  ...
  const fgRows: EdgeRow[] = useMemo(
    () =>
      (fgEdgeData?.rows ?? []).map((r: Record<string, unknown>) => ({
        id: r.id as number,
  ...
  const f5Rows: EdgeRow[] = useMemo(
    () =>
      (f5EdgeData?.rows ?? []).map((r: Record<string, unknown>) => ({
        id: r.id as number,
        gameDate: r.gameDate as string | null,
        awayTeam: r.awayTeam as string,
        homeTeam: r.homeTeam as string,
  ...
  const brierChartData = useMemo(() => {
    ...
    const rollingMap = new Map(
      brierData.rolling.map((r: Record<string, unknown>) => [r.gameIndex, r])
    );
    return brierData.games.map((g: Record<string, unknown>) => ({
      ...g,
    ...
    <AdminShell active="model-results">
      <div style={{ background: "var(--background)" }}>
        ...
        <header className="sticky top-14 z-40 bg-background backdrop-blur-sm border-b border-border">
          <div className="admin-container relative flex items-center py-2">
            ...
            <div className="flex items-center gap-2">
              <Button
                size="sm"
                variant="outline"
                onClick={() => setLocation("/admin/backtest")}
          ...
          <div
            className="admin-container pt-2 pb-2 flex items-center gap-1.5 overflow-x-auto scrollbar-hide"
            ...
            {(marketTab === "kprops" ||
              ...
              <>
                ...
                <button
                  ...
                  className="relative after:absolute after:-inset-2 w-7 h-7 rounded-lg flex items-center justify-center transition-colors flex-shrink-0"
                >
                  <ChevronLeft
                    size={14}
              ...
              <>
                ...
                {(["trend", "heatmap"] as const).map(st => (
                  <button
                    type="button"
                    key={st}
                    onClick={() => setBrierSubTab(st)}
                    className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold transition-colors"
                    style={{
                      background:
                        brierSubTab === st ? "transparent" : "transparent",
                      color:
                        brierSubTab === st
                          ? "var(--primary)"
                          : "var(--foreground)",
                    ...
                    {[10, 20, 30, 50].map(w => (
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 5,
                          background:
                            brierWindow === w ? "transparent" : "transparent",
                          color:
                            brierWindow === w
                              ? "var(--primary)"
                              : "var(--foreground)",
          ...
          {driftData && driftData.driftDetected && (
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  {driftData.message ??
                    "F5 share deviation exceeds threshold — recalibration recommended"}
                </span>
              </div>
            </div>
          )}
          ...
          {marketTab === "fullgame" && (
            <SectionErrorBoundary label="FULL GAME">
              ...
              >
                {/* Rolling Accuracy */}
                <div>
                  <SectionLabel sub={`Last 30 days · mlb_game_backtest table`}>
                    ROLLING ACCURACY — FULL GAME MARKETS
                  </SectionLabel>
                ...
                {brierData && (
                  <div>
                    <SectionLabel>
                      BRIER SCORE SUMMARY — FG ML + FG TOTAL
                    </SectionLabel>
                    <StatGrid>
                      <StatCard
                        label="FG ML AVG BRIER"
                        value={
                          brierData.summary.avgFgMl != null
                            ? brierData.summary.avgFgMl.toFixed(4)
                            : "—"
                        }
                        color={
                          brierData.summary.avgFgMl != null
                            ? brierColor(brierData.summary.avgFgMl)
                            : undefined
                        }
                        sub="lower = better · random = 0.25"
                      />
                      <StatCard
                        label="FG TOTAL AVG BRIER"
                        value={
                          brierData.summary.avgFgTotal != null
                            ? brierData.summary.avgFgTotal.toFixed(4)
                            : "—"
                        }
                        color={
                          brierData.summary.avgFgTotal != null
                            ? brierColor(brierData.summary.avgFgTotal)
                            : undefined
                        }
                        sub="lower = better · random = 0.25"
                      />
                      <StatCard
                        label="GAMES SCORED"
                        value={String(brierData.summary.totalGames)}
                        sub="with outcomes ingested"
                      />
                    </StatGrid>
                  </div>
                )}
                ...
                {brierSubTab === "trend" && (
                  <div>
                    <SectionLabel>
                      BRIER TREND — FG ML (solid) + FG TOTAL (dashed)
                    </SectionLabel>
                    {brierLoading ? (
                      ...
                      <div className="text-center py-12 text-foreground text-sm">
                        No Brier data yet. Run outcome ingestion first.
                      </div>
                    ) : (
                      <BrierTrendChart
                        data={brierChartData}
                        lines={[
                          {
                            key: "rollingFgMl",
                            label: `FG ML (${brierWindow}G)`,
                            color: "var(--primary)",
                          },
                          {
                            key: "rollingFgTotal",
                            label: `FG Total (${brierWindow}G)`,
                            color: "var(--primary)",
                            dashed: true,
                          },
                        ]}
                ...
                {brierSubTab === "heatmap" && (
                  <div>
                    <SectionLabel>
                      BRIER HEATMAP — ALL MARKETS × ALL DATES
                    </SectionLabel>
                    {heatmapLoading ? (
                      <div className="flex items-center justify-center py-8 gap-3">
                        <Loader2
                ...
                <div>
                  ...
                  >
                    <SectionLabel>
                      FG ML EDGE LEADERBOARD — MODEL WIN% vs NO-VIG BOOK
                      IMPLIED%
                    </SectionLabel>
                    <div
                      ...
                      {(["table", "scatter"] as const).map(t => (
                        ...
                        >
                          {t === "table" ? (
                            ...
                            <>
                              <ScatterChartIcon size={11} /> SCATTER
                            </>
                          )}
                        </button>
                      ))}
                  ...
                  {fgEdgeData?.summary && (
                    <StatGrid minColWidth={120}>
                      {[
                        {
                          label: "TOTAL GAMES",
                          value: fgEdgeData.summary.totalGames,
                          color: "var(--foreground)",
                        },
                        {
                          label: "POSITIVE EDGE",
                          value: fgEdgeData.summary.positiveEdge,
                          color: "var(--primary)",
                        },
                        {
                          label: "NEGATIVE EDGE",
                          value: fgEdgeData.summary.negativeEdge,
                          color: "var(--foreground)",
                        },
                        {
                          label: "AVG +EDGE",
                          value: `+${fgEdgeData.summary.avgPositiveEdge.toFixed(2)}pp`,
                          color: "var(--primary)",
                        },
                        {
                          label: "WIN RATE (POS EDGE)",
                          value:
                            fgEdgeData.summary.winRateOnPositiveEdge != null
                              ? `${fgEdgeData.summary.winRateOnPositiveEdge}%`
                              : "PENDING",
                          color: "var(--foreground)",
                        },
                      ].map(c => (
                  ...
                  {fgEdgeLoading ? (
                    ...
                    <EdgeLeaderboardTable
                      ...
                      market="fg"
                      minEdge={fgMinEdge}
                      setMinEdge={setFgMinEdge}
                      side={fgSide}
                      setSide={setFgSide}
                      withOutcome={fgWithOutcome}
                      setWithOutcome={setFgWithOutcome}
                      sortBy={fgSortBy}
                      setSortBy={setFgSortBy}
                      limit={fgLimit}
                      setLimit={setFgLimit}
                    />
                  ) : (
            ...
            <SectionErrorBoundary label="FIRST 5 INNINGS">
              ...
              >
                ...
                {brierData && (
                  <div>
                    <SectionLabel>
                      BRIER SCORE SUMMARY — F5 ML + F5 TOTAL
                    </SectionLabel>
                    <StatGrid>
                      <StatCard
                        label="F5 ML AVG BRIER"
                        value={
                          brierData.summary.avgF5Ml != null
                            ? brierData.summary.avgF5Ml.toFixed(4)
                            : "—"
                        }
                        color={
                          brierData.summary.avgF5Ml != null
                            ? brierColor(brierData.summary.avgF5Ml)
                            : undefined
                        }
                        sub="lower = better · random = 0.25"
                      />
                      <StatCard
                        label="F5 TOTAL AVG BRIER"
                        value={
                          brierData.summary.avgF5Total != null
                            ? brierData.summary.avgF5Total.toFixed(4)
                            : "—"
                        }
                        color={
                          brierData.summary.avgF5Total != null
                            ? brierColor(brierData.summary.avgF5Total)
                            : undefined
                        }
                        sub="lower = better · random = 0.25"
                      />
                ...
                {brierSubTab === "trend" && (
                  <div>
                    <SectionLabel>
                      BRIER TREND — F5 ML (solid) + F5 TOTAL (dashed)
                    </SectionLabel>
                    {brierLoading ? (
                      <div className="flex items-center justify-center py-8 gap-3">
                        <Loader2
                      ...
                      <BrierTrendChart
                        ...
                        lines={[
                          {
                            key: "rollingF5Ml",
                            label: `F5 ML (${brierWindow}G)`,
                            color: "var(--primary)",
                          },
                          {
                            key: "rollingF5Total",
                            label: `F5 Total (${brierWindow}G)`,
                            color: "var(--primary)",
                            dashed: true,
                          },
                        ]}
                ...
                <div>
                  ...
                  >
                    <SectionLabel>
                      F5 ML EDGE LEADERBOARD — MODEL WIN% vs NO-VIG BOOK
                      IMPLIED%
                    </SectionLabel>
                    <div
                      style={{ marginLeft: "auto", display: "flex", gap: 6 }}
                  ...
                  {f5EdgeData?.summary && (
                    <StatGrid minColWidth={120}>
                      {[
                        {
                          label: "TOTAL GAMES",
                          value: f5EdgeData.summary.totalGames,
                          color: "var(--foreground)",
                        },
                        {
                          label: "POSITIVE EDGE",
                          value: f5EdgeData.summary.positiveEdge,
                          color: "var(--primary)",
                        },
                        {
                          label: "NEGATIVE EDGE",
                          value: f5EdgeData.summary.negativeEdge,
                          color: "var(--foreground)",
                        },
                        {
                          label: "AVG +EDGE",
                          value: `+${f5EdgeData.summary.avgPositiveEdge.toFixed(2)}pp`,
                          color: "var(--primary)",
                        },
                        {
                          label: "WIN RATE (POS EDGE)",
                          value:
                            f5EdgeData.summary.winRateOnPositiveEdge != null
                              ? `${f5EdgeData.summary.winRateOnPositiveEdge}%`
                              : "PENDING",
                          color: "var(--foreground)",
                        },
                      ].map(c => (
                  ...
                  {f5EdgeLoading ? (
                    ...
                    <EdgeLeaderboardTable
                      ...
                      market="f5"
                      minEdge={f5MinEdge}
                      setMinEdge={setF5MinEdge}
                      side={f5Side}
                      setSide={setF5Side}
                      withOutcome={f5WithOutcome}
                      setWithOutcome={setF5WithOutcome}
                      sortBy={f5SortBy}
                      setSortBy={setF5SortBy}
                      limit={f5Limit}
                      setLimit={setF5Limit}
                    />
                  ) : (
              ...
              >
                ...
                {brierData && (
                  <div>
                    ...
                    <StatGrid>
                      <StatCard
                        label="NRFI AVG BRIER"
                        value={
                          brierData.summary.avgNrfi != null
                            ? brierData.summary.avgNrfi.toFixed(4)
                            : "—"
                        }
                        color={
                          brierData.summary.avgNrfi != null
                            ? brierColor(brierData.summary.avgNrfi)
                            : undefined
                        }
                        sub="lower = better · random = 0.25"
                      />
                ...
                <div>
                  <SectionLabel sub={`Date: ${formatDateNav(gameDate)}`}>
                    DAILY NRFI RESULTS
                  </SectionLabel>
                  <div
                    style={{
                  ...
                  >
                    Use the Brier Heatmap above to drill into per-game NRFI
                    results by clicking any NRFI cell. Date navigation (arrows
                    above) selects the date for K-Props and HR Props daily
                    views.
                  </div>
                </div>
              </div>
              ...
              >
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {backfillStatus ??
                        "Resolves MLBAM IDs for all K-Props pitchers missing headshots. Calls MLB Stats API."}
                    </div>
                  </div>
                  <Button
                    size="sm"
                ...
                {!kCalibLoading && kCalibData?.metrics && (
                  <div>
                    <SectionLabel>
                      ROLLING CALIBRATION — ALL TIME (
                      {kCalibData.metrics.completedProps} props)
                    </SectionLabel>
                    <StatGrid>
                      <StatCard
                        label="MODEL ACCURACY"
                        value={fmtPct(kCalibData.metrics.modelAccuracy)}
                        sub={`${kCalibData.metrics.completedProps} completed`}
                        color={accuracyColor(kCalibData.metrics.modelAccuracy)}
                      />
                      <StatCard
                        label="OVER ACCURACY"
                        value={fmtPct(kCalibData.metrics.modelOverAccuracy)}
                        sub={`${kCalibData.metrics.overCount} overs`}
                        color={accuracyColor(
                          kCalibData.metrics.modelOverAccuracy
                        )}
                      />
                      <StatCard
                        label="UNDER ACCURACY"
                        value={fmtPct(kCalibData.metrics.modelUnderAccuracy)}
                        sub={`${kCalibData.metrics.underCount} unders`}
                        color={accuracyColor(
                          kCalibData.metrics.modelUnderAccuracy
                        )}
                      />
                      <StatCard
                        label="ROLLING MAE"
                        value={fmtNum(kCalibData.metrics.mae, 3)}
                        sub="mean absolute error"
                        color={
                          kCalibData.metrics.mae <= 0.8
                            ? "var(--primary)"
                            : kCalibData.metrics.mae <= 1.5
                              ? "var(--foreground)"
                              : "var(--foreground)"
                        }
                      />
                      <StatCard
                        label="MEAN BIAS"
                        value={signedNum(kCalibData.metrics.meanBias, 3)}
                        sub="avg (actual − proj)"
                        color={
                          Math.abs(kCalibData.metrics.meanBias) <= 0.2
                            ? "var(--primary)"
                            : Math.abs(kCalibData.metrics.meanBias) <= 0.5
                              ? "var(--foreground)"
                              : "var(--foreground)"
                        }
                      />
                      <StatCard
                        ...
                        value={fmtNum(kCalibData.metrics.calibrationFactor, 4)}
                        sub="multiply proj × factor"
                        color={
                          Math.abs(kCalibData.metrics.calibrationFactor - 1) <=
                          0.03
                            ? "var(--primary)"
                            : "var(--foreground)"
                        }
                      ...
                      <StatCard
                        label="RMSE"
                        value={fmtNum(kCalibData.metrics.rmse, 3)}
                ...
                {kPropsView === "daily" &&
                  (kDailyLoading ? (
                    ...
                    <div className="text-center py-12 text-foreground text-sm">
                      No K-Props data for {formatDateNav(gameDate)}.
                    </div>
                  ) : (
                    <>
                      <div>
                        <SectionLabel>
                          DAILY SUMMARY — {formatDateNav(gameDate)}
                        </SectionLabel>
                        <StatGrid>
                          <StatCard
                            label="ACCURACY"
                            value={
                              kDailyData.results.accuracy != null
                                ? fmtPct(kDailyData.results.accuracy)
                                : "—"
                            }
                            sub={`${kDailyData.results.correct}/${kDailyData.results.completed} correct`}
                            color={accuracyColor(kDailyData.results.accuracy)}
                          />
                          <StatCard
                            label="OVER ACC"
                            value={
                              kDailyData.results.overTotal > 0
                                ? fmtPct(
                                    kDailyData.results.overCorrect /
                                      kDailyData.results.overTotal
                                  )
                                : "—"
                            }
                            sub={`${kDailyData.results.overCorrect}/${kDailyData.results.overTotal} overs`}
                            color={accuracyColor(
                              kDailyData.results.overTotal > 0
                                ? kDailyData.results.overCorrect /
                                    kDailyData.results.overTotal
                          ...
                          <StatCard
                            label="UNDER ACC"
                            value={
                              kDailyData.results.underTotal > 0
                                ? fmtPct(
                                    kDailyData.results.underCorrect /
                                      kDailyData.results.underTotal
                                  )
                                : "—"
                            }
                            sub={`${kDailyData.results.underCorrect}/${kDailyData.results.underTotal} unders`}
                            color={accuracyColor(
                              kDailyData.results.underTotal > 0
                                ? kDailyData.results.underCorrect /
                                    kDailyData.results.underTotal
                          ...
                          <StatCard
                            label="MEAN ERROR"
                            value={
                              kDailyData.results.meanError !== null
                                ? signedNum(kDailyData.results.meanError, 2)
                                : "—"
                            }
                            sub="avg (actual − proj)"
                            color={
                              kDailyData.results.meanError !== null
                                ? Math.abs(kDailyData.results.meanError) <= 0.3
                                  ? "var(--primary)"
                                  : Math.abs(kDailyData.results.meanError) <=
                                      0.8
                                    ? "var(--foreground)"
                                    : "var(--foreground)"
                                : undefined
                          ...
                          <StatCard
                            label="MAE"
                            value={
                              kDailyData.results.mae !== null
                                ? fmtNum(kDailyData.results.mae, 2)
                                : "—"
                            }
                            sub="mean absolute error"
                            color={
                              kDailyData.results.mae !== null
                                ? kDailyData.results.mae <= 0.8
                                  ? "var(--primary)"
                                  : kDailyData.results.mae <= 1.5
                                    ? "var(--foreground)"
                                    : "var(--foreground)"
                                : undefined
                            }
                      ...
                      <div>
                        <SectionLabel
                          sub={`${kDailyData.results.total} pitchers · ${kDailyData.results.correct}/${kDailyData.results.completed} correct`}
                        >
                          PER-PITCHER RESULTS
                        </SectionLabel>
                ...
                {kPropsView === "last7" &&
                  (kLast7Loading ? (
                    ...
                    <div className="text-center py-12 text-foreground text-sm">
                      No K-Props data in the last 7 days.
                    </div>
                  ) : (
                    <div>
                      <SectionLabel
                        sub={`${kLast7Data.totalProps} props across ${kLast7Data.windowDays} days`}
                      >
                      ...
                      <StatGrid>
                        <StatCard
                          ...
                          value={
                            kLast7Data.accuracy != null
                              ? fmtPct(kLast7Data.accuracy)
                              : "—"
                          }
                          sub={`${kLast7Data.correctProps}/${kLast7Data.completedProps} correct`}
                          color={accuracyColor(kLast7Data.accuracy)}
                        />
                        <StatCard
                          label="OVER ACCURACY"
                          value={
                            kLast7Data.overAccuracy != null
                              ? fmtPct(kLast7Data.overAccuracy)
                              : "—"
                          }
                          sub={`${kLast7Data.overCorrect}/${kLast7Data.overTotal}`}
                          color={accuracyColor(kLast7Data.overAccuracy)}
                        />
                        <StatCard
                          label="UNDER ACCURACY"
                          value={
                            kLast7Data.underAccuracy != null
                              ? fmtPct(kLast7Data.underAccuracy)
                              : "—"
                          }
                          sub={`${kLast7Data.underCorrect}/${kLast7Data.underTotal}`}
                          color={accuracyColor(kLast7Data.underAccuracy)}
                        />
                        <StatCard
                          label="ROLLING MAE"
                          value={
                            kLast7Data.mae != null
                              ? fmtNum(kLast7Data.mae, 3)
                              : "—"
                          }
                          sub="mean absolute error"
                          color={
                        ...
                        <StatCard
                          label="MEAN ERROR"
                          value={
                            kLast7Data.meanError != null
                              ? signedNum(kLast7Data.meanError, 3)
                              : "—"
                          }
                          sub="avg (actual − proj)"
                          color={
                            kLast7Data.meanError != null
                              ? Math.abs(kLast7Data.meanError) <= 0.2
                                ? "var(--primary)"
                                : "var(--foreground)"
                              : undefined
                          }
                      ...
                      {/* Per-date breakdown */}
                      {kLast7Data.dailyBreakdown &&
                        kLast7Data.dailyBreakdown.map(
                          (day: {
                            date: string;
                            correct: number;
                            completed: number;
                            ...
                            >
                              ...
                              >
                                ...
                                <span
                                  style={{
                                    fontSize: 11,
                                    color: accuracyColor(day.accuracy),
                                    fontWeight: 700,
                                    fontFamily:
                                      '"Familjen Grotesk", system-ui, -apple-system, sans-serif',
                                  }}
                                ...
                                >
                                  {day.correct}/{day.completed} correct
                                </span>
                                {day.mae != null && (
                                  <span
                                    style={{
                ...
                <div>
                  ...
                  {hrPropsLoading ? (
                    ...
                    <>
                      ...
                      <StatGrid>
                        {(() => {
                          const graded = hrPropsList.filter(
                            p => p.modelCorrect != null
                          );
                          const correct = graded.filter(
                            p => p.modelCorrect === 1
                          ).length;
                          const acc =
                            graded.length > 0 ? correct / graded.length : null;
                          const withEdge = hrPropsList.filter(
                            p =>
                              p.edgeOver != null &&
                              parseFloat(p.edgeOver) >= 0.02
                          );
                          const edgeCorrect = withEdge.filter(
                            p => p.modelCorrect === 1
                          ).length;
                          const edgeAcc =
                            withEdge.length > 0
                              ? edgeCorrect / withEdge.length
                              : null;
                          return (
                            <>
                              <StatCard
                                label="TOTAL PROPS"
                                value={String(hrPropsList.length)}
                                sub="all players"
                              />
                              <StatCard
                                label="GRADED"
                              ...
                              <StatCard
                                label="ACCURACY"
                                value={acc != null ? fmtPct(acc) : "—"}
                                sub="all graded"
                                color={accuracyColor(acc)}
                              />
                              <StatCard
                                label="EDGE PLAYS"
                                value={String(withEdge.length)}
                                sub="≥2pp edge"
                                color="var(--primary)"
                              />
                              <StatCard
                                label="EDGE ACCURACY"
                                value={edgeAcc != null ? fmtPct(edgeAcc) : "—"}
                                sub="≥2pp edge graded"
                                color={accuracyColor(edgeAcc)}
                              />
                            </>
                          );
                        })()}
