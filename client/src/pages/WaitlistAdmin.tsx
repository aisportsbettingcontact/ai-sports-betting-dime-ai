...
function StatCard({
  ...
  return (
    <div className="flex flex-col gap-1 px-5 py-4 rounded-xl bg-background border border-border">
      ...
      <span className="text-xs text-foreground uppercase tracking-widest font-medium">
        {label}
      </span>
    </div>
  );
...
function ContactModal({
  ...
  const [subject, setSubject] = useState(
    "Your Waitlist Application — AI Sports Betting"
  );
  ...
  function handleSend() {
    ...
    console.log(
      `[WaitlistAdmin][ACTION] Opening mailto for id=${entry.id} email=${entry.email}`
    );
...
export default function WaitlistAdmin() {
  ...
  const updateStatusMutation = trpc.waitlist.updateStatus.useMutation({
    onSuccess: (_, vars) => {
      console.log(
        `[WaitlistAdmin][ACTION] updateStatus id=${vars.id} → ${vars.status} ✓`
      );
      toast.success("Status updated", {
        description: `Entry #${vars.id} → ${vars.status}`,
      });
      invalidate();
    },
    onError: (err, vars) => {
      console.error(
        `[WaitlistAdmin][ERROR] updateStatus id=${vars.id} failed: ${err.message}`
      );
  ...
  const bulkUpdateMutation = trpc.waitlist.bulkUpdate.useMutation({
    onSuccess: data => {
      console.log(
        `[WaitlistAdmin][ACTION] bulkUpdate → ${data.updated} rows updated ✓`
      );
      toast.success("Bulk update complete", {
        description: `${data.updated} entries updated`,
      });
  ...
  const deleteMutation = trpc.waitlist.delete.useMutation({
    onSuccess: (_, vars) => {
      console.log(
        `[WaitlistAdmin][ACTION] delete id=${vars.id} ✓ (server confirmed)`
      );
    ...
    onError: (err, vars) => {
      console.error(
        `[WaitlistAdmin][ERROR] delete id=${vars.id} failed: ${err.message}`
      );
  ...
  async function handleExportCsv() {
    console.log(
      `[WaitlistAdmin][ACTION] exportCsv triggered status=${statusFilter}`
    );
    try {
  ...
  function handleBulkUpdate(status: WaitlistStatus) {
    ...
    console.log(
      `[WaitlistAdmin][ACTION] bulkUpdate ids=[${ids.join(",")}] → ${status}`
    );
  ...
  function handleDelete(id: number, email: string) {
    if (!window.confirm(`Permanently delete ${email}? This cannot be undone.`))
      return;
    console.log(
      `[WaitlistAdmin][ACTION] delete id=${id} email=${email} — optimistically removing from UI`
    );
                ...
                <tbody className="divide-y divide-border">
                  {listQuery.isLoading ? (
                    ...
                    Array.from({ length: 8 }).map((_, i) => (
                      <tr key={i} className="bg-background">
                        <td className="px-4 py-3">
                          <div className="w-4 h-4 rounded bg-background animate-pulse" />
                        </td>
                        <td className="px-4 py-3">
                          <div className="h-4 w-48 rounded bg-background animate-pulse" />
                        </td>
                        <td className="px-4 py-3">
                          <div className="h-4 w-28 rounded bg-background animate-pulse" />
                        </td>
                        <td className="px-4 py-3">
                          <div className="h-5 w-16 rounded-full bg-background animate-pulse" />
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <div className="h-4 w-24 rounded bg-background animate-pulse ml-auto" />
                        </td>
                      </tr>
                    ))
                  ) : entries.length === 0 ? (
                    <tr>
                      <td
                        colSpan={7}
                        className="px-4 py-16 text-center text-foreground"
                      >
                    ...
                    entries.map(entry => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* UTM source */}
                          <td
                            className="px-4 py-3 text-foreground text-xs max-w-[140px] truncate"
                            title={utmDisplay}
                          >
                            {utmDisplay}
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-end gap-1.5">
                              ...
                              <button
                                onClick={() => {
                                  console.log(
                                    `[WaitlistAdmin][ACTION] openContact id=${entry.id} email=${entry.email}`
                                  );
                              ...
                              <button
                                onClick={() =>
                                  handleDelete(entry.id, entry.email)
                                }
