...
function StatCard({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex flex-col gap-1 px-5 py-4 rounded-xl bg-zinc-900 border border-zinc-800">
...
function ContactModal({
  ...
  return (
    ...
    >
      <div className="w-full max-w-lg bg-zinc-900 border border-zinc-700 rounded-2xl p-6 flex flex-col gap-5 shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold text-white">Contact Applicant</h3>
            <p className="text-xs text-zinc-400 mt-0.5">{displayName} · {entry.email}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            className="w-full px-3 py-2.5 rounded-lg bg-zinc-800 border border-zinc-700 text-sm text-white placeholder-zinc-600 outline-none focus:border-indigo-500 transition-colors"
          />
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            rows={7}
            className="w-full px-3 py-2.5 rounded-lg bg-zinc-800 border border-zinc-700 text-sm text-white placeholder-zinc-600 outline-none focus:border-indigo-500 transition-colors resize-none"
          />
        </div>
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-white transition-colors disabled:opacity-50"
          >
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-lg bg-zinc-900 border border-zinc-700 text-sm text-white placeholder-zinc-600 outline-none focus:border-indigo-500 transition-colors"
            />
          </div>
        ...
        {somePageSelected && (
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-indigo-500/10 border border-indigo-500/30">
            ...
            <div className="flex items-center gap-2 ml-auto">
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-xs font-bold text-emerald-400 bg-emerald-500/15 hover:bg-emerald-500/25 border border-emerald-500/30 transition-colors disabled:opacity-50"
              >
                Approve All
              </button>
              <button
                ...
                disabled={bulkUpdateMutation.isPending}
                className="px-3 py-1.5 rounded-lg text-xs font-bold text-red-400 bg-red-500/15 hover:bg-red-500/25 border border-red-500/30 transition-colors disabled:opacity-50"
              >
                Deny All
              </button>
              <button
                ...
                disabled={bulkUpdateMutation.isPending}
                className="px-3 py-1.5 rounded-lg text-xs font-bold text-amber-400 bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 transition-colors disabled:opacity-50"
              >
                Reset to Pending
              </button>
              <button
        ...
        <div className="rounded-xl border border-zinc-800 overflow-hidden">
          ...
          {listQuery.isFetching && (
            <div className="px-4 py-2 bg-indigo-500/10 border-b border-indigo-500/20 text-xs text-indigo-400 flex items-center gap-2">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-900/60">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-zinc-400 uppercase tracking-wider">Source</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-zinc-400 uppercase tracking-wider">Joined</th>
                ...
                {listQuery.isLoading ? (
                  ...
                  Array.from({ length: 8 }).map((_, i) => (
                    <tr key={i} className="bg-zinc-950">
                      <td className="px-4 py-3"><div className="w-4 h-4 rounded bg-zinc-800 animate-pulse" /></td>
                      <td className="px-4 py-3"><div className="h-4 w-48 rounded bg-zinc-800 animate-pulse" /></td>
                      <td className="px-4 py-3"><div className="h-4 w-28 rounded bg-zinc-800 animate-pulse" /></td>
                      <td className="px-4 py-3"><div className="h-5 w-16 rounded-full bg-zinc-800 animate-pulse" /></td>
                      <td className="px-4 py-3"><div className="h-4 w-20 rounded bg-zinc-800 animate-pulse" /></td>
                      <td className="px-4 py-3"><div className="h-4 w-32 rounded bg-zinc-800 animate-pulse" /></td>
                      <td className="px-4 py-3"><div className="h-4 w-24 rounded bg-zinc-800 animate-pulse ml-auto" /></td>
                    </tr>
                  ))
                ) : entries.length === 0 ? (
                  <tr>
                    ...
                    return (
                      <tr
                        ...
                        className={`transition-colors ${
                          isSelected ? "bg-indigo-500/5" : "bg-zinc-950 hover:bg-zinc-900/50"
                        }`}
                      >
                        {/* Checkbox */}
                        ...
                        <td className="px-4 py-3 text-zinc-300">
                          {displayName || <span className="text-zinc-600 italic">—</span>}
                        </td>
                        ...
                        {/* UTM source */}
                        <td className="px-4 py-3 text-zinc-500 text-xs max-w-[140px] truncate" title={utmDisplay}>
                          {utmDisplay}
                        </td>
                          ...
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            {entry.status !== "approved" && (
                              <button
                                ...
                                title="Approve"
                                className="px-2.5 py-1 rounded-md text-xs font-semibold text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 transition-colors disabled:opacity-50"
                              >
                                Approve
                              </button>
                            )}
                            ...
                            {entry.status !== "denied" && (
                              <button
                                ...
                                disabled={updateStatusMutation.isPending}
                                title="Deny"
                                className="px-2.5 py-1 rounded-md text-xs font-semibold text-red-400 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 transition-colors disabled:opacity-50"
                              >
                                Deny
                              </button>
                            )}
                            ...
                            {entry.status !== "pending" && (
                              <button
                                ...
                                className="px-2.5 py-1 rounded-md text-xs font-semibold text-amber-400 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/20 transition-colors disabled:opacity-50"
                              >
                                Pending
                              </button>
                            )}
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-1.5 rounded-lg bg-zinc-900 border border-zinc-700 text-zinc-300 hover:text-white hover:border-zinc-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors text-xs font-medium"
              >
                ← Prev
              </button>
