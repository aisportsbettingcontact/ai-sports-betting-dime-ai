...
const DISCORD_ERRORS: Record<string, string> = {
  no_account:
    "No account found for your Discord. Purchase a subscription to get access.",
  ...
  not_in_guild:
    "You are not in the AI Sports Betting Discord server. Join first, then try again.",
  missing_role:
    "You do not have the AI Model Sub role. Purchase a subscription to get access.",
  invite_invalid:
    "This invite link is invalid, has already been used, or has expired.",
  invite_already_used:
    "This invite link has already been used. Each link is single-use.",
  invite_connect_failed:
    "An error occurred processing your invite link. Please try again.",
  ...
  discord_not_configured:
    "Discord integration is not configured. Contact support.",
...
export default function Home() {
  ...
  useEffect(() => {
    if (!authLoading && appUser) {
      ...
      console.log(
        `[Login] [STATE] Already authenticated + explicit returnPath — redirecting to ${returnPath}`
      );
  ...
  const isTransientError =
    discordError === "server_error" ||
    discordError === "db_unavailable" ||
    discordError === "timeout";
  const [retryCountdown, setRetryCountdown] = useState<number | null>(null);
  useEffect(() => {
    if (!isTransientError) return;
    console.log(
      `[Login] [AUTO_RETRY] Transient error '${discordError}' — auto-retrying in 5s`
    );
    let count = 5;
  ...
  const loginMutation = trpc.appUsers.login.useMutation({
    onSuccess: () => {
      ...
      console.log(
        "[Login] [OUTPUT] Login successful — redirecting to",
        destination
      );
  ...
  function handleFormSubmit(e: React.FormEvent) {
    ...
    console.log(
      `[Login] [INPUT] handleFormSubmit — identifier=${identifier.trim()} stayLoggedIn=true`
    );
  ...
  const requestResetMutation = trpc.appUsers.requestPasswordReset.useMutation({
    ...
    onError: err => {
      console.error(
        "[Login] [VERIFY] FAIL — requestPasswordReset error:",
        err.message
      );
      toast.error(
  ...
  function handleForgotSubmit(e: React.FormEvent) {
    ...
    console.log(
      `[Login] [INPUT] handleForgotSubmit — identifier=${forgotIdentifier.trim()}`
    );
  ...
  return (
    ...
    <div
      ...
      style={{
        background: "#000000",
        fontFamily:
          "'Familjen Grotesk', sans-serif" /* brand law: never inherit legacy Inter */,
    ...
    >
      ...
      <div
        ...
        style={{
          background: "#000000" /* brand law: flat surface, no gradients */,
      ...
      >
        ...
        <div>
          ...
          <div className="mb-10">
            ...
            <p
              className="text-[color:var(--text-secondary)] mt-4 leading-relaxed"
              style={{ fontSize: "clamp(0.875rem, 1.1vw, 1.05rem)" }}
            >
              Dime AI compares sportsbook prices against projected probability
              so every market resolves to Pass, Monitor, or Edge Detected.
            </p>
          </div>
              ...
              return (
                <div key={f.title} className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p
                      className="text-[color:var(--text-secondary)] mt-0.5 leading-snug"
        ...
        <p
          className="text-[color:var(--text-secondary)] mt-8"
          style={{ fontSize: "clamp(0.65rem, 0.8vw, 0.75rem)" }}
        >
          No guaranteed outcomes. For informational purposes only. 21+ (or legal
          betting age in your jurisdiction). Gambling problem? Call
          1-800-GAMBLER.
        </p>
      </div>
      ...
      >
        ...
        <div
          className="w-full"
          style={{ maxWidth: "clamp(320px, 40vw, 480px)" }}
        >
          {/* Desktop heading */}
          <div className="hidden lg:block mb-8">
            ...
            <p
              className="text-[color:var(--text-secondary)] mt-1"
          ...
          {!authLoading && appUser && (
            <div
              className="mb-5 px-4 py-3 rounded-lg text-xs text-[color:var(--text-secondary)] flex items-center justify-between gap-3"
              style={{ background: "#000000", border: "1px solid #FFFFFF" }}
              data-testid="login-signed-in-banner"
            >
              <span>
                Logged in as{" "}
                <span className="text-white">@{appUser.username}</span>
              </span>
              <button
                type="button"
                onClick={() => {
                  window.location.assign(resolvePostLoginPath(null));
                }}
          ...
          {discordErrorMsg && (
            <div
              className="mb-5 px-4 py-3 rounded-lg text-xs text-[color:var(--text-secondary)] text-center space-y-1.5"
              style={{ background: "#000000", border: "1px solid #FFFFFF" }}
              role="alert"
            >
              <p
                className="uppercase tracking-widest text-[11px] text-[color:var(--text-secondary)]"
                style={{
                  fontFamily:
            ...
            >
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-0 top-1/2 -translate-y-1/2 w-11 h-11 flex items-center justify-center text-[color:var(--text-secondary)] hover:text-white transition-colors"
              ...
              <div className="flex justify-end -mt-2">
                <button
                  ...
                  className="text-[11px] text-[color:var(--text-secondary)] hover:text-white underline transition-colors px-2 py-3 -my-3 -mx-2 min-h-11 inline-flex items-center"
                >
                  Forgot password?
                </button>
              </div>
            ...
            >
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-[color:var(--text-secondary)] hover:text-white transition-colors min-h-11 min-w-11 inline-flex items-center justify-center -my-2"
              ...
              {forgotSent ? (
                <div className="px-4 py-3 rounded-xl bg-transparent border border-primary text-[12px] text-primary text-center">
                  If an account exists for that username or email, a reset link
                  has been sent.
                </div>
              ) : (
                <>
                  <p className="text-xs text-[color:var(--text-secondary)]">
                    Enter your username or email and we'll send you a password
                    reset link.
                  </p>
                  ...
                  <button
                    type="submit"
                    disabled={
                      requestResetMutation.isPending || !forgotIdentifier.trim()
                    }
                    className="flex items-center justify-center gap-2 w-full px-5 py-3.5 rounded-lg font-bold text-sm text-black transition-opacity hover:opacity-85 disabled:opacity-60 disabled:cursor-not-allowed"
                    style={{ background: "#45E0A8" }}
                  >
          ...
          {!forgotOpen && (
            <div className="flex items-center gap-3 my-5">
              ...
              <span className="text-[11px] text-[color:var(--text-secondary)] font-medium">
                or
              </span>
              <div className="flex-1 h-px bg-[color:var(--border)]" />
            </div>
          ...
          {!forgotOpen && (
            <p className="text-center text-xs text-[color:var(--text-secondary)] mt-5">
          ...
          <p className="text-center text-[11px] text-[color:var(--text-secondary)] mt-6">
            By logging in you agree to gamble responsibly. For informational
            purposes only. 21+. Gambling problem? Call 1-800-GAMBLER.
          </p>
        </div>
      </div>
    </div>
