...
function SummaryCard({
  ...
  return (
    <div className="rounded-lg border border-border bg-background p-4 flex flex-col gap-1">
      ...
      <div
        className={`text-2xl font-bold tabular-nums ${count > 0 ? "text-foreground" : "text-foreground"}`}
      >
        {count}
      </div>
...
function DiscordTestPanel() {
  const [testEventType, setTestEventType] = useState<
    "CSRF_BLOCK" | "RATE_LIMIT" | "AUTH_FAIL" | "ALL"
  >("ALL");
    ...
    <div className="rounded-lg border border-border bg-background p-4 space-y-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-foreground">
          Discord Security Channel — Live Test Controls
        </span>
      ...
      <p className="text-foreground text-xs leading-relaxed">
        ...
        channel is receiving alerts correctly. Test embeds use a synthetic IP
        and are clearly labeled as tests — they will not affect event counts in
        the database.
      </p>
      ...
      <div className="flex flex-wrap items-center gap-3 p-3 rounded-md bg-background border border-border">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div>
            <div className="text-xs font-medium text-foreground">
              Fire Test Event Embed
            </div>
            <div className="text-xs text-foreground mt-0.5">
              Posts a synthetic embed to the Discord security channel to confirm
              delivery. Choose a specific event type or fire all three at once.
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button
            ...
            disabled={isTestPending}
            onClick={() =>
              fireEventMutation.mutate({ eventType: testEventType })
            }
          >
            {isTestPending ? (
      ...
      <div className="flex flex-wrap items-center gap-3 p-3 rounded-md bg-background border border-border">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div>
            ...
            <div className="text-xs text-foreground mt-0.5">
              Manually runs the daily security summary that normally posts at
              08:00 EST. Uses the last 24 hours of real event data — threat
              level, counts, and top IPs.
            </div>
          </div>
        </div>
        <Button
...
export default function SecurityEvents() {
  ...
  const pruneMutation = trpc.security.events.prune.useMutation({
    onSuccess: data => {
      toast.success(
        `Pruned ${data.deleted} events older than ${retentionDays} days`
      );
      setPruneOpen(false);
  ...
  useEffect(() => {
    if (!authLoading && (!user || !isOwner)) {
      console.warn(
        `[SECURITY] Unauthorized: user=${user?.username ?? "unauthenticated"} isOwner=${isOwner} → redirecting to /`
      );
      navigate("/");
    }
  ...
  const events: SecurityEventRow[] =
    (eventsQuery.data as SecurityEventRow[] | undefined) ?? [];
      ...
      <div className="bg-background text-foreground">
        ...
        <div className="admin-container py-6 space-y-6">
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  <TableHead className="text-foreground text-xs w-36">
                    Timestamp (EST)
                  </TableHead>
                  <TableHead className="text-foreground text-xs w-28">
                    Type
                  </TableHead>
                  <TableHead className="text-foreground text-xs w-36">
                    IP Address
                  </TableHead>
                  <TableHead className="text-foreground text-xs">
                    Blocked Origin
                  </TableHead>
                  <TableHead className="text-foreground text-xs">
                    tRPC Path
                  </TableHead>
                  <TableHead className="text-foreground text-xs w-16">
                    Method
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {isLoading ? (
                  <TableRow>
                    <TableCell
                      colSpan={6}
                      className="text-center text-foreground text-sm py-12"
                    >
                    ...
                    <TableCell colSpan={6} className="text-center py-12">
                      <div className="flex flex-col items-center gap-2 text-foreground">
                        ...
                        <span className="text-xs text-foreground">
                          Events appear here when CSRF blocks, rate limits, or
                          auth failures occur
                        </span>
                      </div>
                    </TableCell>
          ...
          <div className="text-foreground text-xs text-center">
            Auto-refreshes every 30 seconds &nbsp;·&nbsp; Showing newest{" "}
          ...
          <DialogContent className="bg-background border-border text-foreground max-w-sm">
            ...
            <div className="space-y-4 py-2">
              <p className="text-foreground text-sm">
                Delete all security events older than the selected retention
