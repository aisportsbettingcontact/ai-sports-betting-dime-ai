...
function MessageBubble({ message }: { message: Message }) {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`rounded-xl px-4 py-3 ${isUser ? "bg-transparent border border-primary text-foreground ml-auto" : "bg-card border border-border text-foreground"}`}
        >
          {isUser ? (
        ...
        >
          ...
          <button
            onClick={handleCopy}
            className="opacity-0 group-hover:opacity-100 focus-visible:opacity-100 focus-within:opacity-100 transition-opacity text-foreground hover:text-foreground"
          >
            {copied ? (
...
export default function ClaudeAssistant() {
  ...
  useEffect(() => {
    if (!loading && (!appUser || !isOwner)) setLocation("/feed/model/mlb");
  }, [loading, appUser, isOwner, setLocation]);
  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
  }, [messages]);

  const chatMutation = trpc.claude.chat.useMutation({
    onError: err => toast.error(`Claude error: ${err.message}`),
  });
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      const result = await chatMutation.mutateAsync({
        messages: history,
        context: { currentPage, additionalContext: `Focus area: ${focusArea}` },
      });
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-foreground text-xs mt-0.5">
                  Powered by Claude Fable 5 · Owner Only
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              {messages.length > 0 && (
                <Button
                  ...
                  className="text-foreground hover:text-foreground hover:bg-transparent text-xs"
                >
          ...
          <div className="admin-container flex items-center gap-3 flex-wrap">
            ...
            <Select value={currentPage} onValueChange={setCurrentPage}>
              ...
              <SelectContent className="bg-background border-border text-foreground text-xs">
                <SelectItem value="/feed/model/mlb">
                  Feed (Model Projections)
                </SelectItem>
                <SelectItem value="/betting-splits/MLB">
                  Betting Splits
                </SelectItem>
              ...
              <div className="space-y-4">
                <div className="text-center py-6">
                  ...
                  <p className="text-foreground text-sm max-w-md mx-auto">
                    Ask Claude to analyze any part of the platform, suggest
                    improvements, or generate specific code changes. Select a
                    page and focus area above to add context.
                  </p>
                </div>
                <div>
                  <div
                    className="flex items-center gap-2 text-foreground text-xs uppercase tracking-wider mb-2 cursor-pointer hover:text-foreground"
        ...
        <div className="flex-shrink-0 border-t border-border bg-background py-3">
          <div className="admin-container">
            ...
            <p className="text-[10px] text-foreground mt-1.5 text-center">
              Claude has full context of the platform's design system,
