...
export default function ManageAccount() {
  ...
  const logoutMutation = trpc.appUsers.logout.useMutation({
    onSuccess: async () => {
      console.log(
        "[ManageAccount][logout] [OUTPUT] Session cleared — redirecting to /"
      );
  ...
  const forgotPasswordMutation = trpc.appUsers.requestPasswordReset.useMutation(
    {
      onSuccess: () => {
        console.log(
          "[ManageAccount][forgotPassword] [OUTPUT] Reset email sent"
        );
        setForgotSent(true);
      ...
      onError: err => {
        console.error(
          "[ManageAccount][forgotPassword] [VERIFY] FAIL",
          err.message
        );
  ...
  const portalMutation = trpc.stripe.createPortalSession.useMutation({
    onSuccess: data => {
      console.log(
        "[ManageAccount][portal] [OUTPUT] Opening Stripe portal:",
  ...
  const cancelMutation = trpc.stripe.cancelSubscription.useMutation({
    onSuccess: data => {
      console.log(
        "[ManageAccount][cancel] [OUTPUT] Subscription cancelled at period end:",
        new Date(data.cancelAt).toISOString()
      );
  ...
  const reactivateMutation = trpc.stripe.reactivateSubscription.useMutation({
    onSuccess: () => {
      console.log(
        "[ManageAccount][reactivate] [OUTPUT] Subscription reactivated — will auto-renew"
      );
      setCancelledAt(null);
      refetch();
      utils.appUsers.me.invalidate();
  ...
  const cancelAtPeriodEnd =
    (appUser as { cancelAtPeriodEnd?: boolean }).cancelAtPeriodEnd ?? false;
  // Derive button state:
  ...
  const subButtonState: "cancel" | "reactivate" | "renew" = !appUser.hasAccess
    ? "renew"
    : cancelAtPeriodEnd || cancelledAt !== null
      ? "reactivate"
      : "cancel";
  console.log(
    `[ManageAccount][RENDER] subButtonState=${subButtonState} hasAccess=${appUser.hasAccess} cancelAtPeriodEnd=${cancelAtPeriodEnd} cancelledAt=${cancelledAt}`
  );
      ...
      <main className="flex-1 flex items-start justify-center px-4 py-8">
        <div className="w-full max-w-md space-y-4">
          ...
          <div className="bg-card border border-border rounded-xl p-5 space-y-3">
            <h1 className="text-base font-black text-foreground tracking-wider uppercase">
              Manage account
            </h1>
            ...
            <div className="space-y-1">
              ...
              {isLifetime ? (
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  >
                    {planId === "annual" ? "ANNUAL" : "MONTHLY"} · EXP{" "}
                    {formatExpiry(expiry!)}
                  </span>
                  {cancelledAt && (
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              onClick={() => {
                console.log(
                  "[ManageAccount][forgotPassword] [INPUT] email=",
                  appUser.email
                );
            ...
            {hasStripe && !isLifetime && (
              <button
                ...
                onClick={() => {
                  console.log(
                    "[ManageAccount][portal] [INPUT] origin=",
              ...
              <>
                ...
                {subButtonState === "reactivate" && (
                  <button
                    ...
                    onClick={() => {
                      console.log(
                        "[ManageAccount][reactivate] [INPUT] userId=",
                        appUser.id
                      );
                ...
                {subButtonState === "renew" && (
                  <button
                    ...
                    onClick={() => {
                      console.log(
                        "[ManageAccount][renew] [INPUT] userId=",
                        appUser.id,
            ...
            <button
              ...
              onClick={() => {
                console.log(
                  "[ManageAccount][logout] [INPUT] userId=",
                  appUser.id
                );
          ...
          <p className="text-center text-[11px] text-muted-foreground px-4">
            By using this service you agree to gamble responsibly. This tool is
          ...
          <div className="w-full max-w-sm bg-card border border-border rounded-2xl p-6 space-y-4 shadow-2xl">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  billing period. You will retain full access until then.
                </p>
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <button
                ...
                onClick={() => {
                  console.log(
                    "[ManageAccount][cancel] [INPUT] userId=",
                    appUser.id
                  );
