...
export function MobileBetTracker() {
  ...
  useEffect(() => {
    mobileOwnerTabLogger.log("mobile_bet_tracker_data_fetch_started", "bet-tracker");
  }, []);

  useEffect(() => {
    if (!betsQuery.isLoading) {
      if (betsQuery.isError) {
        mobileOwnerTabLogger.log("mobile_bet_tracker_data_fetch_failed", "bet-tracker", {
          error: betsQuery.error?.message,
        });
      } else {
        mobileOwnerTabLogger.log("mobile_bet_tracker_data_fetch_completed", "bet-tracker", {
  ...
  useEffect(() => {
    if (isEmpty) mobileOwnerTabLogger.log("mobile_bet_tracker_empty_state_rendered", "bet-tracker");
  }, [isEmpty]);
