/**
 * MobileProfile — mobile profile screen.
...
export function MobileProfile() {
  ...
  useEffect(() => {
    if (!loading && appUser) {
      mobileNavLogger.log("mobile_profile_data_loaded", "profile", {
        userId: appUser.id,
        role: appUser.role,
  ...
  const planLabel =
    appUser.stripePlanId === "monthly"
      ? "Monthly"
      : appUser.stripePlanId === "annual"
        ? "Annual"
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-2">
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${roleBg} ${roleColor} uppercase`}
                >
                  {appUser.role}
                </span>
      ...
      <div className="px-4 mt-4">
        <div className="rounded-xl bg-[#45E0A8] border border-[#45E0A8] p-4">
          ...
          <p className="text-[10px] text-white mt-1">
            No credit limits on this account
          </p>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {[
            ...
            <button
              ...
              onClick={() => {
                mobileNavLogger.log("profile_action", "profile", {
                  action: action.label,
                });
      ...
      <div className="px-4 mt-6 pb-24">
        <div className="rounded-lg bg-black border border-white p-3">
          <p className="text-[9px] text-white font-mono">
            uid: {appUser.id} • access:{" "}
            {appUser.hasAccess ? "granted" : "denied"} • discord:{" "}
            {appUser.discordId ? "linked" : "none"}
          </p>
        </div>
      </div>
    </div>
