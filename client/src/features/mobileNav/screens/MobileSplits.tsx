  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span style={{ fontSize: 12, fontWeight: 700, color: T.barInk }}>
            {a}%
          </span>
        </div>
        <div
...
export function MobileSplits() {
  ...
  useEffect(() => {
    mobileNavLogger.log("mobile_splits_data_fetch_started", "tools", {
      date: today,
    });
  }, [today]);
  ...
  useEffect(() => {
    if (!splitsQuery.isLoading) {
      if (splitsQuery.isError) {
        mobileNavLogger.log("mobile_splits_data_fetch_failed", "tools", {
          error: splitsQuery.error?.message,
        });
      } else {
        mobileNavLogger.log("mobile_splits_data_fetch_completed", "tools", {
          games: splitsQuery.data?.rows.length ?? 0,
        });
      }
    }
  ...
  const cards = useMemo(() => {
    ...
    const activeDate =
      todayRows.length > 0 ? today : [...rows].map(r => r.gameDate).sort()[0];
    const activeRows =
      todayRows.length > 0
        ? todayRows
        : rows.filter(r => r.gameDate === activeDate);

    return activeRows
      .map(r => {
        const awayTeam = r.awayAbbrev
          ? MLB_BY_ABBREV.get(r.awayAbbrev)
          : undefined;
        const homeTeam = r.homeAbbrev
          ? MLB_BY_ABBREV.get(r.homeAbbrev)
          : undefined;
        ...
        pushMarket(
          ...
          r.awayBookSpread != null
            ? `${awayNick} (${fmtSigned(r.awayBookSpread)})`
            : awayNick,
          r.homeBookSpread != null
            ? `${homeNick} (${fmtSigned(r.homeBookSpread)})`
            : homeNick,
          r.spreadAwayBetsPct,
        ...
        return {
          ...
          away: {
            nick: awayNick,
            logo: (r.awayAbbrev && logos[r.awayAbbrev]) || null,
          },
          home: {
            nick: homeNick,
            logo: (r.homeAbbrev && logos[r.homeAbbrev]) || null,
          },
          markets,
        };
      })
  ...
  const isEmpty =
    !splitsQuery.isLoading && !splitsQuery.isError && cards.length === 0;

  useEffect(() => {
    if (isEmpty)
      mobileNavLogger.log("mobile_splits_empty_state_rendered", "tools", {
        date: today,
      });
  }, [isEmpty, today]);
            ...
            {cards.map(card => (
              ...
              >
                ...
                >
                  ...
                  <span style={{ fontSize: 13, color: T.text2, flex: "none" }}>
                    vs
                  </span>
                  <TeamLogo
                    dataUri={card.home.logo}
                    alt={card.home.nick}
                    size={36}
                  />
                  <span
