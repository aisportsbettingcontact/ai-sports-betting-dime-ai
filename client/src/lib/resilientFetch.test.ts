  ...
  for (const response of items as Array<Record<string, unknown>>) {
    try {
      if ("error" in response) {
        const error = superjson.deserialize(response.error as never) as {
          code?: unknown;
        };
        const merged = { ...response, error };
        if (
          !isObject(merged.error) ||
          typeof (merged.error as { code?: unknown }).code !== "number"
        )
          return false;
      }
    } catch {
