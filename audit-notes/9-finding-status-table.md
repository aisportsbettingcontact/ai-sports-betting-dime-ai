...
| 2 | PROD-001 | **IN PROGRESS** | Privacy.tsx and Terms.tsx created. Routes registered at /privacy and /terms. Production /privacy and /terms serve legal content to ALL user agents: VERIFIED (curl-default, Googlebot, python-requests, Chrome, Wget). FE-005 CLOSED (INC-008 resolved). Remaining: owner browser check + legal review note (Terms §11 governing law needs specific state, attorney review before public launch). | None blocking |
...
- PROD-001: Owner browser check + attorney review of Terms §11 governing law (specific state needed)
...
| INC-008 | FE-005 verification timing claim | CLOSED |

**3 OPEN items:** INC-002 (user-owned), INC-007 (user-owned), DB-007 (backlogged)

**Backlog notes (no action now):**
- Terms §11: governing law should name a specific state — flag for attorney review
- PROD-001: "legally-reviewed" means attorney-reviewed before public launch; current content is compliant placeholder
