...
(async () => {
  ...
  for (const f of dbFixtures) {
    console.log(`  [STATE] ${f.fixture_id}: home=${f.home_name}(${f.home_code}) away=${f.away_name}(${f.away_code})`);
  }

  // ── 3. Build name→fixtureId lookup (bidirectional) ──────────────────────────
  // Key: normalized team name → { fixtureId, role: 'home'|'away' }
  const teamLookup = new Map();
  for (const f of dbFixtures) {
    const normalize = (s) => s.toLowerCase().replace(/[^a-z]/g, '');
    teamLookup.set(normalize(f.home_name), { fixtureId: f.fixture_id, role: 'home', fixture: f });
    teamLookup.set(normalize(f.away_name), { fixtureId: f.fixture_id, role: 'away', fixture: f });
    teamLookup.set(normalize(f.home_code), { fixtureId: f.fixture_id, role: 'home', fixture: f });
    teamLookup.set(normalize(f.away_code), { fixtureId: f.fixture_id, role: 'away', fixture: f });
    // Common AN aliases
    const homeAliases = {
  ...
  const updates = []; // { fixtureId, market, selection, line, americanOdds, impliedProb }
  ...
  for (const game of games) {
    ...
    if (res0 && res1 && res0.fixtureId === res1.fixtureId) {
      fixtureId = res0.fixtureId;
      ...
      if (res0.role === 'away' && res1.role === 'home') {
        anHomeIsTeams1 = true;
        console.log(`[STATE] ${fixtureId}: Standard AN orientation (teams[0]=away, teams[1]=home) ✓`);
      } else if (res0.role === 'home' && res1.role === 'away') {
        anHomeIsTeams1 = false;
        console.log(`[STATE] ${fixtureId}: FLIPPED AN orientation (teams[0]=home, teams[1]=away) — correcting`);
      } else {
        console.log(`[STATE] ${fixtureId}: Ambiguous orientation — defaulting to standard`);
      }
    } else {
      ...
      fixtureId = manualMap[name0] ?? manualMap[name1] ?? null;
      if (!fixtureId) {
      ...
      const dbF = dbFixtures.find(f => f.fixture_id === fixtureId);
      ...
      if (name0 === dbHomeName || name0.includes(dbHomeName) || dbHomeName.includes(name0)) {
        ...
        console.log(`[STATE] ${fixtureId}: Fallback — teams[0] matches DB home → FLIPPED`);
      } else {
        anHomeIsTeams1 = true;
        console.log(`[STATE] ${fixtureId}: Fallback — standard orientation`);
      }
    }
    ...
    const dbF = dbFixtures.find(f => f.fixture_id === fixtureId);
    console.log(`[STATE] ${fixtureId} (${dbF?.home_name} vs ${dbF?.away_name}):`);
    ...
    // Add 1X2 rows
    if (mlDbHome) updates.push({ fixtureId, market: '1X2', selection: 'home', line: null, americanOdds: mlDbHome.odds, impliedProb: americanToImplied(mlDbHome.odds) });
    if (mlDbAway) updates.push({ fixtureId, market: '1X2', selection: 'away', line: null, americanOdds: mlDbAway.odds, impliedProb: americanToImplied(mlDbAway.odds) });
    if (mlDbDraw) updates.push({ fixtureId, market: '1X2', selection: 'draw', line: null, americanOdds: mlDbDraw.odds, impliedProb: americanToImplied(mlDbDraw.odds) });
    ...
    if (mlDbHome && mlDbDraw && mlDbAway) {
      ...
      updates.push({ fixtureId, market: 'DOUBLE_CHANCE', selection: 'home_draw', line: null, americanOdds: dc1XOdds, impliedProb: dc1X });
      updates.push({ fixtureId, market: 'DOUBLE_CHANCE', selection: 'away_draw', line: null, americanOdds: dcX2Odds, impliedProb: dcX2 });
    }
    ...
    if (over) {
      ...
      updates.push({ fixtureId, market: 'TOTAL', selection: 'over', line: over.value ?? null, americanOdds: over.odds, impliedProb: americanToImplied(over.odds) });
    }
    if (under) {
      updates.push({ fixtureId, market: 'TOTAL', selection: 'under', line: under.value ?? null, americanOdds: under.odds, impliedProb: americanToImplied(under.odds) });
    }
    ...
    if (spreadDbHome) {
      ...
      updates.push({ fixtureId, market: 'ASIAN_HANDICAP', selection: selH, line: spreadDbHome.value ?? null, americanOdds: spreadDbHome.odds, impliedProb: americanToImplied(spreadDbHome.odds) });
    }
    if (spreadDbAway) {
      const selA = `away${spreadDbAway.value >= 0 ? '+' : ''}${spreadDbAway.value}`;
      updates.push({ fixtureId, market: 'ASIAN_HANDICAP', selection: selA, line: spreadDbAway.value ?? null, americanOdds: spreadDbAway.odds, impliedProb: americanToImplied(spreadDbAway.odds) });
    }
  }
  ...
  const fixtureIds = [...new Set(updates.map(u => u.fixtureId))];
  console.log(`[STEP] Updating DB for fixtures: ${fixtureIds.join(', ')}`);

  // Delete existing DK rows for these fixtures
  for (const fid of fixtureIds) {
    const [delResult] = await conn.query(
      `DELETE FROM wc2026_odds_snapshots WHERE fixture_id = ? AND book_id = 68`,
      [fid]
    );
  ...
  for (const u of updates) {
    await conn.query(
      `INSERT INTO wc2026_odds_snapshots (fixture_id, book_id, market, selection, line, american_odds, implied_prob, snapshot_ts, is_closing)
       VALUES (?, 68, ?, ?, ?, ?, ?, ?, 1)`,
      [u.fixtureId, u.market, u.selection, u.line, u.americanOdds, u.impliedProb.toFixed(6), snapshotTs]
    );
    inserted++;
    console.log(`[OUTPUT] Inserted: ${u.fixtureId} | DK | ${u.market} | ${u.selection} | line=${u.line ?? 'null'} | odds=${u.americanOdds}`);
  }
  ...
  const [finalRows] = await conn.query(`
    SELECT o.fixture_id, o.market, o.selection, o.line, o.american_odds,
      ht.name AS home_name, at.name AS away_name
  ...
  for (const r of finalRows) {
    ...
    console.log(`  [${r.fixture_id}] ${r.home_name} vs ${r.away_name} | ${r.market} ${r.selection}${lineStr} => ${r.american_odds}`);
  }

  // Check for TUR-PAR: if no ML odds from AN, report it
  const turParML = finalRows.filter(r => r.fixture_id === 'wc26-g-030' && r.market === '1X2');
