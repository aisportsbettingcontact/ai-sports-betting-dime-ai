 ...
 * - Completed matchs (June 11-24) with scores
...
`);
console.log(`[MATCHS] ${matchs.length} completed matchs`);
...
// Check which matchs have DK odds
const matchsWithDK = new Set(dkOdds.map(o => o.match_id));
console.log(`[COVERAGE] ${matchsWithDK.size}/54 matchs have DK odds`);

// Show matchs WITHOUT DK odds
const allMatchIds = new Set(matchs.map(f => f.match_id));
const missingDK = [...allMatchIds].filter(id => !matchsWithDK.has(id));
if (missingDK.length > 0) {
  console.log(`[MISSING_DK] Matchs without DK odds: ${missingDK.join(', ')}`);
  
  // Check what books they have
  const [altBooks] = await db.execute(`
    SELECT match_id, book_id, COUNT(*) as cnt
    FROM wc2026_odds_snapshots
    WHERE match_id IN (${missingDK.map(() => '?').join(',')})
    AND book_id != 0
    GROUP BY match_id, book_id
    ORDER BY match_id, book_id
  `, missingDK);
  console.log('[ALT_BOOKS]', JSON.stringify(altBooks));
}

// Save to files
writeFileSync('/home/ubuntu/wc2026_completed_matchs.json', JSON.stringify(matchs, null, 2));
...
console.log('  /home/ubuntu/wc2026_completed_matchs.json');
